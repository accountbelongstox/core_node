...
不要使用线程锁，本项目禁止使用线程锁可以使用其他判断方法 解决。

能否对一个设备 的引导 贞缓存，之后如果有新设备 连接 ，先传引导贞防止报错，之后继续现在的流直接推， 相当于省去很多复杂的工作，是否有可能，查看官方 文档 。不要使用线程锁，本项目禁止使用线程锁可以使用其他判断方法 解决。

目前两个设备连接时，推送失败，你是否明白，scrcpy只连接 一次，任何设备 都会先有一个初始贞，之后新连接的设备 相当于只只是复制scrpy的流分发给不同的客户端，而不重复使用scrcpy连接设备 。
