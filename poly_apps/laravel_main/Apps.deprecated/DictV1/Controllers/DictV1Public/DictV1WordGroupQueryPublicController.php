...
class DictV1WordGroupQueryPublicController
{
    ...
    public static function isGroupNameExist($rowGname, $rowGid = null)
    {
        ...
        if ($success == true) {
            $uid = Auth::id();
            if ($queryByGname) {
                $existGroup = DictV1WordGroupModel::where('gname', $gcredential)->where('uid', $uid)->first();
            } else {
                $existGroup = DictV1WordGroupModel::where('gid', $gcredential)->where('uid', $uid)->first();
            }
