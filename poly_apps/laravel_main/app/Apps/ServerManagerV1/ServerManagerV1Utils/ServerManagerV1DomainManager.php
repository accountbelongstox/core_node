...
server {
    ...
    access_log /var/log/nginx/$domain.access.log;
        ...
        if ($sslEnabled) {
            // Generate HTTP redirect configuration (shared builder stanza)
            $httpRedirectConfig = "# HTTP to HTTPS redirect for $domain\n"
                . "# Generated by ServerManagerV1 at " . date('Y-m-d H:i:s') . "\n\n"
                . ServerManagerV1NginxConfigBuilder::renderHttpRedirectServer($serverNames);

            // Generate SSL configuration as separate file (shared builder stanza)
            $sslConfig = "# HTTPS configuration for $domain\n"
                . "# Generated by ServerManagerV1 at " . date('Y-m-d H:i:s') . "\n\n"
                . "# HTTPS server block\n"
                . "server {\n"
                . ServerManagerV1NginxConfigBuilder::renderTlsStanza("$certDir/fullchain.pem", "$certDir/privkey.pem")
                . "\n    server_name $serverNames;\n\n"
                . "    root $documentRoot;\n\n"
    ...
    private static function generateRedirectConfig(string $sourceDomain, string $targetDomain, int $redirectCode, array $config): bool
    {
        ...
        // HTTPS configuration (shared builder stanza)
        if ($sslEnabled) {
            $baseDomain = self::extractBaseDomain($targetDomain);
            $certDir = ServerManagerV1PathConfig::getSslCertDir($baseDomain);
            $httpConfig .= "\nserver {\n";
            $httpConfig .= ServerManagerV1NginxConfigBuilder::renderTlsStanza("$certDir/fullchain.pem", "$certDir/privkey.pem");
