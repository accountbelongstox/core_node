...
final class RelayDeviceSignatureService
{
    public function __construct(private readonly RelayNonceRepository $nonces)
    {
    }

    public function verify(Request $request): array
    {
        $protocol = trim((string) $request->header(RelayContract::header('protocol'), ''));
        $deviceId = trim((string) $request->header(RelayContract::header('device_id'), ''));
        $credentialId = trim((string) $request->header(RelayContract::header('credential_id'), ''));
        $credentialVersionText = trim((string) $request->header(RelayContract::header('credential_version'), ''));
        $timestampText = trim((string) $request->header(RelayContract::header('timestamp'), ''));
        $nonce = trim((string) $request->header(RelayContract::header('nonce'), ''));
        $contentSha256 = strtolower(trim((string) $request->header(RelayContract::header('content_sha256'), '')));
        $signatureText = trim((string) $request->header(RelayContract::header('signature'), ''));
        ...
        if (!hash_equals(RelayContract::protocolVersion(), $protocol)) {
            throw new RelayDomainException('signature_protocol_invalid', 403);
        }
        if (!Str::isUuid($deviceId) || $credentialVersion < 1) {
            throw new RelayDomainException('signature_device_invalid', 403);
        }
        if ($timestamp < 1 || abs(time() - $timestamp) > RelayContract::duration('signature_clock_skew_seconds')) {
            throw new RelayDomainException('signature_timestamp_invalid', 403);
        }
        if ($nonce === '' || strlen($nonce) > 128 || preg_match('/^[A-Za-z0-9_-]+$/', $nonce) !== 1) {
            throw new RelayDomainException('signature_nonce_invalid', 403);
        }
        if (preg_match('/^[a-f0-9]{64}$/', $contentSha256) !== 1
            || !hash_equals(hash('sha256', $body), $contentSha256)) {
            throw new RelayDomainException('signature_body_digest_invalid', 403);
        }
        ...
        $canonical = implode("\n", [
            ...
            RelayContract::canonicalPath($request->getPathInfo()),
            RelayContract::canonicalRawQuery($request->getQueryString()),
            $deviceId,
            (string) $timestamp,
            $nonce,
            $contentSha256,
        ...
        if (!function_exists('sodium_crypto_sign_verify_detached')) {
            throw new RelayDomainException('signature_runtime_missing', 500);
        }
        if (!is_string($publicKey) || strlen($publicKey) !== SODIUM_CRYPTO_SIGN_PUBLICKEYBYTES
            ...
            throw new RelayDomainException('signature_invalid', 403);
        }

        $this->nonces->claim($credentialScope, $nonce);
        $request->attributes->set('relay_device_id', $deviceId);
        $request->attributes->set('relay_credential_id', $credentialId);
        $request->attributes->set('relay_credential_version', $credentialVersion);
    ...
    private function resolveIdentity(
        ...
        if ($request->routeIs('relay.enrollment.create')) {
            ...
            if ($credentialId !== '' || !hash_equals($deviceId, $bodyDeviceId) || $credentialVersion !== $bodyKeyVersion) {
                throw new RelayDomainException('signature_enrollment_identity_invalid', 403);
            }
        ...
        if ($request->routeIs('relay.enrollment.status')) {
            ...
            if ($credentialId !== '' || $enrollment === null || (int) $enrollment->key_version !== $credentialVersion) {
                throw new RelayDomainException('signature_enrollment_not_found', 403);
            }
        ...
        if (!Str::isUuid($credentialId)) {
            throw new RelayDomainException('signature_credential_missing', 403);
        }
        $registeredDevice = RelayDeviceModel::query()
            ->where('device_id', $deviceId)
        ...
        if ($registeredDevice === null
            || $credential === null
            || (string) $credential->status !== RelayConstants::CREDENTIAL_ACTIVE
            || $credential->revoked_at !== null
            || $credential->expires_at === null
            || $credential->expires_at->lte(now())) {
            throw new RelayDomainException('signature_credential_invalid', 403);
        }
