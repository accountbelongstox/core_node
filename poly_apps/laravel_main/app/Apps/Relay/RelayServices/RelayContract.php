...
final class RelayContract
{
    ...
    public static function assertSameDigest(string $receivedDigest, array $context = []): void
    {
        ...
        Log::warning('[Relay] Contract digest rejected', $context + [
            'expected_digest' => $expected,
            'received_digest' => $receivedDigest,
        ]);
        throw new RelayDomainException('contract_digest_conflict', 409);
    }
    ...
    public static function header(string $name): string
    {
        ...
        if ($value === '') {
            throw new RelayDomainException('contract_header_missing', 500, ['name' => $name]);
        }
    ...
    public static function duration(string $name): int
    {
        ...
        if ($value < 1) {
            throw new RelayDomainException('contract_duration_missing', 500, ['name' => $name]);
        }
    ...
    public static function limit(string $name): int
    {
        ...
        if ($value < 1) {
            throw new RelayDomainException('contract_limit_missing', 500, ['name' => $name]);
        }
    ...
    public static function rateLimit(string $name): int
    {
        ...
        if ($value < 1) {
            throw new RelayDomainException('contract_rate_limit_missing', 500, ['name' => $name]);
        }
    ...
    public static function publicUrl(string $name): string
    {
        ...
        if ($value === '') {
            throw new RelayDomainException('contract_public_url_missing', 500, ['name' => $name]);
        }
    ...
    public static function eventPayloadFields(string $name): array
    {
        ...
        if (!is_array($values) || $values === []) {
            throw new RelayDomainException('contract_event_payload_missing', 500, ['name' => $name]);
        }
    ...
    public static function event(string $name): string
    {
        ...
        if ($value === '') {
            throw new RelayDomainException('contract_event_missing', 500, ['name' => $name]);
        }
    ...
    public static function endpoint(string $name): string
    {
        ...
        if ($value === '') {
            throw new RelayDomainException('contract_endpoint_missing', 500, ['name' => $name]);
        }
    ...
    public static function topic(string $name, array $tokens): string
    {
        ...
        $resolved = $template;
        if ($template === '') {
            throw new RelayDomainException('contract_topic_missing', 500, ['name' => $name]);
        }
    ...
    public static function canonicalPath(string $path): string
    {
        ...
        if (str_contains($rawPath, '?')
            ...
            || str_contains($rawPath, '\\')) {
            throw new RelayDomainException('signature_path_invalid', 400);
        }
    ...
    public static function canonicalQuery(array $query): string
    {
        ...
        foreach ($query as $rawKey => $rawValue) {
            if (!is_string($rawKey)) {
                throw new RelayDomainException('query_key_invalid', 422);
            }
            $values = is_array($rawValue) ? $rawValue : [$rawValue];
            foreach ($values as $rawItem) {
                if (!is_string($rawItem)) {
                    throw new RelayDomainException('query_value_invalid', 422);
                }
    ...
    private static function load(): void
    {
        ...
        foreach ($requiredSections as $section) {
            if (!array_key_exists($section, $document)) {
                throw new RelayDomainException('contract_section_missing', 500, ['name' => $section]);
            }
        }
        ...
        if (count(array_unique(array_values($document['events']), SORT_STRING)) !== count($document['events'])) {
            throw new RelayDomainException('contract_event_missing', 500, ['name' => 'unique_event_values']);
        }
        ...
        if (($document['route_policy_matching']['precedence'] ?? null) !== ['exact', 'prefix', 'suffix']
            ...
            throw new RelayDomainException('contract_route_profile_invalid', 500, ['name' => 'route_policy_matching']);
        }
        if ((string) ($document['signature_profile']['algorithm'] ?? '') !== 'ed25519') {
            throw new RelayDomainException('contract_signature_profile_invalid', 500);
        }
        ...
        foreach ($document['route_policies'] as $policy) {
            ...
            foreach ($requiredProfileFields as $field) {
                if (!array_key_exists($field, $profile)) {
                    throw new RelayDomainException('contract_route_profile_invalid', 500, ['name' => $profileName]);
                }
                if (array_key_exists($field, $policy) && $policy[$field] !== $profile[$field]) {
                    throw new RelayDomainException('contract_route_policy_conflict', 500, ['name' => (string) ($policy['value'] ?? '')]);
                }
            }
        }
    ...
    private static function decodeFormComponent(string $value): string
    {
        ...
        for ($index = 0, $length = strlen($value); $index < $length; $index++) {
            ...
            if (strlen($triplet) !== 2 || !ctype_xdigit($triplet)) {
                throw new RelayDomainException('signature_query_invalid', 400);
            }
        }
    ...
    private static function assertPublicUrls(array $document): void
    {
        ...
        if (!is_array($originParts)
            ...
            throw new RelayDomainException('contract_public_url_invalid', 500, ['name' => 'laravel_api_origin']);
        }
        if (!is_array($hubParts)
            ...
            throw new RelayDomainException('contract_public_url_invalid', 500, ['name' => 'mercure_hub']);
        }
    }
    ...
    private static function assertRequiredNames(mixed $section, array $names, string $errorCode): void
    {
        ...
        foreach ($names as $name) {
            if (!is_string($values[$name] ?? null) || trim((string) $values[$name]) === '') {
                throw new RelayDomainException($errorCode, 500, ['name' => $name]);
            }
        }
    }
