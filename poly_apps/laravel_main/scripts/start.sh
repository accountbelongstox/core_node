...
PHP_ENSURE_SCRIPT="${INSTALL_SHELLS_DIR}/33_ensure_php85_intelligent.sh"
COMPOSER_INSTALL_SCRIPT="${INSTALL_SHELLS_DIR}/36_install_composer.sh"
NODE_INSTALL_SCRIPT="${INSTALL_SHELLS_DIR}/17_install_node_24.sh"
SWOOLE_INSTALL_SCRIPT="${INSTALL_SHELLS_DIR}/34_install_swoole.sh"
P7ZIP_INSTALL_SCRIPT="${INSTALL_SHELLS_DIR}/45_install_p7zip.sh"
POSTGRES_INSTALL_SCRIPT="${INSTALL_SHELLS_DIR}/48_install_postgresql.sh"
PHP_PGSQL_ENSURE_SCRIPT="${INSTALL_SHELLS_DIR}/49_ensure_php_pgsql.sh"
...
# find driver". 49_ensure_php_pgsql.sh builds + enables it (phpize) idempotently.
...
# 48_install_postgresql.sh); start.sh only ENSURES the outcome and is fully
# idempotent:
#   * dd.sh PRIORITY: set the shared global-var flags so the canonical chain
#     agrees, then invoke 48_install_postgresql.sh (itself idempotent/self-healing)
...
# every app start. The canonical 48_install_postgresql.sh sources it itself.
...
# sys:init still reported "Node.js not found"). 17_install_node_24.sh is gated by
