@echo off
REM laravel_main Install Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for dependency management
REM This bat file serves as the Windows entry point for installing laravel_main dependencies
REM IMPORTANT: All Poly applications MUST use BAT triggers to call PS1 scripts

echo [INFO] Installing laravel_main application dependencies...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] laravel_main installation failed with exit code: %ERRORLEVEL%
    exit /b %ERRORLEVEL%
)

echo [SUCCESS] laravel_main installation completed
exit /b 0
