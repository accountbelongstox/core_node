...
def main():
    # 输入图片路径
    input_image = Path(r"D:\programing\core_node\poly_apps\flutter_bloom\assets\apps\app_bank\images\certificate_icons_source.png")
    ...
    temp_output_dir = input_image.parent / f"extracted_certificate_icons_{timestamp}"
    ...
    # 图标映射：提取的图标索引 -> 目标文件名
    # 需要4个图标：流水打印、资信证明、个人信用报告、银行卡持有
    icon_mapping = {
        0: "icon_certificate_statement.png",      # 流水打印
        1: "icon_certificate_credit.png",         # 资信证明
        2: "icon_certificate_report.png",         # 个人信用报告
        3: "icon_certificate_card.png",           # 银行卡持有
    }
    ...
    if len(extracted_icons) < 4:
        print(f"\n警告: 只提取到 {len(extracted_icons)} 个图标，需要至少4个")
        print("请手动选择要使用的图标")
        print("\n提取的图标列表:")
        ...
        try:
            ...
            # 使用用户选择的图标
            for i, icon_idx in enumerate(selected_indices[:4]):
