...
class Step3PlatformController:
    ...
    def save_results_to_unified_data(self) -> bool:
        ...
        try:
            ...
            # Save to unified data exchange
            result = self.data_exchange.save_platform_targets(platform_targets)

            if result['success']:
                PrintHelper.info(f"Platform results saved to unified data exchange: {len(platform_targets)} targets across {len(platform_results['platforms'])} platforms", source=self.step_name)
                return True
            else:
                PrintHelper.error(f"Failed to save to unified data exchange: {result.get('error')}", source=self.step_name)
                return False
