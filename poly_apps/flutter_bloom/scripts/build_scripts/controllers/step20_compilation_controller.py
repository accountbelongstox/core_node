...
class Step20CompilationController:
    ...
    def _generate_compilation_commands(self, compilation_option: str, build_info: Dict[str, Any]) -> Dict[str, str]:
        ...
        commands.update(self._generate_platform_commands(build_root, flutter_command, build_platform, build_info))
    ...
    def _generate_platform_commands(self, build_root: str, flutter_command: str, platform: str, build_info: Dict[str, Any]) -> Dict[str, str]:
        """Generate platform-specific commands with modular scripts"""
        import os
        ...
        if os.name == 'nt':  # Windows
            ...
            project_root = build_info.get('project_root') or ''
            set_android_env_ps1 = ''
            if project_root and platform.lower() == 'android':
                set_env_path = Path(project_root) / "scripts" / "win_common" / "Set-AndroidSdkEnv.ps1"
                set_android_env_ps1 = PathHelper.normalize_for_powershell(str(set_env_path))

            script_content = rf'''# Flutter Build Orchestrator
# Generated by step20_compilation_controller.py

'''
            if set_android_env_ps1:
                script_content += rf'''# Ensure Android SDK env (auto-detect ANDROID_HOME) before build
$setEnvScript = "{set_android_env_ps1}"
if (Test-Path -LiteralPath $setEnvScript) {{
    . $setEnvScript -Quiet
    Write-Host "[BUILD] Android SDK env applied" -ForegroundColor Gray
}} else {{
    Write-Host "[BUILD] Set-AndroidSdkEnv.ps1 not found, using existing env" -ForegroundColor Gray
}}

'''
            script_content += rf'''Set-Location "{build_root_normalized}"
