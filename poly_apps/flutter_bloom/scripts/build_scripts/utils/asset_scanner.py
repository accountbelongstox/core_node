...
class AssetScanner:
    ...
    def collect_resource_directories(self, temp_build_root: Path, app_name: str) -> List[Path]:
        ...
        PrintHelper.step_header("COLLECTING RESOURCE DIRECTORIES")
        ...
        if external_resource_dir.exists() and external_resource_dir.is_dir():
            ...
            PrintHelper.success(f"[1] External static resource directory: {external_resource_dir}")
        else:
            PrintHelper.warning(f"[1] External static resource directory not found: {external_resource_dir}")
        ...
        if builtin_app_dir.exists() and builtin_app_dir.is_dir():
            ...
            PrintHelper.success(f"[2] Built-in app directory: {builtin_app_dir}")
        else:
            PrintHelper.warning(f"[2] Built-in app directory not found: {builtin_app_dir}")

        # 3. Common directory (app_main) (lowest priority)
        common_dir = temp_build_root / "assets" / "apps" / "app_main"
        if common_dir.exists() and common_dir.is_dir():
            resource_dirs.append(common_dir)
            PrintHelper.success(f"[3] Common directory (app_main): {common_dir}")
        else:
            PrintHelper.warning(f"[3] Common directory (app_main) not found: {common_dir}")
    ...
    def _print_directory_status_summary(self, temp_build_root: Path, app_name: str):
        ...
        PrintHelper.section_header("RESOURCE DIRECTORIES STATUS")
    ...
    def scan_all_required_images(self) -> Dict[str, Dict]:
        ...
        """
        PrintHelper.step_header("INTELLIGENT IMAGE SCANNING FOR 7 REQUIRED IMAGES")
        ...
        PrintHelper.info("[STEP 1] Scanning for LOGO in icons subdirectories...")
        logo_info = self._scan_and_create_image_info('logo')
        if logo_info:
            image_data['logo'] = logo_info
            PrintHelper.success(f"✓ LOGO found: {logo_info['final_filename']} [{logo_info.source}]")
        else:
            PrintHelper.warning("✗ LOGO not found")

        # Step 2: Scan for icon group (co-location with logo if found)
        PrintHelper.info("[STEP 2] Scanning for ICON GROUP (ic_icon, ic_launcher, notification_icon, transa_launcher)...")
        ...
        for icon_type in icon_types:
            ...
            if icon_info:
                ...
                PrintHelper.success(f"✓ {icon_type.upper()} found: {icon_info['final_filename']} [{icon_info.source}]")
            else:
                PrintHelper.warning(f"✗ {icon_type.upper()} not found")

        # Step 3: Scan for background in launch subdirectories (priority order)
        PrintHelper.info("[STEP 3] Scanning for BACKGROUND in launch subdirectories...")
        background_info = self._scan_and_create_image_info('background')
        if background_info:
            image_data['background'] = background_info
            PrintHelper.success(f"✓ BACKGROUND found: {background_info['final_filename']} [{background_info.source}]")
        else:
            PrintHelper.warning("✗ BACKGROUND not found")

        # Step 4: Scan for splash (co-location with background if found)
        PrintHelper.info("[STEP 4] Scanning for SPLASH...")
        splash_info = self._scan_splash_type(image_data.get('background'))
        if splash_info:
            image_data['splash'] = splash_info
            PrintHelper.success(f"✓ SPLASH found: {splash_info['final_filename']} [{splash_info.source}]")
        else:
            PrintHelper.warning("✗ SPLASH not found")

        # Step 5: Apply intelligent fallback mechanisms
        PrintHelper.info("[STEP 5] Applying intelligent fallback mechanisms...")
    ...
    def _scan_icon_group_type(self, image_type: str, logo_info: Optional[Dict]) -> Optional[Dict]:
        ...
        if logo_info and logo_info.get('directory'):
            ...
            PrintHelper.info(f"  → Using logo directory for {image_type}: {logo_info['directory']}")
    ...
    def _scan_splash_type(self, background_info: Optional[Dict]) -> Optional[Dict]:
        ...
        if background_info and background_info.get('directory'):
            ...
            PrintHelper.info(f"  → Using background directory for splash: {background_info['directory']}")
            ...
            try:
                ...
                data = {
                    ...
                    "processed_images": {
                        ...
                        for image_type, image_info in self.scan_results.items()
                    }
                }
    ...
    def update_scan_results(self, results: Dict[str, Dict]):
        ...
        PrintHelper.info(f"Scan results updated with {len(results)} entries")
    ...
    def show_image_selection_menu(self, menu_helper, images: List[Dict], image_type: str) -> Optional[Dict]:
        ...
        if not images:
            PrintHelper.warning(f"[MENU] No {image_type} images found to select from")
            return None
