...
```sql
ALTER TABLE users ADD COLUMN occupation VARCHAR(50) NULL;
ALTER TABLE users ADD COLUMN daily_words_target INT DEFAULT 20;
ALTER TABLE users ADD COLUMN daily_study_time INT DEFAULT 30; -- minutes
ALTER TABLE users ADD COLUMN initialization_completed_at TIMESTAMP NULL;
...
CREATE TABLE ai_usage_tracking (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    user_id BIGINT NULL, -- NULL for unauthenticated users
    session_id VARCHAR(255) NULL, -- For tracking unauthenticated users
    endpoint VARCHAR(255) NOT NULL,
    used_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_user_date (user_id, used_at),
    INDEX idx_session_date (session_id, used_at)
);
```

### Vocabulary Libraries Table
```sql
...
CREATE TABLE reading_book_chapters (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
...
- `occupation`: Must be one of: "student", "teacher", "engineer", "doctor", "business", "other"
- `daily_words_target`: Integer between 5 and 100
- `daily_study_time`: Integer between 10 and 180 (minutes)
- `learning_languages`: Array of valid language codes (ISO 639-1)
- `preferences`: Valid JSON object
- `mastery_level`: Decimal between 0.00 and 1.00
- `status`: Must be one of: "learning", "reviewing", "mastered", "archived"

## Implementation Notes

1. **Initialization Status Check**: The endpoint should check if all required fields are set and non-null.
2. **Partial Updates**: The PATCH endpoint should only update provided fields, leaving others unchanged.
3. **Default Values**: If not provided, use sensible defaults:
   - `daily_words_target`: 20
   - `daily_study_time`: 30 minutes
   - `learning_languages`: ["en"] (if not set)
4. **Backward Compatibility**: Existing users without these fields should be treated as "not initialized".
5. **AI Usage Limits**: Unauthenticated users have 10 uses per day, tracked by session_id. Authenticated users have higher limits (e.g., 100 per day).
6. **Memory Bank Performance**: Memory bank queries should be optimized with proper indexes. Audio and translation data are NOT included in memory bank responses to avoid loading large amounts of data. They should be requested separately when needed.
7. **Pagination**: All list endpoints support pagination with `page` and `per_page` parameters. Response includes pagination metadata.
8. **File Upload**: Supported file formats: PDF, DOCX, TXT, EPUB. File size limit: 10MB.
9. **Text Extraction**: Text extraction from files should handle multiple languages and extract unique words, filtering out common stop words.
