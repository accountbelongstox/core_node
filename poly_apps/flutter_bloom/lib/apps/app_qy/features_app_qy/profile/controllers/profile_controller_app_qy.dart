...
import 'package:qyflutter/apps/app_qy/services_app_qy/user_api_app_qy_service.dart';
...
class ProfileControllerAppQy extends GetxController {
  ...
  late final UserApiAppQyService _userApiService;
  ...
  void _initializeDependencies() async {
    ...
    if (Get.context != null) {
      _userApiService = UserApiAppQyService(Get.context!);
    }
  }
  ...
  T? getUserPreference<T>(String key) {
    try {
      ...
      if (key == 'qy_notifications_enabled') {
        return true as T?;
      } else if (key == 'qy_privacy_level') {
        return 'public' as T?;
      }
      return null;
    }
  ...
  bool get notificationsEnabled {
    return getUserPreference<bool>('qy_notifications_enabled') ?? true;
  }

  /// Toggle notifications setting
  Future<void> toggleNotifications() async {
    await setUserPreference('qy_notifications_enabled', !notificationsEnabled);
  }

  /// Get privacy level setting
  String get privacyLevel {
    return getUserPreference<String>('qy_privacy_level') ?? 'public';
  }

  /// Set privacy level
  Future<void> setPrivacyLevel(String level) async {
    await setUserPreference('qy_privacy_level', level);
  }
