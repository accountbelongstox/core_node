...
import 'constants_app_qy.dart';

/// QY app specific SharedPreferences wrapper
/// Provides isolated SharedPreferences instance for QY app
///
...
class PrefsAppQy extends AppPrefsBase {
  SharedPreferences? _instance;
  bool _isInitialized = false;

  // Singleton instance for Provider usage
  static final PrefsAppQy _singleton = PrefsAppQy._internal();

  factory PrefsAppQy() {
    return _singleton;
  }

  PrefsAppQy._internal();

  /// Get SharedPreferences instance for QY app
  SharedPreferences get instance {
    if (_instance == null) {
      throw Exception('PrefsAppQy not initialized. Ensure runCommonApp has completed.');
    }
    return _instance!;
  }
  ...
  ///
  /// USAGE:
  /// - Call this method in main_app_qy.dart after runCommonApp completes
  /// - This ensures the PrefsAppQy instance is fully ready for use
  ...
  SharedPreferences getInstance() {
    if (_instance == null || !_isInitialized) {
      throw Exception('PrefsAppQy not properly initialized. Ensure runCommonApp has completed.');
    }
    return _instance!;
  }

  /// Get value with QY prefix
  @override
  T? get<T>(String key, [T? defaultValue]) {
    if (!_isInitialized) return defaultValue;

    final prefKey = '${QyAppConstants.prefsPrefix}$key';
  ...
  /// Check if key exists with QY prefix
  @override
  ...
  /// Get all QY keys (without prefix)
  @override
  Set<String> getKeys() {
    if (!_isInitialized) return {};

    return _instance!.getKeys()
        .where((key) => key.startsWith(QyAppConstants.prefsPrefix))
        .map((key) => key.replaceFirst(QyAppConstants.prefsPrefix, ''))
        .toSet();
  }

  /// Clear all QY preferences
  @override
  ...
  Map<String, dynamic> getDebugInfo() {
    return {
      ...
      'prefix': QyAppConstants.prefsPrefix,
