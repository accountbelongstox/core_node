...
class StorageAppExample extends AppStorageBase {
  ...
  Future<Map<String, bool>> getNotificationSettings() async {
    final settings = await getApp<Map<String, dynamic>>(keyNotificationSettings);
  ...
  void cacheRecentBookmarks(List<String> bookmarks) {
    setCache('recent_bookmarks', bookmarks, expiry: const Duration(minutes: 30));
  }
  ...
  Future<void> initAppStorage() async {
    ...
    if (bookmarks.isNotEmpty) {
      cacheRecentBookmarks(bookmarks.take(10).toList());
    }
  }
  ...
  Future<void> recordStudySession(Map<String, dynamic> session) async {
    final sessions = await getApp<List<Map<String, dynamic>>>(keyStudySessions) ?? [];
  ...
  Future<bool> importWordLearningData(Map<String, dynamic> data) async {
    try {
      ...
      if (data['learning_stats'] != null) {
        await saveLearningStats(Map<String, dynamic>.from(data['learning_stats']));
      }
