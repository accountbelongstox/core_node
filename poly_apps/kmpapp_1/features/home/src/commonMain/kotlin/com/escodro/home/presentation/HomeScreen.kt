...
private fun AlkaaHomeScaffold(
    ...
    // Check if current destination is a WordFlow page
    val navBackStackEntry by appState.navHostController.currentBackStackEntryAsState()
    val isWordFlowPage = navBackStackEntry?.let { entry ->
        runCatching { entry.toRoute<WordFlowDestination.Login>() }.isSuccess ||
        runCatching { entry.toRoute<WordFlowDestination.Dashboard>() }.isSuccess ||
        runCatching { entry.toRoute<WordFlowDestination.Courses>() }.isSuccess ||
        runCatching { entry.toRoute<WordFlowDestination.CourseDetail>() }.isSuccess
    } ?: false
    ...
    if (isWordFlowPage) {
        // For WordFlow pages, use simple Scaffold without bottom navigation
        Scaffold(
            contentWindowInsets = WindowInsets(0, 0, 0, 0),
            content = { paddingValues ->
                Navigation(
                    startDestination = WordFlowDestination.Login,
        ...
        // For TopLevel pages, use NavigationSuiteScaffold with bottom navigation
