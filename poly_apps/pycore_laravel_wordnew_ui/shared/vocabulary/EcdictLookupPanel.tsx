/**
 * EcdictLookupPanel — shared offline word lookup (ECDICT + WordNet).
 *
 * One panel for both manager UIs (/pycore-manager/vocabulary and
 * /laravel-manager/vocabulary): the hosting app injects an
 * EcdictLookupAdapter (see ecdictLookupTypes.ts) so the same UI queries the
 * shared stardict.db through the pycore-local routes or the laravel routes.
 * Lock races between the two ends surface as busy and are retried via
 * withBusyRetry, then shown with an explicit "retry" hint.
 *
...
const D = {
  ...
  subtitle: 'Free, offline ECDICT (EN↔ZH) + WordNet lookup — one shared database served by both pycore and laravel.',
  ...
  busyRetrying: 'Shared database is busy — retrying…',                  // 共享数据库忙 — 正在重试…
  busyFailed: 'The shared database stayed busy; please retry.',         // 数据库持续繁忙，请重试
...
export interface EcdictLookupPanelProps {
  adapter: EcdictLookupAdapter;
  /** Outer section classes (each app passes its own card surface). */
  className?: string;
}

export default function EcdictLookupPanel({ adapter, className }: EcdictLookupPanelProps) {
  const [status, setStatus] = useState<EcdictStatusInfo | null>(null);
  const [word, setWord] = useState('');
  const [entry, setEntry] = useState<EcdictEntryInfo | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    withBusyRetry(() => adapter.fetchStatus())
  ...
  const lookup = useCallback(async () => {
    ...
    setRetrying(false);
    try {
      const first = await adapter.fetchLookup(w, 'zh');
      if (first.busy) {
        // Shared-database lock race with the other end: the API asked us to
        // retry immediately — show that, then do exactly one deferred retry.
        setRetrying(true);
      }
      const e = first.busy ? await withBusyRetry(() => adapter.fetchLookup(w, 'zh')) : first;
      setRetrying(false);
      if (e.busy) {
        setErr(D.busyFailed);
        return;
  ...
  return (
    <section className={className ?? 'p-6 space-y-4'}>
      {/* header + availability */}
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        {status && (
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
            ...
            {ecdictOn
              ? `ECDICT ${D.installed} · ${entryCount.toLocaleString()} ${D.entries} · ${D.via} ${adapter.sourceLabel}`
              : `ECDICT ${D.notInstalledBadge} · ${D.via} ${adapter.sourceLabel}`}
          </span>
        )}
      </div>
      ...
      {status && !ecdictOn && (
        <div className="flex items-start gap-2 text-xs rounded-xl p-3 border bg-amber-500/10 border-amber-500/30 text-amber-600 dark:text-amber-400">
          ...
          <span className="break-words">{status.error || D.notInstalled}</span>
        </div>
      )}
      ...
      {retrying && (
        <div className="flex items-center gap-2 text-xs text-amber-500">
          <RefreshCw className="w-3.5 h-3.5 animate-spin" /> {D.busyRetrying}
        </div>
      )}
      ...
      {entry && (
        <div className="rounded-2xl p-4 bg-slate-100/60 dark:bg-white/5 border border-slate-300/35 dark:border-white/5 space-y-3">
          ...
          {(entry.targetTranslation || entry.translation) && (
            <div>
          ...
          {(entry.definition || entry.wordnetDefinition) && (
            <div>
