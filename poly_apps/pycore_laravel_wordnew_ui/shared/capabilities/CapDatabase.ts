...
class SqliteBackend implements DbBackend {
  ...
  private table(coll: string): string {
    return STORE_PREFIX + coll.replace(/[^a-zA-Z0-9_]/g, '_');
  }
...
export class CapDatabase {
  ...
  /** Open (or reopen) a named database. Idempotent for the same name. */
  async open(dbName = 'wordnew'): Promise<void> {
    if (this.opened === dbName && this.backend) return;
    if (this.backend) await this.close();
    this.backend = safeIsNative() ? new SqliteBackend() : new IndexedDbBackend();
  ...
  async deleteDatabase(dbName = this.opened || 'wordnew'): Promise<void> {
    const backend = this.backend ?? (safeIsNative() ? new SqliteBackend() : new IndexedDbBackend());
    if (!this.backend) await backend.open(dbName).catch(() => {});
