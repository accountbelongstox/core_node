 ...
 *
 * QUICK START
 *   import { capFs, Directory, useJsonFile } from '@/shared/capabilities/CapFilesystem';
...
export class CapBlobStore {
  ...
  async putFromUrl(key: string, url: string, options: CapBlobPutOptions & { force?: boolean } = {}): Promise<string> {
    ...
    if (safeIsNative()) {
      const dl = (Filesystem as any).downloadFile;
      if (typeof dl === 'function') {
        await dl.call(Filesystem, {
          url,
          path: this.nativePath(key),
          directory: this.directory,
          recursive: true,
          progress: !!options.onProgress,
        });
        return (await capFs.getUri(this.nativePath(key), this.directory)) || '';
      }
      // Fallback: fetch then write (memory-bound).
      const blob = await (await fetch(url)).blob();
      await this.putBlob(key, blob, options);
      return (await capFs.getUri(this.nativePath(key), this.directory)) || '';
    }

    // Web: stream fetch -> OPFS (or IndexedDB if OPFS missing).
    if (opfsSupported()) {
      const res = await fetch(url);
      if (!res.ok || !res.body) throw new Error(`putFromUrl failed: ${res.status}`);
      ...
      options.onProgress?.(1);
      return (await this.getServableUrl(key)) || '';
    }
  ...
  async totalSize(): Promise<number> {
    if (safeIsNative()) return directorySize(this.dir, this.directory);
    const keys = await this.keys();
    let total = 0;
    for (const k of keys) total += await this.size(k);
    return total;
  }
...
export class CapLargeCache {
  ...
  constructor(options: { dir?: string; maxBytes?: number; directory?: Directory } = {}) {
    ...
    this.maxBytes = options.maxBytes ?? 2 * 1024 * 1024 * 1024; // 2 GB default
  }
  ...
  async getOrFetchUrl(key: string, urlFor: () => string | Promise<string>, mime?: string): Promise<string | null> {
    if (!(await this.store.has(key))) {
      ...
      await this.store.putFromUrl(key, url);
  ...
  async enforceBudget(): Promise<void> {
    // Native: use directory stats for mtime-ordered eviction.
    if (safeIsNative()) {
      const entries = await capFs.readdir((this.store as any).dir, Directory.Cache);
      let total = entries.reduce((a, e) => a + (e.size || 0), 0);
      if (total <= this.maxBytes) return;
      for (const e of entries.slice().sort((a, b) => (a.mtime || 0) - (b.mtime || 0))) {
        if (total <= this.maxBytes) break;
        await this.store.delete(e.name);
        total -= e.size || 0;
      }
      return;
    }
    // Web/OPFS: no per-file mtime cheaply; evict in key order until under budget.
    let total = await this.store.totalSize();
    if (total <= this.maxBytes) return;
    for (const key of await this.store.keys()) {
      if (total <= this.maxBytes) break;
      total -= await this.store.size(key);
