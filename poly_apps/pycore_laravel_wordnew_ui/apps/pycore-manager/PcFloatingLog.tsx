 ...
 *
 * Collapsed: a slim glass bar (HTTP status dot + "Logs" + line count + chevron).
 ...
 *            level), auto-scrolling to bottom, with a Clear button and an HTTP
 *            connected/disconnected indicator.
 *
 * Open/closed state persists in localStorage (`pc_log_open`). Consumes the
 * shared usePcLive() buffer; it does not open another event subscription.
 */
...
export const PcFloatingLog: React.FC = () => {
  ...
  useEffect(() => {
    StorageManager.setRaw(StorageKeys.PYCORE_LOG_OPEN, open ? '1' : '0');
  }, [open]);
      ...
      >
        ...
        >
          <span className="relative flex h-2.5 w-2.5 shrink-0">
            ...
            <span
              className={`relative inline-flex rounded-full h-2.5 w-2.5 ${httpConnected ? 'bg-emerald-500' : 'bg-slate-400 dark:bg-slate-600'
                }`}
            />
          </span>
          ...
          <span className={`text-[11px] font-medium inline-flex items-center gap-1 ${httpConnected ? 'text-emerald-500' : 'text-slate-400'}`}>
            ...
            <span className="hidden sm:inline">{httpConnected ? 'Connected' : 'Disconnected'}</span>
          </span>
