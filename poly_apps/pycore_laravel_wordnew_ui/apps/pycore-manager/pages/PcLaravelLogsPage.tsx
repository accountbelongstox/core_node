 ...
 * LaravelLogMirrorService snapshot and refresh routes.
 *
 * Initial open hydrates the snapshot and triggers the only automatic Laravel
 * refresh. Snapshot events re-pull while this page is mounted.
 */
...
const PcLaravelLogsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const offLegacy = pycoreEventBus.subscribe(PYCORE_EVENT_TOPICS.laravelLogsChanged, onLogsChanged);
    const offDurable = pycoreEventBus.subscribe(PYCORE_EVENT_TOPICS.laravelLogsSnapshotUpdated, onLogsChanged);
    return () => {
