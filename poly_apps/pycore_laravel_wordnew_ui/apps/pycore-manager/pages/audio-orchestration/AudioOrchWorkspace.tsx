...
const AudioOrchWorkspace: React.FC<{
    ...
    <div className="space-y-4">
      {onSourceFilterChange && sources.length > 1 && (
        <div className="flex items-center gap-1 border-b border-slate-700/60 overflow-x-auto">
          ...
          {filters.map((filter) => {
            const presentation = filter === 'all' ? null : orchSourcePresentation(filter);
            return (
              <button
                key={filter}
                type="button"
                onClick={() => onSourceFilterChange(filter)}
                className={`inline-flex items-center gap-1 px-3 py-2 text-sm font-medium whitespace-nowrap border-b-2 -mb-px transition-colors ${
                  sourceFilter === filter
                    ? 'border-sky-400 text-sky-300'
                    : 'border-transparent text-slate-400 hover:text-slate-200'
                }`}
              >
                {presentation && <presentation.Icon className="w-3.5 h-3.5" />}
                {presentation ? presentation.label() : ORCH_L.sourceAll}
              </button>
            );
          })}
        </div>
...
export default AudioOrchWorkspace;
