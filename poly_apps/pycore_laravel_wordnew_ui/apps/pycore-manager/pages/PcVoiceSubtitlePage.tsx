 ...
 *
 * Live updates ride the `voice_subtitle_queue_update` HTTP event. Offline the
...
const PcVoiceSubtitlePage: React.FC = () => {
  ...
  const [tab, setTab] = useState<Tab>(() => (StorageManager.getRaw(StorageKeys.PYCORE_VOICE_SUBTITLE_TAB) as Tab) || 'queue');
  const [lang, setLang] = useState<string>(() => StorageManager.getRaw(StorageKeys.PYCORE_VOICE_SUBTITLE_LANGUAGE) || 'en');
  useEffect(() => { StorageManager.setRaw(StorageKeys.PYCORE_VOICE_SUBTITLE_TAB, tab); }, [tab]);
  ...
  useEffect(() => {
    connectPycoreHttp();
    const offQueue = pycoreEventBus.subscribe(PYCORE_EVENT_TOPICS.voiceSubtitleQueueUpdate, (data: any) => {
      applySnapshot(mapQueueSnapshot(data));
      setUnreachable(false);
    });
    const offPlay = pycoreEventBus.subscribe(PYCORE_EVENT_TOPICS.voiceSubtitleUpdate, (data: any) => {
  ...
  const handleLangChange = useCallback(async (next: string) => {
    setLang(next);
    StorageManager.setRaw(StorageKeys.PYCORE_VOICE_SUBTITLE_LANGUAGE, next);
    if (shotOn) {
  ...
  const handleBrowserPlay = useCallback((item: QueueItem) => {
    ...
    // Fetch the clip bytes through the HTTP controller and then play the data URL.
