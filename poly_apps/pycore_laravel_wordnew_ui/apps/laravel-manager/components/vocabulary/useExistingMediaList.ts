...
import { useApiResource } from '@/apps/laravel-manager/hooks/useApiResource';
import type { APIResponse } from '@/apps/laravel-manager/types';
import type { MediaListParams, MediaListResponse } from '@/apps/laravel-manager/api';
import {
  PaginatedTableModel,
...
interface UseExistingMediaListOptions<T> {
  ...
  enabled: boolean;
  fetchPage: (params: MediaListParams) => Promise<APIResponse<MediaListResponse<T>>>;
}
...
export function useExistingMediaList<T>({ perPage, enabled, fetchPage }: UseExistingMediaListOptions<T>) {
  ...
  const [query, setQuery] = useState<Query>({ page: 1, search: '', language: '', sort: null });
  ...
  const setSort = useCallback((key: string) => {
    setQuery((current) => ({
      ...current,
      page: 1,
      sort: PaginatedTableModel.nextSort(current.sort, key),
    }));
  }, []);
  ...
  const resource = useApiResource<MediaListResponse<T>>(
    ...
    { deps: [enabled, query.page, query.search, query.language, query.sort?.key, query.sort?.order, perPage] }
  );
