 ...
 *
 * The caller supplies a `fetchPage(start, limit, sort)` function that returns one
 * backend-sorted page plus the grand total, and a `renderItem`/`columns` description for how
 ...
 * same server-side `fetchPage` contract.
 *
...
export interface PaginatedListColumn<T = any> {
  ...
  className?: string;
  /** Backend sort key. Omit for display/action columns that cannot be ordered. */
  sortKey?: string;
}
...
function PaginatedListModal<T = any>({
  ...
  const [sort, setSort] = useState<PaginatedTableSort | null>(null);
  const requestIdRef = useRef(0);

  const load = useCallback(
    async (nextStart: number, nextSort: PaginatedTableSort | null = sort) => {
      const requestId = ++requestIdRef.current;
      setLoading(true);
      setError(null);
      try {
        const res = await fetchPage(nextStart, limit, nextSort ?? undefined);
  ...
  const changeSort = useCallback((key: string) => {
    const nextSort = PaginatedTableModel.nextSort(sort, key);
    setSort(nextSort);
