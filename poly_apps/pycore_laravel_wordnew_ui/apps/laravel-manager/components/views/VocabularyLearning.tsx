...
const VocabularyLearning: React.FC = () => {
  ...
  const loadWordModal = async (
    language: string,
    page: number,
    perPage: number,
    sort?: { key: DictionaryWordSort; order: 'asc' | 'desc' },
  ): Promise<{ words: VocabularyStatisticsWordRow[]; pagination: VocabularyWordsPagination | null }> => {
    const start = (page - 1) * perPage;
    const response = await api.books.getDictionaryWords({
      language,
      filter: 'all',
      start,
      limit: perPage,
    ...
    if (!response.success || !response.data) return { words: [], pagination: null };
    const total = Number(response.data.total) || 0;
    const lastPage = Math.max(1, Math.ceil(total / perPage));
    const words = response.data.items.map((item, index) => ({
      ...item,
      index: start + index + 1,
      word: item.content,
    }));
  ...
  const drillLanguage = (): string => selectedLanguage;
  ...
  const openTtsQueueDrill = (
    label: string,
    params: { status?: 'pending' | 'processing' | 'completed' | 'failed' | 'leased'; type?: GlobalQueuePositionTaskAlias }
  ) => {
    ...
    const fetchPage: PaginatedListFetcher = async (start, limit) => {
      ...
      if (!r.success || !r.data) throw new Error(r.error || t.queue_drill.load_queue_failed);
      return { items: r.data.items || [], total: r.data.total || 0 };
    };
    logInfo('vocab', `Drill-down: TTS queue "${label}"`);
    setStatDrill({
      title: t.queue_drill.tts_title.replace('{label}', label),
  ...
  const openAssistCategoryDrill = (
    ...
    const fetchPage: PaginatedListFetcher = async (start, limit) => {
      ...
      if (!r.success) throw new Error(r.error || t.queue_drill.load_category_failed);
      return { items: r.items || [], total: r.total || 0 };
    };
    logInfo('vocab', `Drill-down: assist "${label}" (${category}${status ? `/${status}` : ''})`);
    setStatDrill({
      title: t.queue_drill.worker_title.replace('{label}', label),
      subtitle: status === 'leased'
        ? t.queue_drill.active_leases
