 ...
 * contract-owned order / retries / timestamps. The workers half lives in
 * WorkersPanel.
 *
...
type TaskSortMode = 'default' | 'order_desc' | 'order_asc';
...
const renderResultMedia = (result: any, alt: string): React.ReactNode => {
...
const QueuePanel: React.FC<QueuePanelProps> = ({
  ...
  // Live drilldown bundle refreshed by the shared Queue Center Reverb owner.
  ...
  useEffect(() => {
    ...
    setStreamLive(laravelRealtime.isConnected());
  ...
  const handleBump = async (taskId: string, taskType: string) => {
    setBumpingId(taskId);
    setNotice(null);
    try {
      const response = await api.serverManager.bumpTaskToFront(taskId, taskType);
      if (response.success && response.data) {
        const data = response.data;
        if (typeof data.queue_position === 'number') {
          setNotice(`${t.detail.bumped_ok} #${data.queue_position}: ${shortId(taskId)}`);
          moveTaskToFront(taskId, { queue_position: data.queue_position });
        } else {
          setNotice(`${t.detail.bumped_ok} ${data.priority}: ${shortId(taskId)}`);
          moveTaskToFront(taskId, data.priority ?? 0);
        }
        void refreshDetailTask(taskId);
      } else {
  ...
  const rowIsFast = (row: GlobalTaskItem): boolean =>
    !isGlobalTaskQueuePositionOrdered(row.task_type) && (
  ...
  const getFilteredTasks = (): GlobalTaskItem[] => {
    ...
    if (sortMode !== 'default') {
      ...
      rows = [...rows].sort((a, b) =>
        sortMode === 'order_desc'
          ? compareGlobalTasksByContract(a, b)
          : compareGlobalTasksByContract(b, a)
      );
    }
    return rows;
  };
      ...
      {loading && !snapshot ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              ...
              >
                ...
                <option value="order_desc">{t.sort_order_desc}</option>
                <option value="order_asc">{t.sort_order_asc}</option>
              </select>
            </div>
          ...
          {selectedTask && (() => {
            ...
            const taskType = taskData?.task_type ?? selectedTask.task_type;
            const queuePositionOrdered = isGlobalTaskQueuePositionOrdered(taskType);
            const orderValue = queuePositionOrdered
              ? taskData?.queue_position ?? selectedTask.queue_position
              : taskData?.priority ?? selectedTask.priority;
            const orderLabel = queuePositionOrdered
              ? t.detail.queue_position
              : t.detail.priority;
            const capability = taskData?.capability ?? null;
            const executionType = taskData?.execution_type ?? selectedTask.execution_type;
            const isFastTier = !queuePositionOrdered
              && (taskData?.is_fast_tier ?? rowIsFast(selectedTask));
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-4 gap-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {canBump && (
                            <button
                              onClick={() => handleBump(selectedTask.task_id, selectedTask.task_type)}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          {orderValue !== undefined && (
                            <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3">
                              <div className="text-xs text-slate-500 dark:text-slate-400">{orderLabel}</div>
                              <div className="text-sm font-bold">{orderValue}</div>
                            </div>
                          )}
                          <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3">
                        ...
                        <div>
                          ...
                          {renderResultMedia(taskData?.result, t.detail.result)}
