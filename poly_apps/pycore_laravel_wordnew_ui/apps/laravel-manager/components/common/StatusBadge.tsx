...
}> = ({ status, tone, withDot = true, className = '', appearanceClassName, children }) => {
  const t = tone || statusTone(status);
  return (
    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium ${appearanceClassName ?? TONE[t]} ${className}`}>
