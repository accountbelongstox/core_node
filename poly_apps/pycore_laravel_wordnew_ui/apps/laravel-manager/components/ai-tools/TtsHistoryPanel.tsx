 ...
 * public GET /ai_tools/tts/queue/stats: by-status distribution plus recent
 * outcomes (content, language, status, retries, error, provider/engine).
 */
...
const TtsHistoryPanel: React.FC = () => {
  ...
  const labels = TRANSLATIONS[lang].vocabulary.queue_drill;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setStats(data);
      setError(res?.success === false ? (res?.message || labels.load_stats_failed) : null);
    } catch (e: any) {
      setError(e?.message || labels.laravel_unreachable);
    } finally {
      setLoading(false);
    }
  }, [labels.laravel_unreachable, labels.load_stats_failed]);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold flex items-center gap-2 text-slate-800 dark:text-slate-100">
            <Volume2 className="w-5 h-5 text-emerald-400" /> {labels.history_title}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            {labels.history_subtitle}
          </p>
        </div>
        <button
          onClick={load}
          ...
          <RefreshCcw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} /> {labels.refresh}
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <StatusChip label={labels.pending} value={byStatus.pending ?? 0} tone="bg-amber-500/10 text-amber-500" />
        <StatusChip label={labels.processing} value={byStatus.processing ?? 0} tone="bg-sky-500/10 text-sky-500" />
        <StatusChip label={labels.completed} value={byStatus.completed ?? 0} tone="bg-emerald-500/10 text-emerald-500" />
        <StatusChip label={labels.failed} value={byStatus.failed ?? 0} tone="bg-rose-500/10 text-rose-500" />
        <StatusChip label={labels.total} value={stats?.total ?? 0} tone="bg-slate-500/10 text-slate-500 dark:text-slate-300" />
      </div>

      <div>
        <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-2">{labels.recent_attempts}</h3>
        <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 overflow-hidden bg-white/40 dark:bg-white/5">
          {logs.length === 0 && !loading ? (
            <p className="text-xs text-slate-400 text-center py-10">{labels.no_records}</p>
          ) : (
            logs.map((log) => {
