/**
 * WordNewBookReaderDailyCounter — the book reader's "read today" counter.
...
function load(): DailyShape {
  ...
  try {
    const parsed = StorageManager.get<DailyShape | null>(StorageKeys.WORDNEW_READER_DAILY, null);
...
function save(shape: DailyShape): void {
  try {
    StorageManager.set(StorageKeys.WORDNEW_READER_DAILY, shape);
  } catch {
