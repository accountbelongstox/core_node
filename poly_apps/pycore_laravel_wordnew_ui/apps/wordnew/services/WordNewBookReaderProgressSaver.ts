...
export interface WordNewBookReaderProgressPayload {
...
interface WordNewBookReaderProgressSaverOpts {
  sourceKey: string;
...
export class WordNewBookReaderProgressSaver {
  private debounceTimer: ReturnType<typeof setTimeout> | null = null;
  private retryTimer: ReturnType<typeof setTimeout> | null = null;
  private pendingPayload: WordNewBookReaderProgressPayload | null = null;
  private pendingRef = '';
  private saveInFlight = false;
  private timeoutNotified = false;

  constructor(private readonly opts: WordNewBookReaderProgressSaverOpts) {}
  ...
  schedule(verse: WfNewBookVerse, pageNum: number): void {
    const payload: WordNewBookReaderProgressPayload = {
    ...
    void wordNewReadingProgressCenter.set(this.opts.sourceKey, {
      index: verse.seq,
      total: 0,
    });
  ...
  private async saveWithTimeout(payload: WordNewBookReaderProgressPayload): Promise<boolean> {
    try {
