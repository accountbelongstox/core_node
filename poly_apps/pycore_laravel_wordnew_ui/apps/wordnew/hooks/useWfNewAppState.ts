...
export function useWfNewAppState(deps: { shellLang: string; dark: boolean }) {
  ...
  useEffect(() => {
    ...
    void wfNewAdminApi.probeStatus().then((status) => {
      ...
      if (status.enabled) {
        ...
        try {
          if (!StorageManager.hasSession(StorageKeys.WORDNEW_SUPER_TOAST)) {
            StorageManager.setSession(StorageKeys.WORDNEW_SUPER_TOAST, true);
   ...
   *   - cached media: the WordNewAudioCenter URL cache + the browser's HTTP cache of
  ...
  const handleLogout = () => {
    ...
    wordNewReaderSettingsRoamer.flush();
  ...
  useEffect(() => {
    void wordNewReaderSettingsRoamer.pull();
  }, []);
