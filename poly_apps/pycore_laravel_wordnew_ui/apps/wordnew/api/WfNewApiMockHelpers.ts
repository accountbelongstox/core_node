...
export const MOCK_AUTH_USERS_KEY = StorageKeys.WORDNEW_MOCK_AUTH_USERS;
...
export const SEED_AUTH_USERS: MockAuthRecord[] = [
  {
    ...
    nickname: 'Demo Cadet',
    name: 'Demo Cadet',
    email: 'demo@wordnew.test',
    avatar: '',
    native_language: 'zh',
    learning_languages: ['en'],
    member_type: 'free',
    bio: 'Offline mock cadet — select the live API mode to connect.',
    password: 'demo123',
  },
];
...
export function readAuthUsers(): MockAuthRecord[] {
  try {
    const parsed = StorageManager.get<MockAuthRecord[]>(MOCK_AUTH_USERS_KEY, []);
...
export function writeAuthUsers(users: MockAuthRecord[]): void {
  StorageManager.set(MOCK_AUTH_USERS_KEY, users);
}
...
export const MOCK_PREFS_KEY = StorageKeys.WORDNEW_MOCK_PREFERENCES;
...
export function readMockPreferences(): WfNewPreferences {
  try {
    const parsed = StorageManager.get<WfNewPreferences | null>(MOCK_PREFS_KEY, null);
...
export function writeMockPreferences(prefs: WfNewPreferences): void {
  StorageManager.set(MOCK_PREFS_KEY, prefs);
}

// --- mock guest device settings (fingerprint client_key) -------------------- #

const MOCK_DEVICE_SETTINGS_KEY = StorageKeys.WORDNEW_MOCK_DEVICE_SETTINGS;

export function readMockDeviceSettings(clientKey: string): WfNewClientDeviceSettings | null {
  try {
    const map = StorageManager.get<Record<string, WfNewClientDeviceSettings>>(MOCK_DEVICE_SETTINGS_KEY, {});
...
export function writeMockDeviceSettings(
  ...
  try {
    map = StorageManager.get(MOCK_DEVICE_SETTINGS_KEY, {});
  } catch {
    map = {};
  }
  const entry: WfNewClientDeviceSettings = { clientKey, reader, updatedAt };
  map[clientKey] = entry;
  StorageManager.set(MOCK_DEVICE_SETTINGS_KEY, map);
  return entry;
}

// --- mock learning-language selection --------------------------------------- #

export const MOCK_LANGS_KEY = StorageKeys.WORDNEW_MOCK_LANGUAGES;

export function readMockLanguages(): WfNewLanguageSelection {
  try {
    const parsed = StorageManager.get<WfNewLanguageSelection | null>(MOCK_LANGS_KEY, null);
...
export function writeMockLanguages(selection: WfNewLanguageSelection): void {
  StorageManager.set(MOCK_LANGS_KEY, selection);
}

// --- mock social datasets --------------------------------------------------- #

export const MOCK_FRIENDS_KEY = StorageKeys.WORDNEW_MOCK_FRIENDS;
...
export function readMockFriends(): WfNewFriend[] {
  try {
    const parsed = StorageManager.get<WfNewFriend[]>(MOCK_FRIENDS_KEY, []);
...
export function writeMockFriends(friends: WfNewFriend[]): void {
  StorageManager.set(MOCK_FRIENDS_KEY, friends);
}
...
export const MOCK_CONVOS_KEY = StorageKeys.WORDNEW_MOCK_CONVERSATIONS;
export const MOCK_MESSAGES_KEY = StorageKeys.WORDNEW_MOCK_MESSAGES;
export const MOCK_REQUESTS_KEY = StorageKeys.WORDNEW_MOCK_REQUESTS;
export const MOCK_NOTIFS_KEY = StorageKeys.WORDNEW_MOCK_NOTIFICATIONS;
...
export function readJson<T>(key: StorageKey, seed: T): T {
  return StorageManager.get(key, seed);
}
export function writeJson(key: StorageKey, val: any): void {
...
export const MOCK_POSTS_KEY = StorageKeys.WORDNEW_MOCK_POSTS;
export const MOCK_COMMENTS_KEY = StorageKeys.WORDNEW_MOCK_COMMENTS;
export const MOCK_LIVE_KEY = StorageKeys.WORDNEW_MOCK_LIVE;
export const MOCK_LIVE_CHAT_KEY = StorageKeys.WORDNEW_MOCK_LIVE_CHAT;
