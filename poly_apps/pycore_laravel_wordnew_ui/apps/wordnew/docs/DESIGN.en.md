...
| Auto-play recitation loop (`playControlNewCase`) | `useWfNewReciteController` (play/pause, per-word N-plays, interval, replay-gap, auto-scroll) | `WordNewAudioCenter.playWord` / Web-Speech + `playPhoneticSpeech` prop; settings from `wfNewSettings` (`wmPlayCount`/`wmReplayCount`/`wmPlayInterval`/`wmReplaySpeed`/`wmReplayGapWords`) |
| Mark-read `submit_haveread` / set-review | `WfNewStudyWordList` + `WfNewFlashcard` mark buttons → `wfNewStudyProgress.mark()` | best-effort `wordNewProgressCenter.reportAnswers(gid, [{word_id, correct}])` (`POST /group/update_progress`) + `wordNewRecitationCenter.recordAction` (`POST /recitation/log`); local persist otherwise |
| Three-bar progress rail + read/unread counts | `WfNewStudyStatsBar` | `wordNewProgressCenter.getBlob(gid)` + `computeStats` / `dueWords` (`POST /group/get_progress_blob`); falls back to `Word.masteryLevel` + local store |
| Review-due buckets (notifications) | **Review** mode tab in `WfNewGroupStudyPanel` | `wordNewProgressCenter.dueWords(blob)`; local `needsReview` set fallback |
| Least-recently-read ordering (`group_map` sort) | Recite/Review ordering in the controller | `wordNewProgressCenter` entry `last_read_at` / local `lastStudiedAt`; unstudied first |
...
| Pronounce buttons + `<audio>` | per-word speaker button | `Word.audioUrl` (direct `Audio`) → `playPhoneticSpeech` / `WordNewAudioCenter` fallback |
...
| daily_readed daily goal | daily-goal bar in the stats header | `wfNewSettings.dailyGoal` + `wordNewRecitationCenter.getToday()` (best-effort) |
...
- Default playback is list/sequential (R1). As the recite controller finishes a
  ...
  `wordNewProgressCenter`/`wfNewApi.updateGroupProgress({ gid, word_id, action: 'read', play_time: <seconds> })`.
...
**Frontend (`poly_apps/pycore_laravel_wordnew_ui/apps/wordnew/`)**
...
- R4 play-time: `useWfNewReciteController.ts:176-195` computes
  ...
  `WfNewStudyProgress.recordSeen` → `WordNewProgressCenter.reportReadWithPlayTime` →
  `wfNewApi.updateGroupProgress({action:'read', play_time})` (fire-and-forget).
- API layer: `getGroupWordsPage` `unread_only`/`limit` in sync across
  ...
  the active `wfNewApi` surface (with `play_time` support) —
