/* WordNew i18n - functionally modeled on apps/wordnew/WordNewLanguageCenter.ts
...
// --- Language mechanism (mirrors WordNewLanguageCenter; en/zh/ja/ko only) ---------
...
export const WORDNEW_NEW_LANGUAGE_CONFIGS: Record<WfNewSupportedLanguage, WfNewLanguageConfig> = {
...
export function isSupported(lang: string): boolean {
  return lang in WORDNEW_NEW_LANGUAGE_CONFIGS;
}

export function getSupportedLanguages(): WfNewLanguageConfig[] {
  return Object.values(WORDNEW_NEW_LANGUAGE_CONFIGS);
}

export function getLanguageConfig(lang: string): WfNewLanguageConfig {
  return WORDNEW_NEW_LANGUAGE_CONFIGS[(isSupported(lang) ? lang : 'en') as WfNewSupportedLanguage];
}

/** Pure lookup: active-language value -> English fallback -> the raw key.
 * Supports {name} placeholder interpolation (same contract as WordNewLanguageCenter). */
