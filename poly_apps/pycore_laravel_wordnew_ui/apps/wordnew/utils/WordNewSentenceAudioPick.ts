...
import type { WordNewAudioFileVariant, WfNewBookVerseLang } from '../api/types/media';
import { mapUiAccent } from '../hooks/wordNewWordAudioFallback';

export interface WordNewSentenceAudioPick {
  url: string | null;
  variantKey: string;
  accent: string | null;
  isFallback: boolean;
...
export function sentenceVariantLabel(v: WordNewAudioFileVariant, trans?: (k: string) => string): string {
...
export function readySentenceVariants(cell: WfNewBookVerseLang | undefined): WordNewAudioFileVariant[] {
