...
const KEYFRAMES = `
@keyframes wordNewCoverSlideLeft { from { transform: translateX(100%); } to { transform: translateX(0); } }
@keyframes wordNewCoverSlideRight { from { transform: translateX(-100%); } to { transform: translateX(0); } }
@keyframes wordNewCoverSlideUp { from { transform: translateY(100%); } to { transform: translateY(0); } }
@keyframes wordNewCoverSlideDown { from { transform: translateY(-100%); } to { transform: translateY(0); } }
@keyframes wordNewCoverZoomReveal {
  from { transform: scale(0.35); border-radius: 50%; opacity: 0.4; }
  to { transform: scale(1); border-radius: 0; opacity: 1; }
}
.wf-cover-enter-slide-left { animation: wordNewCoverSlideLeft 0.7s ease both; }
.wf-cover-enter-slide-right { animation: wordNewCoverSlideRight 0.7s ease both; }
.wf-cover-enter-slide-up { animation: wordNewCoverSlideUp 0.7s ease both; }
.wf-cover-enter-slide-down { animation: wordNewCoverSlideDown 0.7s ease both; }
.wf-cover-enter-zoom-reveal { animation: wordNewCoverZoomReveal 0.8s ease both; }
`;
