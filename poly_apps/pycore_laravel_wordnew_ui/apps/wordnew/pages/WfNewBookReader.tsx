...
import {
  ...
  visibleBookLangs,
} from '../utils/WordNewBookReaderLangUtils';
import { langCodeToBcp47, orderLangsForReadAloud } from '../utils/WordNewBookReaderA11y';
...
export const WfNewBookReader: React.FC<WfNewBookReaderProps> = ({
  ...
  useEffect(() => () => { wordNewReaderSettingsRoamer.flush(); }, []);
  ...
  useEffect(() => {
    progressSaverRef.current = new WordNewBookReaderProgressSaver({
      sourceKey,
      trans,
  ...
  useEffect(() => {
    ...
    const loadProgress = async (): Promise<WfNewBookReadingProgress | null> => {
      ...
      const local = await wordNewReadingProgressCenter.get(sourceKey);
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      {showSettings && (
        <WordNewBookReaderSettingsPanel
