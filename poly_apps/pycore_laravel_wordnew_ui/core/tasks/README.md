...
| `wordnew.article-processor`  | `WordNewToolsArticleProcessorPage` (submit→poll task)      |
...
### wordnew adopter — why one, and why StorageCenter still covers the rest

wordnew (Capacitor/native) keeps settings/auth/preferences in `StorageCenter`
...
exists is **WordNewToolsArticleProcessorPage** (`wordnew.article-processor`): submit
...
settle the loop while keeping the result visible. All other wordnew pages are
