...
const loadStateFromStorage = (): UnifiedAppState => {
  try {
    ...
    const savedUser = StorageManager.get<UnifiedUser | null>(StorageKeys.UnifiedUser, null);
...
const saveStateToStorage = (state: UnifiedAppState): void => {
  try {
    ...
    if (state.UnifiedUser) {
      StorageManager.set(StorageKeys.UnifiedUser, state.UnifiedUser);
    } else {
      StorageManager.remove(StorageKeys.UnifiedUser);
    }
