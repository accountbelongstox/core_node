...
class ApiManager {
  ...
  setRecheckIntervalMs(ms: number): void {
    ...
    StorageManager.setRaw(StorageKeys.LARAVEL_API_RECHECK_INTERVAL_MS, String(clamped));
  }
  ...
  private setAutoDetectedEndpoint(endpointId: string): void {
    StorageManager.setRaw(StorageKeys.LARAVEL_API_AUTO_DETECTED_ENDPOINT, endpointId);
    this.setStoredCurrentEndpoint(endpointId);
  }
  ...
  private setUserModifiedEndpoint(endpointId: string): void {
    StorageManager.setRaw(StorageKeys.LARAVEL_API_USER_MODIFIED_ENDPOINT, endpointId);
    this.setStoredCurrentEndpoint(endpointId);
  }
  ...
  private setStoredCurrentEndpoint(endpointId: string): void {
    StorageManager.setRaw(StorageKeys.LARAVEL_API_CURRENT_ENDPOINT, endpointId);
  }
  ...
  private migrateResolvedEndpointId(sourceId: string, resolvedId: string): void {
    ...
    if (this.getUserModifiedEndpoint() === sourceId) {
      StorageManager.setRaw(StorageKeys.LARAVEL_API_USER_MODIFIED_ENDPOINT, resolvedId);
    }
    if (this.getAutoDetectedEndpoint() === sourceId) {
      StorageManager.setRaw(StorageKeys.LARAVEL_API_AUTO_DETECTED_ENDPOINT, resolvedId);
    }
  ...
  clearUserModifiedEndpoint(): void {
    StorageManager.remove(StorageKeys.LARAVEL_API_USER_MODIFIED_ENDPOINT);
  }
   ...
   */
  reset(): void {
    StorageManager.remove(StorageKeys.LARAVEL_API_AUTO_DETECTED_ENDPOINT);
    StorageManager.remove(StorageKeys.LARAVEL_API_USER_MODIFIED_ENDPOINT);
    StorageManager.remove(StorageKeys.LARAVEL_API_CURRENT_ENDPOINT);
