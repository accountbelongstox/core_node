...
import type { BackendApiEndpoint } from '../../../config/api-endpoints';
import {
  addCustomEndpoint,
  buildApiUrl,
  getAllEndpoints,
...
import { QUEUE_CENTER_CONTRACT, QUEUE_CENTER_DIFF_DELIVERY } from '../../contracts/QueueCenterContract';
import type {
...
const ROUTES = {
  ...
  queueCenterOverview: '/api/queue-center/overview',
  // Queue Center pump read/claim surface (diff delivery over global_tasks).
  queueCenterIdPages: (queue: string): string =>
    `/api/queue-center/queues/${encodeURIComponent(queue)}/id-pages`,
  queueCenterPageData: (queue: string): string =>
    `/api/queue-center/queues/${encodeURIComponent(queue)}/page-data`,
  workerTaskAccept: (taskType: string): string =>
    `/api/worker/tasks/${encodeURIComponent(taskType)}/accept`,
  workerRegister: '/api/worker/register',
  ...
  wordMedia: (language: string, word: string): string =>
    `/api/app_qy_v1/word/${encodeURIComponent(language)}/${encodeURIComponent(word)}/media`,
  wordBoost: '/api/app_qy_v1/word/boost-priority',
  wordBoostBatch: '/api/app_qy_v1/word/boost-priority/batch',
...
const laravelMethods = {
  ...
  stackQueue: (words: string[], language: string, targetLanguage: string, priority?: number) =>
    requestLaravel<any>('POST', ROUTES.translationQueueStack, {
      words,
      language,
  ...
  getWordAudioMediaDataUrl: async (word: string, language = 'en'): Promise<string> => {
    const payload = await requestLaravel<any>('GET', ROUTES.wordMedia(language, word));
    const data = unwrapData<any>(payload);
    return data.audio_url ? resourceDataUrl(data.audio_url) : '';
  },
  boostWordAudioPriority: (md5: string, lang: string) =>
    requestLaravel<any>('POST', ROUTES.wordBoost, { md5, lang }),
  boostWordAudioPriorities: (items: Array<{ md5: string; lang: string }>) =>
    requestLaravel<any>('POST', ROUTES.wordBoostBatch, { items }),
