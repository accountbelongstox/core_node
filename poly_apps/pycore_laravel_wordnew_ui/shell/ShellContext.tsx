...
export const ShellProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [dark, setDarkState] = useState<boolean>(() => {
    const stored = StorageManager.getRaw(StorageKeys.SHELL_DARK);
  ...
  const [lang, setLangState] = useState<string>(() => {
    const stored = StorageManager.getRaw(StorageKeys.SHELL_LANGUAGE);
    return stored ? stored : 'en';
  });

  const [themeOverride, setThemeOverrideState] = useState<ThemeId | null>(() => {
    const stored = StorageManager.getRaw(StorageKeys.SHELL_THEME_OVERRIDE);
  ...
  // Per-app live-service gate: scope the pycore HTTP event transport to the
  ...
  const setDark = useCallback((v: boolean) => {
    setDarkState(v);
    StorageManager.setRaw(StorageKeys.SHELL_DARK, v ? '1' : '0');
  }, []);
  const toggleDark = useCallback(() => setDark(!dark), [dark, setDark]);

  const setLang = useCallback((code: string) => {
    setLangState(code);
    StorageManager.setRaw(StorageKeys.SHELL_LANGUAGE, code);
  }, []);

  const setThemeOverride = useCallback((t: ThemeId | null) => {
    setThemeOverrideState(t);
    if (t) StorageManager.setRaw(StorageKeys.SHELL_THEME_OVERRIDE, t);
    else StorageManager.remove(StorageKeys.SHELL_THEME_OVERRIDE);
  }, []);
