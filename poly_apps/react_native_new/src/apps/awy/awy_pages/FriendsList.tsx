    ...
    <MobileLayout showNav={true}>
      ...
      <View style={[localStyles.filterContainer, { maxHeight: showFilter ? 200 : 0, paddingBottom: showFilter ? 16 : 0 }]}>
        <GlassCard style={localStyles.filterCard}>
          <Text style={[localStyles.filterTitle, { color: colors.textSecondary }]}>
            {t('friend.filterByStatus')}
          </Text>
          <View style={localStyles.filterButtons}>
            {[
              { key: 'all', label: t('friend.filterAll') },
              { key: 'online', label: t('friend.filterOnline') },
              { key: 'monitored', label: t('friend.filterMonitored') },
              { key: 'alerts', label: t('friend.filterAlerts') }
            ].map(f => (
              <TouchableOpacity 
                key={f.key} 
                style={[
                  localStyles.filterButton,
                  f.key === 'all' && localStyles.filterButtonActive,
                  { backgroundColor: f.key === 'all' ? colors.primary : colors.bg }
                ]}
              >
                <Text style={[
                  localStyles.filterButtonText,
                  { color: f.key === 'all' ? 'white' : colors.textSecondary }
                ]}>
                  {f.label}
                </Text>
              </TouchableOpacity>
                  ...
                  >
                    <View style={localStyles.chatPreviewLeft}>
                      ...
                      <Text style={[localStyles.chatPreviewText, { color: colors.textSecondary }]} numberOfLines={1}>
                        {friend.chat?.lastMessage || t('friend.noRecentMessages')}
                      </Text>
                    </View>
