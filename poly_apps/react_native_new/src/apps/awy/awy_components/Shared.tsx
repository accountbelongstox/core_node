...
import { View, Text, TouchableOpacity, TextInput, StyleSheet, ScrollView, Platform, GestureResponderEvent } from 'react-native';
...
export const BottomNav: React.FC = () => {
  ...
  const handleNavPress = (routeName: string, event?: GestureResponderEvent) => {
  ...
  const handleCenterPress = (event?: GestureResponderEvent) => {
    event?.stopPropagation();
    // Use requestAnimationFrame to ensure navigation happens after any pending renders
    requestAnimationFrame(() => {
  ...
  const NavItem = ({ routeName, iconName, label }: { routeName: string, iconName: string, label: string }) => {
    ...
    return (
      <TouchableOpacity 
        onPress={(event) => handleNavPress(routeName, event)}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <TouchableOpacity 
            onPress={(event) => handleCenterPress(event)}
            activeOpacity={0.8}
            style={{ 
              overflow: 'hidden', 
              borderRadius: 28,
