...
const WfMediaDetailPage: React.FC = () => {
  ...
  const toggleSentenceAudio = (sentence: WfMediaSentence) => {
    ...
    const audio = new Audio(resolveSentenceAudioUrl(sentence.audio));
    audioRef.current = audio;
    setPlayingSeq(sentence.seq);
    audio.onended = () => {
      audioRef.current = null;
      setPlayingSeq(null);
    };
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            {sentence.audio && (
                              <button
                                ...
                                className={`inline-flex items-center justify-center w-8 h-8 rounded-full flex-shrink-0 transition-all ${
                                  isPlaying
                                    ? 'bg-[var(--klein-blue)] text-[var(--klein-on)] shadow-[var(--klein-glow)]'
                                    : 'bg-[var(--klein-blue-soft)] text-[var(--klein-blue)] hover:opacity-80'
                                }`}
                                title={t('media.hasAudio') || 'Audio'}
