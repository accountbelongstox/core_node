...
| `GET /app_qy_v1/word-groups/{gid}/analysis` | `api.analyzeCourse(groupId)` (legacy `services/api.ts`) | `pages/Library/CourseDetail.tsx` | ✅ Implemented (`AppQyV1WordGroupProgressController::getCourseAnalysis`, auth `custom.authenticate`). Returns `CourseAnalysis` `{ groupId, totalWords, knownWords, newWords, estimatedDays, similarity }`; `knownWords`/`similarity` = global memory overlap (proficiency ≥ 60 across all progress). Previously 404, masked by `api.ts` mock fallback. |
...
| `GET /words/{id}` | `ApiCenter.words.getDetail()` | `pages/Library/WordDetail.tsx` | ✅ Implemented (`getWordDetails`, returns canonical `Word` shape) |
| `POST /words/{id}/learn` | `ApiCenter.learning.markWordAsLearned()` | `pages/Library/WordDetail.tsx` | ❌ NOT IMPLEMENTED / deferred (`markAsLearned` missing) |
| `POST /words/{id}/review` | `ApiCenter.learning.markWordAsReviewed()` | Learning progress tracker | ❌ NOT IMPLEMENTED / deferred (`markAsReviewed` missing) |
| `POST /words/{id}/favorite` | `ApiCenter.learning.toggleWordFavorite()` | `pages/Library/WordDetail.tsx` | ❌ NOT IMPLEMENTED / deferred (`toggleFavorite` missing) |
| `GET /words/search/{query}` | `ApiCenter.words.search()` | `pages/Search/Dictionary.tsx` | ✅ Implemented (`searchWords`, returns `Word[]`, `audioUrl: null`) |
| `GET /words/public/{word}` | `ApiCenter.words.publicLookup()` | Available for public access | ✅ Implemented (`publicWordLookup`, no auth, `{ word, found, data? }`) |
...
### Words (read endpoints implemented; 3 state endpoints deferred)
16. ✅ `GET /words/daily` → `pages/Dashboard/Home.tsx`
17. ✅ `GET /words/{id}` → `getWordDetails` (canonical `Word` shape)
18. ❌ `POST /words/{id}/learn` → NOT IMPLEMENTED / deferred (`markAsLearned` missing)
19. ❌ `POST /words/{id}/review` → NOT IMPLEMENTED / deferred (`markAsReviewed` missing)
20. ❌ `POST /words/{id}/favorite` → NOT IMPLEMENTED / deferred (`toggleFavorite` missing)
21. ✅ `GET /words/search/{query}` → `searchWords` (`Word[]`, `audioUrl: null`)
...
| 23 | `/words/{id}` | GET | `ApiCenter.words.getDetail()` | `pages/Library/WordDetail.tsx:29` | ✅ IMPLEMENTED (`getWordDetails`, canonical `Word` shape) |
| 24 | `/words/{id}/learn` | POST | `ApiCenter.learning.markWordAsLearned()` | `pages/Library/WordDetail.tsx:71` | ❌ NOT IMPLEMENTED / deferred (`markAsLearned` missing) |
| 25 | `/words/{id}/review` | POST | `ApiCenter.learning.markWordAsReviewed()` | `services/LearningProgressTracker.ts` | ❌ NOT IMPLEMENTED / deferred (`markAsReviewed` missing) |
| 26 | `/words/{id}/favorite` | POST | `ApiCenter.learning.toggleWordFavorite()` | `pages/Library/WordDetail.tsx:53` | ❌ NOT IMPLEMENTED / deferred (`toggleFavorite` missing) |
| 27 | `/words/search/{query}` | GET | `ApiCenter.words.search()` | `pages/Search/Dictionary.tsx:35` | ✅ IMPLEMENTED (`searchWords`, `Word[]`, `audioUrl: null`) |
| 28 | `/words/public/{word}` | GET | `ApiCenter.words.publicLookup()` | `services/ApiCenter.ts:515` | ✅ IMPLEMENTED (`publicWordLookup`, no auth) |
...
16. ✅ `GET /words/{id}`
    - Backend: `getWordDetails` — returns canonical `Word` shape; `auth:sanctum`, optional `?language=`
    - API: `ApiCenter.words.getDetail` (line 409-413)
    - UI: `pages/Library/WordDetail.tsx` (line 29)

17. ❌ `POST /words/{id}/learn` — NOT IMPLEMENTED / deferred
    - Backend: route maps to missing `markAsLearned` (only `upLearned` exists; word_id space mismatch) — will 500 if called

18. ❌ `POST /words/{id}/review` — NOT IMPLEMENTED / deferred
    - Backend: route maps to missing `markAsReviewed` (same word_id space mismatch) — will 500 if called

19. ❌ `POST /words/{id}/favorite` — NOT IMPLEMENTED / deferred
    - Backend: route maps to missing `toggleFavorite` (no favorite storage yet) — will 500 if called

20. ✅ `GET /words/search/{query}`
    - Backend: `searchWords` — returns `Word[]` (`audioUrl: null`); `auth:sanctum`, optional `?language=`
...
### 3.10 GET `/api/app_qy_v1/word-groups/{gid}/analysis`
- **Backend**: ✅ Implemented (`AppQyV1WordGroupProgressController::getCourseAnalysis`) — requires auth (`custom.authenticate`)
- **Frontend**: ✅ Integrated (legacy `services/api.ts` `analyzeCourse(groupId)`)
- **UI**: ✅ Available (`pages/Library/CourseDetail.tsx`)
- **Response**: `CourseAnalysis` `{ groupId, totalWords, knownWords, newWords, estimatedDays, similarity }`; `knownWords`/`similarity` are a **global memory overlap** — how many of the group's words the user already knows (proficiency ≥ 60 across all their progress)
- **Status**: ✅ **WORKING** — previously **MISSING (404)** but the failure was hidden because `api.ts` falls back to hardcoded mock data on any error, so the page showed fake numbers
...
- **Backend**: ✅ Implemented (`AppQyV1WordQueryController::getWordDetails`). Requires `auth:sanctum`; optional `?language=` (default `en`). Returns the canonical `Word` shape `{ id, text, phonetic, translation, definition, example, masteryLevel, tags, audioUrl }`.
- **Frontend**: ❌ **NOT INTEGRATED**
- **UI**: ❌ **MISSING**
- **Status**: ❌ **NOT USED**

### 9.3 POST `/api/words/{id}/learn`
- **Backend**: ❌ **NOT IMPLEMENTED / deferred.** Route exists but maps to missing `markAsLearned` (`AppQyV1WordLearningStatusController` only has `upLearned`; word_id space mismatch unresolved). Will 500 if called. Deferred by product decision — no data model yet.
- **Frontend**: ❌ **NOT INTEGRATED**
- **UI**: ❌ **MISSING**
- **Status**: ❌ **NOT USED**

### 9.4 POST `/api/words/{id}/review`
- **Backend**: ❌ **NOT IMPLEMENTED / deferred.** Route exists but maps to missing `markAsReviewed` (same word_id space mismatch). Will 500 if called. Deferred by product decision — no data model yet.
- **Frontend**: ❌ **NOT INTEGRATED**
- **UI**: ❌ **MISSING**
- **Status**: ❌ **NOT USED**

### 9.5 POST `/api/words/{id}/favorite`
- **Backend**: ❌ **NOT IMPLEMENTED / deferred.** Route exists but maps to missing `toggleFavorite` (no favorite storage exists yet). Will 500 if called. Deferred by product decision — no data model yet.
- **Frontend**: ❌ **NOT INTEGRATED**
- **UI**: ❌ **MISSING**
- **Status**: ❌ **NOT USED**

### 9.6 GET `/api/words/search/{query}`
- **Backend**: ✅ Implemented (`AppQyV1WordQueryController::searchWords`). Requires `auth:sanctum`; optional `?language=`. Returns `Word[]`; list results have `audioUrl: null`.
- **Frontend**: ❌ **NOT INTEGRATED**
- **UI**: ❌ **MISSING**
- **Status**: ❌ **NOT USED**

### 9.7 GET `/api/words/public/{word}`
- **Backend**: ✅ Implemented (`AppQyV1WordQueryController::publicWordLookup`). **No auth.** Returns `{ word, found: boolean, data?: Word }` with HTTP 200 even when not found.
- **Frontend**: ❌ **NOT INTEGRATED**
- **UI**: ❌ **MISSING**
- **Status**: ❌ **NOT USED**
...
| GET | `/words/{id}` | `ApiCenter.words.getDetail()` | ✅ Implemented (`getWordDetails`) |
| POST | `/words/{id}/learn` | `ApiCenter.learning.markWordAsLearned()` | ❌ NOT IMPLEMENTED / deferred |
| POST | `/words/{id}/review` | `ApiCenter.learning.markWordAsReviewed()` | ❌ NOT IMPLEMENTED / deferred |
| POST | `/words/{id}/favorite` | `ApiCenter.learning.toggleWordFavorite()` | ❌ NOT IMPLEMENTED / deferred |
| GET | `/words/search/{query}` | `ApiCenter.words.search()` | ✅ Implemented (`searchWords`) |
| GET | `/words/public/{word}` | `ApiCenter.words.publicLookup()` | ✅ Implemented (`publicWordLookup`) |
...
## ARCHIVED (folded 2026-05-18) — `docs/api/API_INTEGRATION_ANALYSIS.md` — superseded/resolved by current docs (`api/API_PATH_MAPPING.md`, `backend/BACKEND_RESPONSE_TO_FRONTEND_QUESTIONS.md`). Do NOT use as current guidance. Any "complete/100%" or "not implemented" claims here are historical and may conflict with reality.
...
- ✅ `/api/app_qy_v1/word-groups/{gid}/analysis` - GET - 需要认证 (`custom.authenticate`) - 已集成 (`AppQyV1WordGroupProgressController::getCourseAnalysis` → `api.analyzeCourse` → `CourseDetail.tsx`；返回 `CourseAnalysis`；`knownWords`/`similarity` = 全局记忆重叠 proficiency ≥ 60；此前 404 被 `api.ts` 模拟数据回退掩盖)
...
- ❌ `/api/app_qy_v1/words/{id}/learn` - POST - 后端未实现 / deferred (`markAsLearned` missing)
- ❌ `/api/app_qy_v1/words/{id}/review` - POST - 后端未实现 / deferred (`markAsReviewed` missing)
- ❌ `/api/app_qy_v1/words/{id}/favorite` - POST - 后端未实现 / deferred (`toggleFavorite` missing)
- ⚠️ `/api/app_qy_v1/words/search/{query}` - GET - 需要认证 - 后端已实现 (`searchWords`)，前端**未集成**
...
## ARCHIVED (folded 2026-05-18) — `docs/api/AI_ENDPOINT_VERIFICATION_REPORT.md` — superseded/resolved by current docs (`api/API_PATH_MAPPING.md`, `backend/BACKEND_RESPONSE_TO_FRONTEND_QUESTIONS.md`). Do NOT use as current guidance. Any "complete/100%" or "not implemented" claims here are historical and may conflict with reality.
