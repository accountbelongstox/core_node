...
//                    });
                ...
                int ratio = 3; // 3 for portrait, 1 for landscape
...
{
    // Scroll area for the entire left sidebar
    ...
    // Right-click popup for creating new group on left menu bar
    m_scrollArea->installEventFilter(this);
    // Test the actual area of the space, the entire area turns purple when the mouse hovers over it
...
//                                        .QScrollArea:hover{background:rgba(200, 0, 200, 200);}");
    ...
    /**
     * @brief Initialize m_nodeDataList for display based on configuration data in devicegroups->groupTree
     *           (including each group and each device in the group)
     */
    ...
    /**
     * @brief Add specific layout based on m_nodeDataList data information
     */
    ...
    for (int i = 0; i < deviceCount; i++)
    {
        ...
        // Change the height of the entire left sidebar after group expands or collapses
        ...
        // Set group title information first;
...
// Change the size occupied by the entire left sidebar when a group is expanded
...
void CustomTreeWidget::onUpdateData()
{
    ...
    // Update data for one node every 500ms;
...
void DeviceListWidget::initTitleBackWidget()
{
    ...
    // Change device bar color when mouse hovers over it
...
bool DeviceListWidget::eventFilter(QObject *watched, QEvent *event)
{
    if (watched == m_titleBackWidget)
    {
        if (event->type() == QEvent::MouseButtonRelease)
        {
            ...
            {
                // Expand or collapse nodes on click;
                if (m_isFolded)
                {
...
// Set group title bar information;
...
void DeviceListWidget::onDeleteDevice(int deviceIndex)
{
    ...
    delete item;
    m_listWidget->setFixedHeight(m_listWidget->count() * ITEM_HEIGHT + 10);
    // Whether the current group display size changes after deleting or moving a device
    this->setFixedHeight(m_titleBackWidget->height() + m_listWidget->height());
...
void DeviceItemWidget::initWidget()
{
    ...
    // Right-click popup for deleting device and moving to group
