...
Device::Device(DeviceParams params, QObject *parent, Ui::MainWindow *ui) : QObject(parent), m_params(params)
{
    this->gridLayout = ui->gridLayout;
    ...
    if (params.display) {
        ...
        m_videoForm = new VideoForm(params.framelessWindow, Config::getInstance().getSkin(), nullptr, ui);
        m_videoForm->setDevice(this);
        gridLayout->addWidget(m_videoForm, row, (int)column % 6, 1, 1);
        column++;
        if(column % 6 == 0) {
            row++;
        }
