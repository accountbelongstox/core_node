/**
 * Web shim for @capacitor/core (wordflow end).
 *
 * The unified shell is a plain web build, so there is no Capacitor runtime.
 * WordFlow's services guard every native call behind Capacitor.isNativePlatform(),
...
export function registerPlugin<T = any>(_name: string, _impl?: unknown): T {
  ...
  // does not throw at import time. Real features come from WordFlow's web fallbacks.
  return {} as T;
}
