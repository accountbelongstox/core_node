/* [v4.1-Iris] Notification Settings — ported from
 * qy_capacitor/pages/Settings/Notifications.tsx. The notifications section
 * (enabled / dailyReminder / achievementAlerts) persists via wfSettingsCenter;
 * the remaining toggles are local UI prefs. Permission grant + test
 * notification use the browser Notification API (try/caught, graceful when
 * unavailable). Self-contained: useNavigate + wfPath(), useWfApp().t, shared
 * Iris primitives. Iris reference parity. */
import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Card, Icons, BackButton, SectionTitle } from '../WfUI';
...
const WfSettingsNotificationsPage: React.FC = () => {
  ...
  // Hydrate the notifications section from the settings center on mount.
  useEffect(() => {
    let alive = true;
    wfSettingsCenter.load().then((s) => {
      if (!alive) return;
      setDailyReminder(s.notifications.dailyReminder);
      setAchievements(s.notifications.achievementAlerts);
    });
    return () => { alive = false; };
  }, []);

  const changeDailyReminder = (value: boolean) => {
    setDailyReminder(value);
    wfSettingsCenter.update({ notifications: { dailyReminder: value } });
  };

  const changeAchievements = (value: boolean) => {
    setAchievements(value);
    wfSettingsCenter.update({ notifications: { achievementAlerts: value } });
  };
    ...
    try {
      if ('Notification' in window) {
        ...
        if (permission === 'granted') {
          wfSettingsCenter.update({ notifications: { enabled: true } });
      ...
      console.error('[WfSettingsNotifications] permission request failed:', e);
    }
  };
  ...
  const sendTestNotification = () => {
    try {
      if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('WordFlow Test', { body: 'Notifications are working!' });
      } else {
        alert('Please enable notifications in your browser settings');
      }
    } catch (e) {
      console.error('[WfSettingsNotifications] test notification failed:', e);
    }
  };
  ...
  return (
    <div className="min-h-screen pb-28">
      ...
      <div className="relative w-full max-w-md mx-auto px-[var(--page-padding-h)] pt-[var(--page-padding-v)] pb-[var(--space-breath)]">
        ...
        <p className="text-[var(--color-text-secondary)] pl-1">Manage your notification preferences</p>
      </div>
        ...
        <div className="ds-stack-tight flex flex-col">
          ...
          {dailyReminder && (
            <div className="ds-row w-full p-4 ml-8 flex items-center justify-between gap-3">
              ...
              <span className="text-sm text-[var(--klein-blue)] font-mono flex-shrink-0">20:00</span>
            </div>
          )}
          ...
          <Card>
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  <div className={`w-5 h-5 bg-white rounded-full shadow-sm transform transition-transform ${doNotDisturb ? 'translate-x-5' : ''}`} />
                </button>
              </div>
...
export default WfSettingsNotificationsPage;
