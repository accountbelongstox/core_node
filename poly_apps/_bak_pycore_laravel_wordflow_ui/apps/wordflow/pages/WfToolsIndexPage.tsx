/* [v4.1-Iris] Tools Hub — ported from
 * poly_apps/qy_capacitor/pages/Tools/Index.tsx. Self-contained: a static tool
 * catalog with featured gradient hero bento, recently-used icon tiles
 * (persisted in localStorage), and a category pill filter. react-router
 * useNavigate + wfPath() for every /wordflow nav, useWfT() for the title.
 * Faithful Iris look (design-reference-{light,dark}.webp). */
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { Icons, Card, SectionTitle, IconTile } from '../WfUI';
import { useWfT } from '../WfAppContext';
import { wfPath } from '../WfBottomTabNav';

const RECENT_TOOLS_KEY = 'wf_recent_tools';
...
const WfToolsIndexPage: React.FC = () => {
  ...
  // Restore recently-used tools from localStorage (degrade to empty on failure).
  useEffect(() => {
    try {
      const raw = localStorage.getItem(RECENT_TOOLS_KEY);
      const parsed = raw ? JSON.parse(raw) : [];
      setRecentTools(Array.isArray(parsed) ? parsed : []);
    } catch {
      setRecentTools([]);
    }
  }, []);
  ...
  const recentToolsData = tools.filter(
    (tool) => Array.isArray(recentTools) && recentTools.includes(tool.id)
  );
  ...
  return (
    <div className="ds-page ds-section-gap route-fade pt-20 pb-32">
      ...
      {/* Category pill nav */}
      <div className="ds-pill-nav" role="tablist" aria-label="Tool categories">
        {categories.map((cat) => (
          <button
            key={cat.id}
            type="button"
            ...
            className={`ds-pill-chip ${activeCategory === cat.id ? 'is-active' : ''}`}
          >
            {cat.label}
          </button>
        ))}
...
export default WfToolsIndexPage;
