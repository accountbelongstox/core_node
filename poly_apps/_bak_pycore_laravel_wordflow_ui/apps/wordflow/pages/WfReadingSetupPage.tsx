/* [v4.1-Iris] Reading Setup — ported from qy_capacitor/pages/Reading/Setup.tsx.
 * Self-contained: lists study materials from wordflowApi.getWordGroups() and
 * launches the immersive reading runner with the chosen group via wfPath(). API
 * call is try/caught and degrades to an EmptyState. Faithful Iris look. */
...
const WfReadingSetupPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.error('[WfReadingSetup] Failed to load groups:', error);
  ...
  const openRunner = (group: WordGroup) => {
    navigate(`${wfPath('reading_run')}?groupId=${encodeURIComponent(group.id)}&language=${encodeURIComponent(group.language || 'en')}`);
  };
  ...
  return (
    <div className="ds-page ds-section-gap route-fade h-full flex flex-col pt-12 pb-32">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-[var(--color-text-primary)] truncate">{t('reading.library') || 'Library'}</h1>
      </div>
...
export default WfReadingSetupPage;
