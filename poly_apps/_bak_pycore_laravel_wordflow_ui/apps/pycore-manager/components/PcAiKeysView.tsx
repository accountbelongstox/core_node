...
const PcAiKeysView: React.FC<{ refreshSignal?: number }> = ({ refreshSignal }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (r && r.success === true && Array.isArray(r.providers)) {
        setProviders(r.providers);
        setRawKeyFiles(Array.isArray(r.raw_key_files) ? r.raw_key_files : []);
        setError(null);
        setUnreachable(false);
      } else {
        // Reachable but not the expected payload. getJSON does not throw on a
        // non-2xx status, so a 404 from a STALE pycore (one started before the
        // /api/local/ai/keys route was wired in) arrives here as
        // {detail:"Not Found"}. Surface it instead of silently rendering an
        // empty provider list as "No AI providers reported.".
        const detail = (r as any)?.detail || r?.error;
        setProviders(null);
        setRawKeyFiles([]);
        setError(detail
          ? `${detail} — AI keys endpoint missing; restart pycore to load it.`
          : 'AI keys endpoint returned no providers; restart pycore to load it.');
        setUnreachable(false);
      }
    } catch (e: any) {
      setProviders(null);
