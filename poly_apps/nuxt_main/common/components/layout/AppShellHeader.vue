<template>
    <header :class="{ dark: config.semidark && config.menu === 'horizontal' }">
        <div class="shadow-sm">
            <div class="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-[#0e1726]">
                ...
                >
                    ...
                    <div class="dropdown shrink-0">
                        <client-only>
                            <Popper :placement="config.rtlClass === 'rtl' ? 'bottom-end' : 'bottom-start'" offsetDistance="8">
                                <button
                                    type="button"
                                    class="block rounded-full bg-white-light/40 p-2 hover:bg-white-light/90 hover:text-primary dark:bg-dark/40 dark:hover:bg-dark/60"
                                >
                                ...
                                <template #content="{ close }">
                                    <ul
                                        ...
                                        @click="close()"
                                    >
                                        <template v-for="item in languageOptions" :key="item.code">
                                            <li>
                                                <button
                                                    type="button"
                                                    class="w-full hover:text-primary"
                                                    :class="{ 'bg-primary/10 text-primary': config.locale === item?.code }"
                    ...
                    <div class="dropdown shrink-0">
                        <client-only>
                            <Popper :placement="config.rtlClass === 'rtl' ? 'bottom-start' : 'bottom-end'" offsetDistance="8">
                                <button
                                    type="button"
                                    class="block rounded-full bg-white-light/40 p-2 hover:bg-white-light/90 hover:text-primary dark:bg-dark/40 dark:hover:bg-dark/60"
                                >
                    ...
                    <div class="dropdown shrink-0">
                        <client-only>
                            <Popper :placement="config.rtlClass === 'rtl' ? 'bottom-end' : 'bottom-start'" offsetDistance="8" class="!block">
            ...
            >
                    ...
                    <ul class="sub-menu">
                        ...
                        <li class="relative">
                            ...
                            >
                                ...
                                <li>
                                    <NuxtLink to="/datatables/alt-pagination">{{ $t('alt._pagination') }}</NuxtLink>
                                </li>
                                <li>
                ...
                <li class="menu nav-item relative">
                    ...
                    <ul class="sub-menu">
                        ...
                        <li>
                            <NuxtLink to="/forms/checkbox-radio">{{ $t('checkbox_&_radio') }}</NuxtLink>
                        </li>
                        <li>
                        ...
                        <li>
                            <NuxtLink to="/forms/date-picker">{{ $t('date_&_range_picker') }}</NuxtLink>
                        </li>
                        <li>
...
<script lang="ts" setup>
    ...
    const { config, setTheme, setLocale: setConfigLocale } = useGlobalConfig();
    const languageOptions = computed(() => store.languageList);

    // multi language
    const changeLanguage = (item: any) => {
        const available = languageOptions.value;
        if (!available || available.length === 0) {
            return;
        }
        let lang = item;
        if (!lang) {
            lang = available.find((option) => option.code === config.value.locale);
        }
        if (!lang) {
            lang = available.find((option) => option.code === 'en') || available[0];
        }
        if (!lang) {
            return;
        }
        const code = lang.code;
        setConfigLocale(code);
        store.toggleLocale(code, (newLocale: string) => {
            locale.value = newLocale;
        });
    };
    const currentFlag = computed(() => {
        const code = config.value.locale || 'en';
