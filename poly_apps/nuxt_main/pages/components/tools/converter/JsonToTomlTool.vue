<template>
  <div class="h-full flex flex-col bg-white">
        ...
        <section class="border border-slate-200 rounded-xl bg-white shadow-sm">
          <header class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-500">Provide JSON to convert into TOML configuration format.</p>
            </div>
            <button
              @click="jsonInput = defaultJson"
          ...
          <div class="px-5 py-4">
            <textarea
              v-model="jsonInput"
              rows="12"
              class="w-full px-4 py-3 border border-slate-200 rounded-xl shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent transition font-mono text-sm"
            ></textarea>
          </div>
        </section>
        ...
        <section class="border border-slate-200 rounded-xl bg-white shadow-sm">
          <header class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-500">Configuration-friendly TOML result.</p>
            </div>
    ...
    <div class="px-6 py-4 border-t bg-slate-50 flex items-center justify-between">
      ...
      <span class="text-xs text-slate-500">Endpoint: <code class="text-slate-700">/converter/json-to-toml</code></span>
    </div>
  </div>
</template>
...
const convert = async () => {
  ...
  try {
    const response = await props.api.jsonToToml(jsonInput.value);
    ...
    executionTime.value = Math.round(performance.now() - start);
    tomlOutput.value = '';
    error.value = err?.message || 'JSON → TOML service unavailable';
  } finally {
    loading.value = false;
  }
};
