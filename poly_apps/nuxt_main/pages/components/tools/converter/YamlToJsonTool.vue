<template>
  <div class="h-full flex flex-col bg-white">
    <div class="px-6 py-4 border-b bg-gradient-to-r from-emerald-50 to-lime-50">
        ...
        <section class="border border-slate-200 rounded-xl bg-white shadow-sm">
          <header class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-500">Provide YAML content to convert into JSON.</p>
            </div>
            <button
              @click="yamlInput = ''"
        ...
        <section class="border border-slate-200 rounded-xl bg-white shadow-sm">
          <header class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-500">Converted JSON, minified for easy reuse.</p>
            </div>
    ...
    <div class="px-6 py-4 border-t bg-slate-50 flex items-center justify-between">
      ...
      <span class="text-xs text-slate-500">Endpoint: <code class="text-slate-700">/converter/yaml-to-json</code></span>
    </div>
  </div>
</template>
...
const convert = async () => {
  ...
  try {
    const response = await props.api.yamlToJson(yamlInput.value);
    ...
    executionTime.value = Math.round(performance.now() - start);
    jsonOutput.value = '';
    error.value = err?.message || 'YAML → JSON service unavailable';
  } finally {
    loading.value = false;
  }
};
