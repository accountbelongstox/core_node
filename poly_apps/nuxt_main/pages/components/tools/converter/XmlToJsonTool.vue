<template>
  <div class="h-full flex flex-col bg-white">
    <div class="px-6 py-4 border-b bg-gradient-to-r from-lime-50 to-teal-50">
        ...
        <section class="border border-slate-200 rounded-xl bg-white shadow-sm">
          <header class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-500">Provide XML content. Converter will produce JSON.</p>
            </div>
            <button
              @click="xmlInput = ''"
        ...
        <section class="border border-slate-200 rounded-xl bg-white shadow-sm">
          <header class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-500">Converted JSON string.</p>
            </div>
    ...
    <div class="px-6 py-4 border-t bg-slate-50 flex items-center justify-between">
      <button
        @click="convert"
        :disabled="xmlInput.trim().length === 0 || loading"
      ...
      <span class="text-xs text-slate-500">Endpoint: <code class="text-slate-700">/converter/xml-to-json</code></span>
    </div>
  </div>
</template>
...
const convert = async () => {
  if (!xmlInput.value.trim() || loading.value) return;
  ...
  try {
    const response = await props.api.xmlToJson(xmlInput.value);
    ...
    executionTime.value = Math.round(performance.now() - start);
    jsonOutput.value = '';
    error.value = err?.message || 'XML → JSON service unavailable';
  } finally {
    loading.value = false;
  }
};
