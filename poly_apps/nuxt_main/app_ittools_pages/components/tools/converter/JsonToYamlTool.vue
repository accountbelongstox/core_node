<template>
  <div class="h-full flex flex-col bg-white">
        ...
        <section class="border border-slate-200 rounded-xl bg-white shadow-sm">
          <header class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-slate-500">Converted YAML with standard indentation.</p>
            </div>
    ...
    <div class="px-6 py-4 border-t bg-slate-50 flex items-center justify-between">
      ...
      <span class="text-xs text-slate-500">Endpoint: <code class="text-slate-700">/converter/json-to-yaml</code></span>
    </div>
  </div>
</template>
...
const convert = async () => {
  ...
  try {
    const response = await props.api.jsonToYaml(jsonInput.value);
    ...
    executionTime.value = Math.round(performance.now() - start);
    yamlOutput.value = '';
    error.value = err?.message || 'JSON → YAML service unavailable';
  } finally {
    loading.value = false;
  }
};
