...
import { API_BASE_URL } from '~/app_ittools_pages/config/api-endpoints'
...
const fetchResources = async () => {
  ...
  try {
    const response = await api.call('STATIC_FILE_TREE')

    if (response.success && response.data) {
      resources.value = parseResources(response.data)
    } else {
      throw new Error(response.error?.message || t('errors.unknownError'))
    }
  } catch (err: any) {
...
const getPreviewUrl = (resource: ResourceFile): string => {
  // Build preview URL using unified API base URL
  return `${API_BASE_URL.replace('/api', '')}/static-resources/stream-file?path=${encodeURIComponent(resource.path)}`
}
...
const startUpload = async () => {
  ...
  try {
    for (const file of uploadQueue.value) {
      ...
      await api.post('STATIC_UPLOAD', formData)
    }
