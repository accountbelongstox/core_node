@echo off
REM nuxt_main:dashboard Install Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for dependency management
REM This bat file serves as the Windows entry point for installing nuxt_main:dashboard dependencies
REM IMPORTANT: All Poly applications MUST use BAT triggers to call PS1 scripts

echo [INFO] Installing nuxt_main:dashboard application dependencies...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] nuxt_main:dashboard installation failed with exit code: %ERRORLEVEL%
    exit /b %ERRORLEVEL%
)

echo [SUCCESS] nuxt_main:dashboard installation completed
exit /b 0
