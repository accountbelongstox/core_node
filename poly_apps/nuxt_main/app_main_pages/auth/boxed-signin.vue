<template>
    <div
        class="flex min-h-screen items-center justify-center bg-[url('/assets/images/map.svg')] bg-cover bg-center dark:bg-[url('/assets/images/map-dark.svg')]"
    >
        <div class="panel m-6 w-full max-w-lg sm:w-[480px]">
            ...
            <form class="space-y-5" @submit.prevent="handleLogin">
                ...
                <div>
                    ...
                    >
                        ...
                        <optgroup label="Task Administrators">
                            <option 
                                v-for="role in taskAdminRoles" 
                                :key="role.id" 
                                :value="role.id"
                            >
                                {{ role.name }}
                        ...
                        <optgroup label="Platform Administrators">
                            <option 
                                v-for="role in platformAdminRoles" 
                                :key="role.id" 
                                :value="role.id"
                            >
                                {{ role.name }}
            ...
            <p class="text-center">
                Dont't have an account ? <NuxtLink to="/auth/boxed-signup" class="font-bold text-primary hover:underline">Sign Up</NuxtLink>
            </p>
        </div>
    </div>
</template>
<script lang="ts" setup>
    ...
    // 获取角色列表
    const taskAdminRoles = computed(() => appStore.getTaskAdminRoles());
    const platformAdminRoles = computed(() => appStore.getTestPlatformAdminRoles());
    const systemRoles = computed(() => appStore.getRoles().filter((role: Role) => 
        !role.id.startsWith('task_admin_') && role.id !== 'test_platform_admin'
    ));

    // 处理登录
    const handleLogin = async () => {
        if (!loginForm.value.email || !loginForm.value.password || !loginForm.value.roleId) {
        ...
        try {
            ...
            // 设置用户信息
            const selectedRole = appStore.getRoles().find((role: Role) => role.id === loginForm.value.roleId);
            ...
            // 设置用户角色
            if (selectedRole) {
                appStore.setUserRole(selectedRole.id);
            }
