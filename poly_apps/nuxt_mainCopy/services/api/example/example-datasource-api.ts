...
// Example App Data Source API Service
export class ExampleDataSourceAPI {
  private baseUrl: string;
  private namespace: string;

  constructor() {
    const appConfig = getAppEntryConfig('example');
    this.baseUrl = appConfig.api.baseUrl;
    this.namespace = appConfig.api.namespace;
  }

  // Get all data sources for example app
  async getDataSources(): Promise<DataSourceConfig[]> {
    try {
      ...
      console.error('Failed to fetch example app data sources:', error);
      return [];
    }
  }
  ...
  async getDataSource(id: string): Promise<DataSourceConfig | null> {
    try {
      ...
      console.error(`Failed to fetch example app data source ${id}:`, error);
      return null;
    }
  }
  ...
  async createDataSource(config: Partial<DataSourceConfig>): Promise<DataSourceConfig | null> {
    try {
      ...
      console.error('Failed to create example app data source:', error);
      return null;
    }
  }
  ...
  async updateDataSource(id: string, config: Partial<DataSourceConfig>): Promise<DataSourceConfig | null> {
    try {
      ...
      console.error(`Failed to update example app data source ${id}:`, error);
      return null;
    }
  }
  ...
  async deleteDataSource(id: string): Promise<boolean> {
    try {
      ...
      console.error(`Failed to delete example app data source ${id}:`, error);
      return false;
    }
  }
  ...
  async testConnection(id: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      console.error(`Failed to test example app data source ${id}:`, error);
  ...
  async getStatistics(): Promise<{
    ...
    try {
      ...
      console.error('Failed to fetch example app data source statistics:', error);
  ...
  async getHealthStatus(): Promise<Record<string, DataSourceStatus>> {
    try {
      ...
      console.error('Failed to fetch example app data source health status:', error);
      return {};
    }
  }
}
...
export const exampleDataSourceAPI = new ExampleDataSourceAPI();
export default exampleDataSourceAPI;
