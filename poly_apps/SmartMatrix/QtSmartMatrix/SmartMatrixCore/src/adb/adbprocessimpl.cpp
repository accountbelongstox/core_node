...
const QString &AdbProcessImpl::getAdbPath()
{
    if (s_adbPath.isEmpty()) {
        ...
        potentialPaths << QString::fromLocal8Bit(qgetenv("SmartMatrix_ADB_PATH")) << g_adbPath
#ifdef Q_OS_WIN32
...
QStringList AdbProcessImpl::getDevicesSerialFromStdOut()
{
    ...
    QStringList serials;
    // Qt5 API: QRegExp lineExp("\r\n|\n"); QRegExp tExp("\t");
    // Qt6 API: QRegularExpression replaces QRegExp (both work with QString::split())
...
QString AdbProcessImpl::getDeviceIPFromStdOut()
{
    ...
    // Qt5 API: QRegExp ipRegExp(strIPExp, Qt::CaseInsensitive);
    //          if (ipRegExp.indexIn(m_standardOutput) != -1) { ip = ipRegExp.cap(0); }
    // Qt6 API: QRegularExpression with match() returns QRegularExpressionMatch
    //          Qt::CaseInsensitive -> QRegularExpression::CaseInsensitiveOption
    //          indexIn() -> match().hasMatch(), cap() -> captured()
