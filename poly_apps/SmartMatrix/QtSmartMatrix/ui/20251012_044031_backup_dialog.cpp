...
void Dialog::on_languageComboBox_currentIndexChanged(const QString &language)
{
    ...
    Config::getInstance().m_settings->beginGroup("common");
    Config::getInstance().m_settings->setValue("Language", languageCode);
    Config::getInstance().m_settings->endGroup();
    Config::getInstance().m_settings->sync();
