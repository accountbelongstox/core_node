...
#ifndef Q_OS_OSX
        // Removing title bar on Mac affects showfullscreen
        // Remove title bar
        setWindowFlags(windowFlags() | Qt::FramelessWindowHint);
...
void VideoForm::switchFullScreen()
{
    if (isFullScreen()) {
        ...
        // Landscape fullscreen fills the screen without maintaining aspect ratio
        if (m_widthHeightRatio > 1.0f) {
        ...
        // This temporary title bar addition before fullscreen causes mousemove events to be lost, making setmousetrack ineffective
        ...
        // Prevent computer sleep and screen saver in fullscreen mode
#ifdef Q_OS_WIN32
...
void VideoForm::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->button() == Qt::RightButton) {
        ...
        // Qt6: Use position() and globalPosition() instead of localPos() and globalPos()
...
{
    auto device = qsc::IDeviceManage::getInstance().getDevice(m_serial);
    // Qt6: position() is available since Qt 5.15
    if (m_videoWidget->geometry().contains(event->position().toPoint())) {
        ...
        device->wheelEvent(&wheelEvent, m_videoWidget->frameSize(), m_videoWidget->size());
    }
}
...
void VideoForm::paintEvent(QPaintEvent *paint)
{
    Q_UNUSED(paint)
    QStyleOption opt;
    // Qt6: Use initFrom() instead of init()
    opt.initFrom(this);
    QPainter p(this);
