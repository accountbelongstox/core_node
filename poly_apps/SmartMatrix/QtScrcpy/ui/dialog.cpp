...
// Qt5 API: #include <QTime>  (QTime::currentTime() deprecated)
// Qt6 API: Use QElapsedTimer for time measurements
#include <QElapsedTimer>
#include <QTimer>
#include <QTranslator>
#include <QApplication>
// Qt5 API: #include <QRegExp>
// Qt6 API: QRegularExpression replaces QRegExp for pattern matching
...
void Dialog::initUI()
{
    ...
    // Add right-click context menu for deviceIpEdt
    ...
    // Add right-click context menu for devicePortEdt
    ...
    QString currentLanguage = Config::getInstance().getLanguage();
    if (currentLanguage == "zh_CN") {
        ui->languageComboBox->setCurrentIndex(2); // 中文
    } else if (currentLanguage == "en_US") {
        ui->languageComboBox->setCurrentIndex(1); // English
    } else if (currentLanguage == "ja_JP") {
        ui->languageComboBox->setCurrentIndex(3); // 日本語
    } else {
        ui->languageComboBox->setCurrentIndex(0); // Auto
    }
}
...
void Dialog::execAdbCmd()
{
    ...
    // Qt5 (< 5.15): QString::SkipEmptyParts
    // Qt5 (>= 5.15) & Qt6: Qt::SkipEmptyParts (enum moved from QString to Qt namespace)
    m_adb.execute(ui->serialBox->currentText().trimmed(), cmd.split(" ", Qt::SkipEmptyParts));
}

void Dialog::delayMs(int ms)
{
    // Qt5 API: QTime dieTime = QTime::currentTime().addMSecs(ms);
    //          while (QTime::currentTime() < dieTime) { ... }
    // Qt6 API: QElapsedTimer provides better performance for time measurements
...
void Dialog::on_wirelessConnectBtn_clicked()
{
    ...
    // Save IP history - only save IP part, not including port
...
#ifdef Q_OS_WIN32
    // On Windows, showing too early allows seeing the resize process
...
int Dialog::findDeviceFromeSerialBox(bool wifi)
{
    ...
    // Qt5 API: QRegExp regIP(regStr);
    //          bool isWifi = regIP.exactMatch(ui->serialBox->itemText(i));
    // Qt6 API: QRegularExpression replaces QRegExp
    //          exactMatch() -> match().hasMatch() (checks if pattern matches entire string)
...
void Dialog::on_languageComboBox_currentIndexChanged(const QString &language)
{
    ...
    Config::getInstance().setLanguage(languageCode);

    // Show message to restart application
    QMessageBox::information(this, tr("Language Change"), 
        tr("Language setting has been saved. Please restart the application to apply the new language."));
}
