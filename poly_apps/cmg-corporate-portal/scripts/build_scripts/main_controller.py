...
from pre_build_checker import PreBuildChecker
...
"""
    ...
    def prepare_android_build(self) -> None:
        ...
        print("\n[Python] Running pre-build checks...")
        checker = PreBuildChecker(
            project_root=self.project_root,
            android_path=self.android_path,
            assets_path=self.assets_path,
            build_config_path=self.build_config_path
        )
        
        all_passed, check_results = checker.run_all_checks()
        checker.print_report()
        
        # If critical errors exist, block build
        critical_errors = [c for c in check_results if c.severity == 'error' and not c.passed]
        if critical_errors:
            print("\n" + "=" * 80)
            print("🚫 BUILD BLOCKED: Critical errors detected")
            print("=" * 80)
            print("\nPlease fix the errors above before proceeding with the build.")
            print("The AI assistant can help you fix these issues.")
            print("\nAfter fixing, you can:")
            print("  1. Re-run the build script")
            print("  2. Or manually continue if you believe the errors are false positives")
            print("=" * 80)
            
            # Ask user if they want to continue anyway
            print("\n⚠️  Do you want to continue with build despite errors? (not recommended)")
            print("   This may cause build failures.")
            try:
                response = input("   Continue anyway? (yes/no): ").strip().lower()
                if response not in ['yes', 'y']:
                    self.var_system.set_var("ERROR", "pre_build_checks_failed")
                    return
