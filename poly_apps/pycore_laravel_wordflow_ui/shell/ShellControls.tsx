...
/** localStorage key for the dock's persisted vertical position (px from top). */
const DOCK_Y_KEY = 'wf_shell_dock_y';
...
export const ShellControls: React.FC = () => {
  ...
  const [dockY, setDockY] = useState<number>(() => {
    const saved = Number(localStorage.getItem(DOCK_Y_KEY));
  ...
  const onPointerUp = useCallback((e: React.PointerEvent<HTMLButtonElement>) => {
    ...
    if (d.moved) {
      ...
      setDockY(newY);
      localStorage.setItem(DOCK_Y_KEY, String(newY));
      setSnapX(dx);
    } else {
