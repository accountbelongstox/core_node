 ...
 * "LIVE" emerald column (balance / positions / bills from callRpc('okx.account_overview'))
...
export const OkxAccountPanel: React.FC<Props> = ({ dark, lang, simCash, simPositionsCount, simEquity }) => {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const r = await callRpc('okx.account_overview', {}, 12000);
      if (r) setAcct(r);
  ...
  // initial load driven by WS readiness, mirroring OkxBacktestPanel.
  useEffect(() => {
    connectPycoreWs();
    const off = onWsStatus((c) => { if (c) refresh(); });
    return () => { off(); };
