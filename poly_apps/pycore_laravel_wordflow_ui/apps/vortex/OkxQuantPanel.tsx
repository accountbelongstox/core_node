 ...
 * service over the SINGLE pycore WS interface (callRpc('okx.*')); on WS connect it
 * pulls callRpc('okx.quant_info') and renders read-only operational cards:
...
import { connectPycoreWs, callRpc, onWsStatus } from '../../core/api-libs/pycore';
...
export const OkxQuantPanel: React.FC<{ dark: boolean; lang: string }> = ({ dark, lang }) => {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const r = await callRpc('okx.quant_info', {}, 10000);
      if (r) setInfo(r);
  ...
  // initial load driven by WS readiness (onWsStatus fires immediately + on connect),
  ...
  useEffect(() => {
    connectPycoreWs();
    const off = onWsStatus((c) => { if (c) { refresh(); loadSerSettings(); } });
    return () => { off(); };
  ...
  const toggleReveal = useCallback(async () => {
    ...
    setRevealing(true);
    try {
      const r = await callRpc('okx.reveal_credentials', {}, 10000);
  ...
  const serialize = useCallback(async () => {
    ...
    try {
      await callRpc('okx.serialize', {}, 15000);
  ...
  const togglePreopen = useCallback(async () => {
    ...
    try {
      const r = await callRpc('okx.preopen', {}, 15000);
