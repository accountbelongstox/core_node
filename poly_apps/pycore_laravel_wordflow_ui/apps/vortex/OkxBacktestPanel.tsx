 ...
 *
 * Talks to the pycore OKX market-data service over the SINGLE pycore WS interface
 * (callRpc('okx.*') + subscribe('okx_market_*')). Nothing fetches until the user
 * presses "填充回测数据 / Fill backtest data", which kicks off the rate-limited
 * gap-fill + catch-up on the backend; live progress streams over WS. The table is
...
import { connectPycoreWs, subscribe, callRpc, onWsStatus } from '../../core/api-libs/pycore';
...
export const OkxBacktestPanel: React.FC<{ dark: boolean; lang: string }> = ({ dark, lang }) => {
  ...
  const refreshCoins = useCallback(async () => {
    try {
      const [st, cs] = await Promise.all([
        callRpc('okx.status', {}, 8000),
        callRpc('okx.coins', { bar: barRef.current }, 15000),
      ]);
      if (st) setStatus(st);
  ...
  // initial load + WS subscriptions. NOTE: do NOT call refreshCoins() synchronously
  // here — callRpc rejects before the socket is OPEN, which would flash "unreachable".
  // onWsStatus fires with the current state immediately (and again on connect), so it
  // drives the first load exactly when the WS is ready (covers an already-open socket).
  ...
  const loadSettings = useCallback(async () => {
    try {
      const s = await callRpc('okx.get_settings', {}, 8000);
      if (s) {
  ...
  const saveQuote = (q: 'usdt' | 'usdc' | 'all') => {
    ...
    callRpc('okx.set_settings', { display_quote: q }, 8000).catch(() => { /* best-effort */ });
  };

  useEffect(() => {
    connectPycoreWs();
    const offStatus = onWsStatus((c) => { setWsOk(c); if (c) { refreshCoins(); loadSettings(); } });
    const offS = subscribe('okx_market_status', (d: any) => setStatus(d || {}));
    const offP = subscribe('okx_market_progress', (d: Progress) => {
    ...
    const offU = subscribe('okx_market_update', () => { /* universe/tick — table refresh below */ });
  ...
  // smoothly between WS events. Reset when a new op begins (state 'starting' or op change)
  ...
  // keep the local clock in sync with backend-reported elapsed (latest WS event wins)
  useEffect(() => {
  ...
  //   2)  fill the backtest history (gap-fill; progress streams over WS).
  const startAll = async () => {
    ...
    try {
      ...
      try { await callRpc('okx.load_universe', {}, 30000); setUnreachable(false); refreshCoins(); }
      catch { /* best-effort */ }
      setLoadingUniverse(false);
      // Phase 1b — DIFF from local fragments → chart, BEFORE any download.
      try {
        const plan = await callRpc('okx.fill_plan', { bar, hours, scope }, 20000);
        if (plan && !plan.error) setFillPlan(plan as FillPlan);
      } catch { /* plan is best-effort */ }
      // Phase 2 — download only the missing ranges (progress streams over WS).
      await callRpc('okx.fill_backtest', { bar, hours, scope }, 8000);
      setProgress({ state: 'starting', op: 'fill', phase: 'diff' });
    } catch { setUnreachable(true); }
    finally { setLoading(false); setLoadingUniverse(false); refreshCoins(); }
  };
  const cancelFill = async () => { try { await callRpc('okx.cancel_fill', {}, 8000); } catch { /* ignore */ } };
  ...
  const saveSetting = async (patch: Partial<typeof settings>) => {
    ...
    setSettings(next);
    try { await callRpc('okx.set_settings', patch, 8000); } catch { /* best-effort */ }
  };

  const openChart = useCallback(async (inst_id: string) => {
    setSelected(inst_id); setSelCandles([]); setSelLoading(true);
    try {
      const r = await callRpc('okx.candles', { inst_id, bar: barRef.current }, 15000);
  ...
  useEffect(() => {
    if (view !== 'charts' || !wsOk || chartCoins.length === 0) return;
    ...
    (async () => {
      try {
        const r = await callRpc('okx.sparklines', { inst_ids: chartCoins.map((c) => c.inst_id), bar, points: 32 }, 20000);
        if (alive && r?.series) setSpark(r.series as Record<string, number[]>);
      } catch { /* leave prior series; cards show no-data */ }
    })();
    return () => { alive = false; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, wsOk, bar, chartKey]);
  ...
  const cmpReq = useRef('');
  useEffect(() => {
    if (view !== 'compare' || !wsOk || cmpCoins.length === 0) return;
    ...
    (async () => {
      try {
        ...
        const [sp, mt] = await Promise.all([
          callRpc('okx.sparklines', { inst_ids: ids, bar, points: 200, hours }, 25000),
          callRpc('okx.metrics', { inst_ids: ids, bar, hours }, 25000),
        ]);
