 ...
 *
 * The UI consumes one RPC v2 snapshot and only normalizes display-safe scalar
...
import {
  ...
  pycoreApi,
  PYCORE_LARAVEL_API_CHANGED_EVENT,
...
function readAutoRefreshPref(): boolean {
  try {
    const value = localStorage.getItem(QC_AUTO_KEY);
...
/** App-wide RPC v2 hub. Mount once in PcProviders. */
export const QueueCenterHubProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const poll = useCallback(async (silent = false) => {
    ...
    if (!isWsConnected()) {
      if (!silent) {
        setHub((previous) => ({
          ...previous,
          loading: false,
          pycoreReachable: pycoreHealth.up === false ? false : previous.pycoreReachable,
          hubState: pycoreHealth.up === false ? 'error' : previous.hubState,
        }));
      }
      return;
    }
    ...
    try {
      ...
      if (snapshot.data.recent) taskCenterState.ingestRecent(snapshot.data.recent);
    } catch (error: unknown) {
      if (!mounted.current || currentRequest !== requestId.current) return;
