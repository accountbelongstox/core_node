...
const EXPAND_KEY = 'pc_word_audio_expanded';
const ENGINE_KEY = 'pc_word_audio_engine';
const CONCURRENCY_KEY = 'pc_word_tts_concurrency';
...
export function PcWordAudioPanel(): JSX.Element {
  ...
  const [engine, setEngine] = useState(() => localStorage.getItem(ENGINE_KEY) || 'edge');
  const [concurrencyInput, setConcurrencyInput] = useState(
    () => localStorage.getItem(CONCURRENCY_KEY) ?? '',
  );
  ...
  const saveConcurrency = useCallback((raw: string) => {
    ...
    localStorage.setItem(CONCURRENCY_KEY, raw);
