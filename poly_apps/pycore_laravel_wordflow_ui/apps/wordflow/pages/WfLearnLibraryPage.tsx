...
export const libGradient = (seed: string): string => {
  // Calm, cohesive duotone: muted saturation + soft lightness with a narrow hue
  // sweep, so a wall of covers reads as a harmonious set rather than a clashing
  // rainbow (tuned down from the old vivid 72%/66% pairs to echo the soft,
  // restrained palette of the design reference).
  const a = seedHash(seed) % 360;
  const b = (a + 22) % 360;
  return `linear-gradient(140deg, hsl(${a} 48% 62%) 0%, hsl(${b} 44% 50%) 100%)`;
};
const libInitial = (name: string): string => (name || '?').trim().charAt(0).toUpperCase();
/** Tidy a synced media filename for display: drop the container/subtitle
 * extension and turn dot/underscore separators into spaces (e.g.
 * "The.Matrix.1999.mp4" → "The Matrix 1999"). Falls back to the raw title if
 * cleaning empties it. */
const cleanMediaTitle = (raw: string): string =>
  (raw || '')
    .replace(/\.(mp4|mkv|avi|mov|m4v|webm|flv|wmv|ts|srt|ass|vtt|sub)$/i, '')
    .replace(/[._]+/g, ' ')
    .trim() || raw;
...
export const CoverThumb: React.FC<{
  ...
  // Treat an explicit 'none' (movie DBs + AI cover all missed) like a failure
  // for the affordance: show the monogram placeholder + an optional fetch retry.
  const isFailed = normalized === 'failed' || normalized === 'none';
  ...
  // Failed / none / final 404 fallback. Instead of a generic book glyph, render
  // a bold monogram (like the group chips and the reference's lettered avatars)
  // over the gradient, with a soft sheen for legibility and an optional retry.
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-1 text-white relative overflow-hidden">
      <span
        className="pointer-events-none absolute inset-0 bg-gradient-to-b from-white/10 via-transparent to-black/25"
        aria-hidden
      />
      <span className="relative text-2xl font-black tracking-tight drop-shadow-[0_1px_3px_rgba(0,0,0,0.4)] select-none">
...
const WfLearnLibraryPage: React.FC = () => {
  ...
  // Per-media poster-retry in flight, keyed by `${kind}-${id}`.
  ...
  // Re-fetch the public book/subtitle preview lists (used after a poster retry
  // so the row flips from "failed/none" toward the new poster_status).
  const refreshPublicMedia = React.useCallback(async () => {
    try {
      ...
      console.error('[WfLearnLibrary] Failed to refresh public media:', error);
    }
  }, []);

  // Trigger an on-demand poster fetch for a book/subtitle, then refresh the
  // media preview so the row reflects the new poster_status. Non-blocking.
  const handleRetryPoster = React.useCallback(
    async (kind: 'book' | 'subtitle', id: number) => {
      const key = `${kind}-${id}`;
      setRetryingPosters((prev) => { const n = new Set(prev); n.add(key); return n; });
      try {
        await wordflowApi.retryPoster(kind, { id });
        await refreshPublicMedia();
      } catch (error) {
        console.error('[WfLearnLibrary] Poster retry failed:', error);
      } finally {
              ...
              {publicMediaItems.length === 0 ? (
                ...
                <>
                  <div className="flex items-center justify-between pl-1 pr-0.5">
                    ...
                    <span className="text-[0.7rem] font-bold text-[var(--color-text-tertiary)] tabular-nums">
                      {publicMediaItems.length}
                    </span>
                  </div>
                  {publicMediaItems.map((item) => {
                    const TypeGlyph = item.kind === 'book' ? BookOpen : Captions;
                    return (
                      <div
                        ...
                        className="ds-row group flex items-center gap-3.5 p-3 pr-3.5 cursor-pointer"
                      >
                        {/* Movie/TV poster — tall card (reuses CoverThumb's
                            ready/pending/failed/none lifecycle over a soft
                            gradient) with a type-glyph corner badge. */}
                        <div className="relative flex-shrink-0">
                          <div
                            className="ds-cover w-12 h-[4.25rem] rounded-xl overflow-hidden shadow-[0_8px_18px_-8px_rgba(0,0,0,0.55)] ring-1 ring-black/5 dark:ring-white/10"
                          ...
                          >
                            <CoverThumb
                              ...
                              onRetry={() => handleRetryPoster(item.kind, item.id)}
                              labels={{
                          ...
                          <span className="absolute -bottom-1.5 -right-1.5 w-6 h-6 rounded-full flex items-center justify-center bg-[var(--color-surface)] shadow-md ring-1 ring-black/5 dark:ring-white/10 text-[var(--klein-blue)] [&_svg]:w-3 [&_svg]:h-3">
                            <TypeGlyph aria-hidden />
                          </span>
                        </div>
                        <div className="flex flex-col min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            ...
                            <span className="text-xs font-medium text-[var(--color-text-tertiary)] truncate">
                              {item.counts}
                            </span>
                          </div>
                        </div>
                        {/* Add — solid soft chip (was a low-contrast glass circle
                            that read as an empty grey dot on the row surface). */}
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center bg-[var(--klein-blue-soft)] text-[var(--klein-blue)] shadow-sm transition-all duration-300 hover:bg-[var(--klein-blue)] hover:text-[var(--klein-on)] hover:scale-110 hover:rotate-90 active:scale-95"
