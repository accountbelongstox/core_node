/**
 * WfLibraryWordCell - adapt a library word (+ optional on-demand resolved
...
export function mapWordMediaVariants(resolved: WfNewWordMedia | undefined): WfAudioFileVariant[] {
  const files = resolved?.audioFiles ?? [];
  if (files.length) return files;
  const acc = resolved?.audioVariants ?? [];
  return acc
    .filter((v) => v && v.url)
    .map((v): WfAudioFileVariant => ({
      variantKey: v.accent,
