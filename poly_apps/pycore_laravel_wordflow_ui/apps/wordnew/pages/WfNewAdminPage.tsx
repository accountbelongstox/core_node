...
const ADMIN_TAB_KEY = 'wfnew_admin_tab';
...
export const WfNewAdminPage: React.FC<WfNewAdminPageProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<WfNewAdminTab>(() => {
    try {
      const saved = localStorage.getItem(ADMIN_TAB_KEY) as WfNewAdminTab | null;
  ...
  const switchTab = (key: WfNewAdminTab): void => {
    setActiveTab(key);
    try { localStorage.setItem(ADMIN_TAB_KEY, key); } catch { /* ignore */ }
  };
