...
function readStoredLang(): string {
  try {
    return localStorage.getItem(ADMIN_LANG_KEY) || FALLBACK_LANGS[0];
  } catch {
...
interface WfNewAdminWordEditorProps {
  mode: 'create' | 'edit';
...
/**
 * Word create/edit form rendered through the shared overlay framework
 * (Portal + OVERLAY_Z scale, same pattern as WfNewApiServerDialog). Mounted
 * fresh per open, so field state initializes straight from the row.
 */
const WfNewAdminWordEditor: React.FC<WfNewAdminWordEditorProps> = ({
