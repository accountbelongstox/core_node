 ...
 * live endpoints via the wordflow service centers — the UI never blocks on or
 * fails because of the network.
 *
 * Reused live stack (best-effort, never load-bearing):
 *   - wfProgressCenter.getBlob / reportAnswers  → POST /group/get_progress_blob,
 *     /group/update_progress
 *   - wfRecitationCenter.recordAction           → POST /recitation/log
...
import { wfProgressCenter, wfRecitationCenter } from '../../services';
import type { WfGroupProgressBlob } from '../../api-libs/wordflow/WordflowApi';
import { expandProgressEntry } from '../../api-libs/wordflow/WordflowApi';
...
const STORAGE_KEY = 'wfnew_study_progress_v1';
...
class WfNewStudyProgressClass {
  ...
  recordSeen(gid: string, word: Word, language?: string, playTimeSeconds?: number): void {
    ...
    if (wfNewApi.isAuthenticated()) {
      try {
        wfRecitationCenter.recordAction(word.text, 'read', language);
      } catch {
   ...
   */
  ingestBlob(gid: string, blob: WfGroupProgressBlob): void {
  ...
  private mirror(gid: string, word: Word, known: boolean, language?: string): void {
    ...
    try {
      wfRecitationCenter.recordAction(
        word.text,
