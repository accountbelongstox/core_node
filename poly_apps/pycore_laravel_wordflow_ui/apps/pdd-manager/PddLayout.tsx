/**
 * pdd-manager layout: a left icon sidebar (driven by the page registry) + the
 * active page via <Outlet/>. Mirrors apps/pycore-manager/PcLayout.tsx; works
 * light + dark. Nexus theme surfaces (this end uses the Nexus theme).
 */
...
export const PddLayout: React.FC = () => {
  const { t } = useTranslation('pdd');
  const top = PDD_PAGES.filter((p) => !p.bottom);
  const bottom = PDD_PAGES.filter((p) => p.bottom);

  const renderLink = (p: typeof PDD_PAGES[number]) => {
    const Icon = p.Icon;
    return (
      <NavLink
        key={p.id}
  ...
  return (
    <div className="fixed inset-0 flex overflow-hidden" data-end="pdd-manager">
