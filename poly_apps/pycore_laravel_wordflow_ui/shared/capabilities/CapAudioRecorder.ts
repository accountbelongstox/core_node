 ...
 *
 * QUICK START
 *   import { capRecorder, useAudioRecorder } from '@/shared/capabilities/CapAudioRecorder';
  ...
  emit<K extends keyof M>(e: K, p: M[K]): void {
    ...
    for (const fn of Array.from(s)) {
      try {
        ...
        console.error('[CapAudioRecorder] listener error', err);
      }
    }
  }
  clear(): void {
...
export class CapAudioRecorderService {
  private readonly emitter = new Emitter<CapRecorderEventMap>();
