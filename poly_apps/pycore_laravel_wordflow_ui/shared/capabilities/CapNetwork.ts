 ...
 *   poly_apps/pycore_laravel_wordflow_ui may import and use it. It is, however,
...
class Emitter<M> {
  ...
  once<K extends keyof M>(event: K, fn: (p: M[K]) => void): () => void {
  ...
  off<K extends keyof M>(event: K, fn: (p: M[K]) => void): void {
    this.map.get(event)?.delete(fn as (p: any) => void);
  }
  ...
  emit<K extends keyof M>(event: K, payload: M[K]): void {
    ...
    for (const fn of Array.from(set)) {
      try {
        ...
        // Never let a faulty listener break the emitter loop.
        // eslint-disable-next-line no-console
        console.error('[CapNetwork] listener error', err);
      }
    }
  }
...
export class CapNetworkService {
  ...
  private readonly emitter = new Emitter<CapNetworkEventMap>();
