  ...
  const fetchStatus = async () => {
    ...
    try {
      ...
      if (s && (s as any).success !== false && Array.isArray((s as any).engines)) {
        ...
        setErr((s as any)?.error || (s as any)?.detail || 'TTS engine status unavailable');
      }
    } catch (e: any) {
      if (mounted.current) setErr(e?.message || 'TTS engine status unavailable');
    } finally {
...
/**
 * VocabPosterStrip — Movie/TV poster pipeline management card, modelled on the
 * Vocabulary Libraries cover-status UI (VocabularyCoverManagerMenu + the per-
 * library "Retry cover" block). Shows the TMDB/OMDB provider key badges + per-
 * type poster counts (books / subtitles: pending / ready / failed / none) and a
 * "Fetch pending" control that hits the laravel media-poster endpoints
 * (api.appQyV1.getPosterStatus / fetchPoster). Polls every ~10s; degrades to a
 * muted, fully-usable line if the backend is offline.
 *
 * 404 hardening: BaseAPI's get() resolves a non-2xx 404 body into the response
 * envelope rather than throwing, so a stale backend that lacks the route yields
 * `{success:false}` OR a 200 body missing `counts`. We treat EITHER as
 * "endpoint missing" and show a clear "restart backend" notice instead of
...
const VocabPosterStrip: React.FC = () => {
  ...
  const [data, setData] = useState<PosterStatusData | null>(null);
  const [loading, setLoading] = useState(true);
  const [fetching, setFetching] = useState(false);
  // null = not loaded; '' = loaded OK; non-empty = error / endpoint-missing.
  const [err, setErr] = useState<string | null>(null);
  const mounted = useRef(true);
  useEffect(() => () => { mounted.current = false; }, []);
  ...
  const fetchStatus = async () => {
    ...
    try {
      ...
      if (!mounted.current) return;
      // Shape-check: a real snapshot carries a `counts` object with book/subtitle.
      // A stale-backend 404 resolves to {success:false} (or a body without counts).
      if (res.success && res.data && (res.data as any).counts && Array.isArray((res.data as any).providers)) {
        setData(res.data); setErr('');
      } else {
        setData(null);
        setErr('Poster status endpoint missing — restart the laravel backend.');
      }
    } catch (e: any) {
      if (mounted.current) { setData(null); setErr(e?.message || 'Poster status unavailable'); }
    } finally {
      if (mounted.current) setLoading(false);
    }
  };
