 ...
 * and ALSO pushes per-engine deltas over the rpc_v2 WS bus as the
 * 'engine_load_status_update' event. This store multiplexes BOTH: it subscribes to
 * the WS event for instant state transitions, and it fast-polls the endpoint ONLY
 ...
 * the WS delta does not carry between transitions).
 *
...
import { subscribe } from './PycoreWs';
import type { EngineLoadStatusEntry, EngineLoadState } from './pycoreTypes';

export const PYCORE_ENGINE_LOAD_EVENT = 'pycore-engine-load-changed';
/** Backend rpc_v2 broadcast event name (model_load_status.BROADCAST_EVENT). */
const WS_EVENT = 'engine_load_status_update';
/** Fast poll while a load is relevant — streams the server log tail + elapsed. */
const POLL_MS = 1500;
...
let subscribers = 0;         // mounted hooks (drives WS subscription lifetime)
...
function onWsUpdate(data: any): void {
...
function retain(): void {
  subscribers += 1;
  if (subscribers === 1) {
    wsOff = subscribe(WS_EVENT, onWsUpdate);
