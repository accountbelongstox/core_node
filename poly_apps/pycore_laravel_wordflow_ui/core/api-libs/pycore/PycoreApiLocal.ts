...
export const pycoreApiLocal = {
  ...
  queueTranslation: (refresh = false) =>
    callRpc(PYCORE_RPC_ROUTES.translationQueue, { action: 'snapshot', refresh: refresh ? 1 : 0 }),
  setQueuePriority: (task_id: string, priority: number) =>
    callRpc(PYCORE_RPC_ROUTES.translationQueue, { action: 'priority', task_id, priority }),
  stackQueue: (words: string[], language: string, target_language: string, priority?: number) =>
    callRpc(PYCORE_RPC_ROUTES.translationQueue,
      { action: 'stack', words, language, target_language, ...(priority != null ? { priority } : {}) }),
  ...
  getTranslationTaskDetail: (taskId: string) =>
    callRpc(PYCORE_RPC_ROUTES.translationQueue, { action: 'task', task_id: taskId }),
  ...
  setAssistConfig: (config: AssistConfigPatch) =>
    callRpc(PYCORE_RPC_ROUTES.assist, { action: 'config', ...config }),
  runAssistCycle: () =>
    callRpc(PYCORE_RPC_ROUTES.assist, { action: 'cycle' }),
  ...
  getImageSearchStatus: () => callRpc(PYCORE_RPC_ROUTES.imageSearch, { action: 'status' }),
  searchImages: (query: string, num = 12, country?: string, record = true) =>
    callRpc(PYCORE_RPC_ROUTES.imageSearch, { action: 'search', query, num, country, record }),
  ...
  deleteImageSearchHistory: (id: string) =>
    callRpc(PYCORE_RPC_ROUTES.imageSearch, { action: 'delete', id }),
  clearImageSearchHistory: () =>
    callRpc(PYCORE_RPC_ROUTES.imageSearch, { action: 'clear' }),
  ...
  getSentenceAudioAutoStatus: () =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'sentence_status' }),
  setSentenceAudioAutoConfig: (autoStart: boolean) =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'sentence_config', auto_start: autoStart }),
  ...
  setSentenceAudioConcurrency: (concurrency: number, autoStart: boolean) =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'sentence_config', auto_start: autoStart, concurrency }),
  runSentenceAudioOnce: () =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'sentence_run_once' }),
  ...
  getQueueBumps: (limit = 30) =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'bumps', limit }),

  getTaskCapabilityChains: () =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'chains' }),
  saveTaskCapabilityChain: (taskType: string, priority: string[]) =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'chain_update', task_type: taskType, priority }),
  ...
  getWordTtsAutoStatus: () =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'word_tts_status' }),
  setWordTtsAutoConfig: (autoStart: boolean) =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'word_tts_config', auto_start: autoStart }),
  // Backend config model requires auto_start, so the current value goes along.
  setWordTtsConcurrency: (concurrency: number, autoStart: boolean) =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'word_tts_config', auto_start: autoStart, concurrency }),
  runWordTtsOnce: () =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'word_tts_run_once' }),

  // --- Heartbeat workers overview (Queue Center worker strip) ------------- #
  getHeartbeatWorkersStatus: () =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'heartbeat_status' }),
  setHeartbeatWorkerConfig: (callbackName: string, enabled: boolean) =>
    callRpc(PYCORE_RPC_ROUTES.workers, { action: 'heartbeat_config', callback_name: callbackName, enabled }),
  getTaskCenter: () =>
  ...
  getDictionaryStatus: () =>
    callRpc(PYCORE_RPC_ROUTES.dictionary, { action: 'status' }),
  getDictionaryLookup: (word: string, target = 'zh') =>
    callRpc(PYCORE_RPC_ROUTES.dictionary, { action: 'lookup', word, target }),
  ...
  getVocabTranslationLanguages: () =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'translation_languages' }),
  translateVocab: (payload: VocabTranslateRequest) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'translation_translate', body: payload }),
  queueVocabTranslationBatch: (payload: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'translation_queue_add', body: payload }),
  generateVocabTts: (payload: VocabTtsGenerateRequest) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'tts_generate', body: payload }),
  queueVocabTtsBatchQuery: (items: Record<string, unknown>[]) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'tts_queue_query', body: items }),
  getVocabSentenceAudio: (params: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'tts_sentence_audio', query: params }),
  getVocabTtsQueueStats: () =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'tts_queue_stats' }),
  getVocabTtsQueueItems: (params: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'tts_queue_items', query: params }),
  getVocabAssistOverview: () =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'assist_overview' }),
  getVocabAssistOverviewItems: (params: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'assist_overview_items', query: params }),
  ...
  deleteVocabLibrary: (libraryId: number) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'library_delete', library_id: libraryId }),
  getVocabStatistics: (params: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'statistics', query: params }),
  getVocabLanguageBreakdown: (params: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'language_breakdown', query: params }),
  ...
  deleteVocabDictionaryWord: (md5: string, params: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'dictionary_word_delete', md5, query: params }),
  batchVocabDictionaryWords: (payload: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'dictionary_words_batch', body: payload }),
  getVocabDictionarySentences: (params: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'dictionary_sentences', query: params }),
  reportVocabValidity: (payload: Record<string, unknown>) =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'validity_report', body: payload }),
  getVocabStorageSummary: () =>
    callRpc(PYCORE_RPC_ROUTES.vocabulary, { action: 'storage_summary' }),
