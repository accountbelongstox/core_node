...
export const pycoreApiAi = {
  ...
  getAiCatalog: () => callRpc(PYCORE_RPC_ROUTES.aiCatalog, {}, 15_000),
  ...
  probeAi: (refresh = false) =>
    callRpc(PYCORE_RPC_ROUTES.aiProbe, { refresh: refresh ? 1 : 0 }),

  // Test ONE provider (per-card "Test"): live, never cached, rate-aware.
  probeAiOne: (provider: string) =>
    callRpc(PYCORE_RPC_ROUTES.aiProbe, { provider }),
  ...
  getAiBalances: () => callRpc(PYCORE_RPC_ROUTES.aiBalance, {}),
  getAiBalanceOne: (provider: string) =>
    callRpc(PYCORE_RPC_ROUTES.aiBalance, { provider }),
  ...
  getAiRateLimits: () =>
    callRpc(PYCORE_RPC_ROUTES.aiRateLimits, {}),
  ...
  getAiKeys: () => callRpc(PYCORE_RPC_ROUTES.aiKeys, { action: 'list' }),
  ...
  setAiKey: (body: AiKeySetRequest) =>
    callRpc(PYCORE_RPC_ROUTES.aiKeys, { action: 'set', ...body }),
  ...
  deleteAiKey: (keyName: string) =>
    callRpc(PYCORE_RPC_ROUTES.aiKeys, { action: 'delete', key_name: keyName }),
  ...
  resetKeyCooldown: (req: { provider: string; index?: number; image?: boolean }) =>
    callRpc(PYCORE_RPC_ROUTES.aiKeys, { action: 'reset_cooldown', ...req }),
  ...
  getAiUsage: async (limit = 150): Promise<{ success: boolean; data: AiUsageResponse | null; error: string | null }> => {
    try {
      const r = await callRpc(PYCORE_RPC_ROUTES.aiUsage, { limit });
  ...
  generateImage: (req: { prompt: string; size?: string; model?: string; provider?: string; source?: string }) =>
    callRpc(PYCORE_RPC_ROUTES.aiImage, { action: 'generate', ...req }),
  ...
  testImageProvider: (req: { provider: string; prompt?: string; size?: string; model?: string }) =>
    callRpc(PYCORE_RPC_ROUTES.aiImage, { action: 'test', ...req }),
  ...
  getImageHistory: (limit = 50) =>
    callRpc(PYCORE_RPC_ROUTES.aiImage, { action: 'history', limit }),
  ...
  deleteImageHistory: (id: string) =>
    callRpc(PYCORE_RPC_ROUTES.aiImage, { action: 'delete', id }),
  clearImageHistory: () =>
    callRpc(PYCORE_RPC_ROUTES.aiImage, { action: 'clear' }),
  /** Reveal a generated image's folder in the OS file manager (path resolved by id). */
  revealImage: (id: string) =>
    callRpc(PYCORE_RPC_ROUTES.aiImage, { action: 'reveal', id }),
