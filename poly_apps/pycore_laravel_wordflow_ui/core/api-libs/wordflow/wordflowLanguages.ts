/**
 * wordflowLanguages — the wordflow end's learning-language catalog.
 *
 ...
 *
 * Used in two places:
 *   1. wordflowAuthMock serves this list verbatim for /system/supported-languages
 *      when WF_AUTH_USE_MOCK is on (so the register UI works fully offline).
 *   2. WfAuthLoginPage uses it as the fallback option set when the live endpoint
 *      returns nothing.
 *
...
export interface WfSupportedLanguage {
  code: string;
  name: string;
  native_name: string;
  voice_id?: string;
...
export const WF_SUPPORTED_LANGUAGES: WfSupportedLanguage[] = [
...
export const WF_SUPPORTED_LANGUAGE_CODES: ReadonlySet<string> = new Set(
  WF_SUPPORTED_LANGUAGES.map((l) => l.code)
);
...
export function filterSupportedLanguageCodes(codes: unknown): string[] {
  ...
  for (const c of codes) {
    if (typeof c === 'string' && WF_SUPPORTED_LANGUAGE_CODES.has(c) && !seen.has(c)) {
      seen.add(c);
      out.push(c);
    }
  }
