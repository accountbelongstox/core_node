...
const WS_INTERNAL_HEADER = 'x-ws-internal-key'

const logAdminAuthSuccess = (username, duration) => {
  const message = `🔐 Admin authenticated: ${username} in ${duration}ms`
  if (config.logging?.security?.logAdminSuccess) {
...
const logUserAuthSuccess = (username, userId, duration) => {
  const message = `👤 User authenticated: ${username} (${userId}) in ${duration}ms`
  if (config.logging?.security?.logUserSuccess) {
    logger.info(message)
  } else {
    logger.debug(message)
  }
...
function isLoopbackAddress(address) {
  ...
  if (address.startsWith('::ffff:')) {
    return address.endsWith('127.0.0.1')
  }
  return address.startsWith('127.')
}

function isWsInternalRequest(req) {
  const internalKey =
    req.headers?.[WS_INTERNAL_HEADER] || req.headers?.[WS_INTERNAL_HEADER.toLowerCase()]
  const apiKey = req.headers?.['x-api-key']
  const key = internalKey || apiKey
  if (!key) {
    return false
  }
  if (config.websocketClient?.enabled !== true) {
    return false
  }
  const expected = config.websocketClient?.clientApiKey || ''
...
const authenticateApiKey = async (req, res, next) => {
  ...
  try {
    ...
    if (!validation.valid && validation.error === 'API key not found') {
      const remoteAddress = req.socket?.remoteAddress || req.connection?.remoteAddress || ''
      const isInternalWsKey =
        apiKey === config.websocketClient?.clientApiKey && isLoopbackAddress(remoteAddress)
      if (isInternalWsKey) {
        const repaired = await apiKeyService.repairApiKeyHashMap(apiKey)
        if (!repaired) {
          await apiKeyService.ensureInternalApiKey(apiKey)
        }
...
const authenticateAdmin = async (req, res, next) => {
  ...
  try {
    if (isWsInternalRequest(req)) {
      req.user = { username: 'ws-internal', role: 'system' }
      return next()
    }
    ...
    const authDuration = Date.now() - startTime
    logAdminAuthSuccess(adminSession.username, authDuration)
...
const authenticateUser = async (req, res, next) => {
  ...
  try {
    ...
    const authDuration = Date.now() - startTime
    logUserAuthSuccess(user.username, user.id, authDuration)
...
const requestLogConfig = (() => {
  ...
  const sampleRate = Number.isFinite(requestConfig.sampleRate) ? requestConfig.sampleRate : 0.01
  const slowThresholdMs = Number.isFinite(requestConfig.slowThresholdMs)
    ? requestConfig.slowThresholdMs
    : 10000
  const skipExactPaths = new Set(
    Array.isArray(requestConfig.skipExactPaths) ? requestConfig.skipExactPaths : []
  )
  const skipErrorPaths = new Set(
    Array.isArray(requestConfig.skipErrorPaths) ? requestConfig.skipErrorPaths : []
  )
  const skipPrefixPaths = Array.isArray(requestConfig.skipPrefixPaths)
    ? requestConfig.skipPrefixPaths
    : []
  const skipMethodRules = Object.entries(requestConfig.skipMethodsForPrefix || {}).map(
    ([prefix, methods]) => ({
      prefix,
      methods: new Set(
        (Array.isArray(methods) ? methods : [methods]).map((method) => String(method).toUpperCase())
      )
    })
  )

  return {
    sampleRate: Math.min(Math.max(sampleRate, 0), 1),
    slowThresholdMs,
    logApiKey: requestConfig.logApiKey === true,
    skipExactPaths,
    skipErrorPaths,
    skipPrefixPaths,
    skipMethodRules
...
const shouldSampleRequest = (sampleRate) => {
...
const shouldSkipRequestLog = (req) => {
  ...
  if (requestLogConfig.skipExactPaths.has(path)) {
    return true
  }

  for (const prefix of requestLogConfig.skipPrefixPaths) {
  ...
  for (const rule of requestLogConfig.skipMethodRules) {
    if (!rule.prefix) {
      continue
    }
...
const requestLogger = (req, res, next) => {
  ...
  const isSkipPath = shouldSkipRequestLog(req)
  const isSampled = shouldSampleRequest(requestLogConfig.sampleRate)

  res.on('finish', () => {
    const duration = Date.now() - start
    const contentLength = res.get('Content-Length') || '0'
    const isSlow = duration >= requestLogConfig.slowThresholdMs
    const isError = res.statusCode >= 400
    const shouldSkipError = isError && requestLogConfig.skipErrorPaths.has(req.path || '')
    const shouldLog = (isError && !shouldSkipError) || isSlow || (!isSkipPath && isSampled)
    ...
    if (res.statusCode >= 500) {
      ...
      logger.warn(
        `🐌 [${requestId}] ${req.method} ${req.originalUrl} | ${res.statusCode} | ${duration}ms | ${contentLength}B`,
        { ...logMetadata, slow: true }
      )
    ...
    // API Key相关日志
    if (req.apiKey && shouldLog && requestLogConfig.logApiKey) {
      logger.api(
