...
class UnifiedClaudeScheduler {
    ...
    try {
      ...
      if (apiKeyData.claudeAccountId) {
        ...
        if (apiKeyData.claudeAccountId.startsWith('group:')) {
          ...
          const allowCcrInGroup = vendor === 'ccr' || ccrPoolMode !== 'disabled'
      ...
      if (!includeCcrInPool && ccrPoolMode === 'fallback' && availableAccounts.length === 0) {
        logger.info('🔁 No standard accounts available, including CCR pool as fallback')
  ...
  _getCcrPoolMode() {
    const appConfig = require('../../config/config')
    const mode = String(appConfig?.ccr?.poolMode || 'fallback').toLowerCase()
