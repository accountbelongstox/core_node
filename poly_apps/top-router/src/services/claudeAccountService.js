...
class ClaudeAccountService {
  ...
  _shouldRetryNetworkError(error) {
    ...
    const code = this._getNetworkErrorCode(error)
    return ['ECONNRESET', 'ETIMEDOUT', 'ECONNREFUSED', 'ENOTFOUND', 'EAI_AGAIN', 'ECONNABORTED'].includes(
      code
    )
  }

  async _requestWithRetry(requestFn, options = {}) {
    const { retries = 2, baseDelayMs = 500, label = 'request' } = options
    let attempt = 0
    ...
    while (true) {
      try {
        ...
        const errorCode = this._getNetworkErrorCode(error)
        if (errorCode === 'ECONNRESET') {
          logger.warn(`⚠️ ${label} ECONNRESET, attempt ${attempt + 1}/${retries + 1}`)
        }

        if (!this._shouldRetryNetworkError(error) || attempt >= retries) {
          throw error
        }

        const delayMs = baseDelayMs * Math.pow(2, attempt)
        logger.warn(`⏳ ${label} retry ${attempt + 1}/${retries} in ${delayMs}ms`, {
          code: errorCode
        })
  ...
  async fetchOAuthUsage(accountId, accessToken = null, agent = null) {
    try {
      ...
      const response = await this._requestWithRetry(
        () => axios.get('https://api.anthropic.com/api/oauth/usage', axiosConfig),
        {
          retries: 2,
          baseDelayMs: 1000,
          label: `OAuth usage request for account ${accountId}`
        }
      )
      ...
      const errorCode = this._getNetworkErrorCode(error)
      if (errorCode === 'ECONNRESET') {
        logger.warn(`⚠️ OAuth usage request ECONNRESET for account ${accountId}`)
        return null
      }
  ...
  async fetchAndUpdateAccountProfile(accountId, accessToken = null, agent = null) {
    try {
      ...
      const response = await this._requestWithRetry(
        () => axios.get('https://api.anthropic.com/api/oauth/profile', axiosConfig),
        {
          retries: 2,
          baseDelayMs: 1000,
          label: `OAuth profile request for account ${accountId}`
        }
      )
      ...
      } else if (this._getNetworkErrorCode(error) === 'ECONNRESET') {
        logger.warn(`⚠️ Profile request ECONNRESET for account ${accountId}`)
      } else {
