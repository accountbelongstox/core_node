...
class ApiKeyService {
  ...
  // 🧩 修复 API Key 哈希映射（用于内部 WS key 丢失映射的场景）
  async repairApiKeyHashMap(apiKey) {
    try {
      if (!apiKey || !apiKey.startsWith(this.prefix)) {
        return null
      }
      ...
      for (const keyId of keyIds) {
        ...
        if (keyData.apiKey === hashedKey) {
          await redis.setApiKey(keyId, keyData, hashedKey)
          logger.warn(`⚠️ Repaired API key hash map for key ${keyId}`)
          return { id: keyId, ...keyData }
        }
      }
    } catch (error) {
      logger.error('❌ Failed to repair API key hash map:', error)
    }
    return null
  }

  // 🧩 为 WS 内部调用确保 API Key 存在（仅在映射缺失时创建最小记录）
  async ensureInternalApiKey(apiKey) {
    try {
      if (!apiKey || !apiKey.startsWith(this.prefix)) {
        return null
      }
      const hashedKey = this._hashApiKey(apiKey)
      const existing = await redis.findApiKeyByHash(hashedKey)
      if (existing) {
        return existing
      }
      ...
      const keyData = {
        id: keyId,
        name: 'ws-internal',
        description: 'Auto-provisioned for WS client relay',
        apiKey: hashedKey,
        tokenLimit: '0',
        concurrencyLimit: '0',
        rateLimitWindow: '0',
      ...
      await redis.setApiKey(keyId, keyData, hashedKey)
      logger.warn(`⚠️ Auto-provisioned WS internal API key: ${keyId}`)
      return { id: keyId, ...keyData }
    } catch (error) {
      logger.error('❌ Failed to ensure internal API key:', error)
    }
    return null
  }
