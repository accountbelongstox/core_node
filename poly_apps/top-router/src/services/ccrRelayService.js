...
const normalizeCcrApiBaseUrl = (apiUrl) => {
  ...
  return withoutTrailing.replace(/\/v1\/messages$/, '').replace(/\/v1$/, '')
}

const buildCcrApiEndpoint = (apiUrl, customPath = '') => {
  const baseUrl = normalizeCcrApiBaseUrl(apiUrl)
  if (!baseUrl) {
    return ''
  }
  if (customPath) {
...
class CcrRelayService {
  ...
  async relayRequest(
    ...
    try {
      ...
      // 构建完整的API URL
      const apiEndpoint = buildCcrApiEndpoint(account.apiUrl, options.customPath)
      if (!apiEndpoint) {
        throw new Error('CCR account API URL is invalid')
      }
  ...
  async _makeCcrStreamRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      // 构建完整的API URL
      const apiEndpoint = buildCcrApiEndpoint(account.apiUrl)
      if (!apiEndpoint) {
        throw new Error('CCR account API URL is invalid')
      }
