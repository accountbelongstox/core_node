...
class ClaudeRelayService {
  ...
  _attachUpstreamSocketHandlers(req, accountId, contextLabel) {
    if (!req || typeof req.on !== 'function') {
      return
    }

    // 处理代理 TLS 握手阶段的 socket 错误，避免未捕获异常导致进程退出
    req.on('socket', (socket) => {
      if (!socket || socket.__claudeSocketErrorHandlerAttached) {
        return
      }

      socket.__claudeSocketErrorHandlerAttached = true
      socket.on('error', (socketError) => {
        logger.warn(`⚠️ Claude upstream socket error${contextLabel ? ` (${contextLabel})` : ''}`, {
          accountId,
          message: socketError?.message,
          code: socketError?.code,
          host: socketError?.host,
          port: socketError?.port
        })
        if (!req.destroyed) {
          req.destroy(socketError)
        }
      })
    })
  }
  ...
  async _makeClaudeRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      this._attachUpstreamSocketHandlers(req, accountId, 'non-stream')
  ...
  async _makeClaudeStreamRequestWithUsageCapture(
    ...
    return new Promise((resolve, reject) => {
      ...
      this._attachUpstreamSocketHandlers(req, accountId, 'stream')
