...
class CostRankService {
  ...
  async _atomicUpdateIndex(client, rankKey, costs) {
    ...
    try {
      ...
      if (typeof client.rename === 'function') {
        ...
        // SQLite 兼容：缺少 rename，直接删除并重建目标
        await client.del(rankKey)
        const membersPlain = []
        costs.forEach((cost, keyId) => {
          membersPlain.push(cost, keyId)
        })
        if (membersPlain.length > 0) {
          await client.zadd(rankKey, ...membersPlain)
        }
      }
    } catch (error) {
      // 清理临时 key
  ...
  async _calculateBatchCosts(keyIds, dateRange) {
    ...
    const supportsPipelineGet =
      client && typeof client.pipeline === 'function' && typeof client.pipeline().get === 'function'

    if (dateRange.useTotal) {
      // 'all' 时间范围：直接读取 total cost
      if (supportsPipelineGet) {
        const pipeline = client.pipeline()
        keyIds.forEach((keyId) => {
        ...
        for (const keyId of keyIds) {
          const value = await client.get(RedisKeys.totalCost(keyId))
          costs.set(keyId, parseFloat(value || 0))
        }
      }
    } else {
      // 特定日期范围：汇总每日费用
      ...
      if (supportsPipelineGet) {
        ...
        for (const keyId of keyIds) {
          ...
          for (const date of dates) {
            const value = await client.get(RedisKeys.dailyCost(keyId, date))
            if (value) {
              totalCost += parseFloat(value)
            }
          }
