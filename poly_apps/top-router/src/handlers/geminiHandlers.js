...
function normalizeGeminiModel(model) {
  ...
  return vendor === 'ccr' ? baseModel : model
}

function isApiAccountType(accountType) {
  return accountType === 'gemini-api' || accountType === 'ccr'
}

async function getAccountByType(accountId, accountType) {
  if (accountType === 'ccr') {
    return await ccrAccountService.getAccount(accountId)
  }
  if (accountType === 'gemini-api') {
    return await geminiApiAccountService.getAccount(accountId)
  }
  return await geminiAccountService.getAccount(accountId)
}

async function markAccountUsedByType(accountId, accountType) {
  if (accountType === 'ccr') {
...
function getApiBaseUrl(account, accountType) {
  return accountType === 'ccr' ? account.apiUrl : account.baseUrl
}

function mapCcrModel(accountType, account, requestedModel) {
  if (
    accountType === 'ccr' &&
    account &&
...
async function maybeRelayViaClient({
  accountType,
  accountId,
  req,
  res,
  ...
  const targetModel = model || req.body?.model || 'gemini-2.5-flash'
  if (isStream) {
    await clientRelayService.relayStreamRequestWithUsageCapture(
      req,
      res,
      accountId,
      sessionHash,
    ...
    await clientRelayService.relayNonStreamRequest(
      req,
      res,
      accountId,
      sessionHash,
...
async function handleMessages(req, res) {
  ...
  try {
    ...
    if (translationOptions) {
      try {
        const translated = await translationService.translateMessages(messages, {
          ...translationOptions,
          keyId: apiKeyData?.id
        })
        messages = translated
        req.body.messages = translated
      } catch (error) {
    ...
    const isApiAccount = isApiAccountType(accountType)

    // 获取账户详情
    const account = await getAccountByType(accountId, accountType)
    if (!account) {
      const label = accountType === 'ccr' ? 'CCR' : isApiAccount ? 'Gemini API' : 'Gemini OAuth'
      return res.status(503).json({
        error: {
    ...
    logger.info(
      `Using ${accountType === 'ccr' ? 'CCR' : isApiAccount ? 'Gemini API' : 'Gemini OAuth'} account: ${account.id} for API key: ${apiKeyData.id}`
    )
    await markAccountUsedByType(account.id, accountType)
    ...
    if (isApiAccount) {
      ...
      const apiBaseUrl = getApiBaseUrl(account, accountType)
      const upstreamModel = mapCcrModel(accountType, account, model)
...
async function handleModels(req, res) {
  try {
    ...
    try {
      ...
      isApiAccount = isApiAccountType(accountType)
      if (
        await maybeRelayViaClient({
          accountType: accountSelection.accountType,
      ...
      account = await getAccountByType(accountSelection.accountId, accountType)
    } catch (error) {
    ...
    if (isApiAccount) {
      ...
      try {
        const apiUrl = buildGeminiApiUrl(
          getApiBaseUrl(account, accountType),
          null,
          null,
          account.apiKey,
          {
...
function handleSimpleEndpoint(apiMethod) {
  return async (req, res) => {
    try {
      ...
      // 从路径参数或请求体中获取模型名
      let requestedModel = req.body.model || req.params.modelName || 'gemini-2.5-flash'
      ...
      if (accountType === 'gemini-api' || accountType === 'ccr') {
        logger.error(
          `❌ v1internal routes do not support ${accountType === 'ccr' ? 'CCR' : 'Gemini API'} accounts. Account: ${accountId}`
        )
        return res.status(400).json({
          error: {
            message:
              'This endpoint only supports Gemini OAuth accounts. Gemini API Key/CCR accounts are not compatible with v1internal format.',
...
async function handleCountTokens(req, res) {
  try {
    ...
    // 从路径参数或请求体中获取模型名
    let model = requestData.model || req.params.modelName || 'gemini-2.5-flash'
    model = normalizeGeminiModel(model)
    const sessionHash = sessionHelper.generateSessionHash(req.body)
    ...
    if (!account) {
      return res.status(404).json({
        error: {
          message: `${accountType === 'ccr' ? 'CCR' : isApiAccount ? 'Gemini API' : 'Gemini'} account not found`,
          type: 'account_not_found'
        }
      })
    }
    ...
    if (isApiAccount) {
      ...
      const modelName = upstreamModel.startsWith('models/')
        ? upstreamModel.replace('models/', '')
        : upstreamModel
      const apiUrl = buildGeminiApiUrl(apiBaseUrl, modelName, 'countTokens', account.apiKey)
...
async function handleGenerateContent(req, res) {
  try {
    ...
    // 从路径参数或请求体中获取模型名
    let model = req.body.model || req.params.modelName || 'gemini-2.5-flash'
    model = normalizeGeminiModel(model)
    ...
    if (Array.isArray(req.body.messages)) {
      ...
      if (translationOptions) {
        try {
          req.body.messages = await translationService.translateMessages(req.body.messages, {
            ...translationOptions,
            keyId: req.apiKey?.id
          })
        } catch (error) {
...
async function handleStandardGenerateContent(req, res) {
  ...
  try {
    ...
    // 从路径参数中获取模型名
    let model = req.params.modelName || 'gemini-2.0-flash-exp'
    model = normalizeGeminiModel(model)
    ...
    account = await getAccountByType(actualAccountId, accountType)
    if (!account) {
      return res.status(404).json({
        error: {
          message: `${accountType === 'ccr' ? 'CCR' : isApiAccount ? 'Gemini API' : 'Gemini'} account not found`,
    ...
    if (isApiAccount) {
      ...
      const modelName = upstreamModel.startsWith('models/')
        ? upstreamModel.replace('models/', '')
        : upstreamModel
      const apiUrl = buildGeminiApiUrl(apiBaseUrl, modelName, 'generateContent', account.apiKey)
