...
const openaiOfficialRoutes = require('./routes/openaiOfficialRoutes')
...
class Application {
  ...
  async initialize() {
    try {
      // 🔗 连接数据存储（Redis/SQLite/MySQL）
      const datastoreDriver = redis.driver || (redis.getClient?.().driver) || 'redis'
      const datastoreLabel =
        datastoreDriver === 'sqlite'
          ? 'SQLite datastore'
          : datastoreDriver === 'mysql'
            ? 'MySQL datastore (redis fallback for compatibility)'
            : 'Redis datastore'
      logger.info(`🔄 Connecting to ${datastoreLabel}...`)
      await redis.connect()
      logger.success(`✅ ${datastoreLabel} connected successfully`)
      ...
      const enableAdminNextDebug = process.env.DEBUG_ADMIN_NEXT === 'true'

      // 调试中间件 - 拦截所有 /admin-next 请求
      if (enableAdminNextDebug) {
        this.app.use((req, res, next) => {
      ...
      if (fs.existsSync(adminSpaPath)) {
        ...
        this.app.all('/admin-next/', (req, res) => {
          if (enableAdminNextDebug) {
            logger.debug('🎯 HIT: /admin-next/ route handler triggered!')
            logger.debug(`Method: ${req.method}, Path: ${req.path}, URL: ${req.url}`)
          }
      ...
      this.app.use('/admin', adminClientsRoutes)
      ...
      // 官方 OpenAI 兼容入口（不经 Codex 改写，单 Key 简版）
      this.app.use('/openai/official', openaiOfficialRoutes)
  ...
  async initializeAdmin() {
    try {
      ...
      await redis.setSession('admin_credentials', adminCredentials, null)
  ...
  async checkRedisHealth() {
    const driver = redis.driver || (redis.getClient?.().driver) || 'redis'

    // SQLite: 直接返回连接状态，无 ping
    if (driver === 'sqlite') {
      return {
  ...
  async initializeRealtimeServices() {
    ...
    if (wsMode === 'both') {
      ...
      logger.warn(`⚠️ WS_MODE=both is not supported; using mode=${wsMode}`)
    }

    const wsServerEnabled = wsMode === 'server' && wantsServer
    const wsClientEnabled = wsMode === 'client' && wantsClient

    let wsServerInstance = null
    if (wsMode === 'server' && !wantsServer) {
      logger.warn('⚠️ WS server mode requested but WS_SERVER_ENABLED is false')
    }
    if (wsMode === 'client' && !wantsClient) {
      logger.warn('⚠️ WS client mode requested but WS_CLIENT_ENABLED is false')
    }

    if (wsServerEnabled && typeof websocket.attachServer === 'function') {
      try {
        wsServerInstance = websocket.attachServer(this.server)
        logger.info('🔌 WebSocket server started')
      } catch (err) {
        logger.warn(`⚠️ WebSocket server start failed: ${err.message}`)
      }
    }
    if (wsClientEnabled && typeof websocket.startClient === 'function') {
      try {
        await websocket.startClient()
        logger.info('🔌 WebSocket client started')
      } catch (err) {
        logger.warn(`⚠️ WebSocket client start failed: ${err.message}`)
      }
    }

    const vpnMode = (config.vpn?.mode || 'client').toLowerCase()
    const vpnServerEnabled = vpnMode === 'server' || vpnMode === 'both'
    const vpnClientEnabled = vpnMode === 'client' || vpnMode === 'both'

    if (vpnServerEnabled && typeof vpn.startServer === 'function') {
      try {
        const vpnServer = await vpn.startServer({ wsServer: wsServerInstance })
        if (vpnServer?.bridge && wsServerInstance?.setTunnelBridge) {
          wsServerInstance.setTunnelBridge(vpnServer.bridge)
        }
        logger.info('🛜 VPN server started')
      } catch (err) {
        logger.warn(`⚠️ VPN server start failed: ${err.message}`)
      }
    }
    if (vpnClientEnabled && typeof vpn.startClient === 'function') {
      try {
        await vpn.startClient()
        logger.info('🛜 VPN client started')
      } catch (err) {
        logger.warn(`⚠️ VPN client start failed: ${err.message}`)
      }
    }
  }
  ...
  startCleanupTasks() {
    ...
    // 🔔 启动订阅生命周期任务（过期/取消/续费处理）
    try {
      const subscriptionLifecycleService = require('./services/subscriptionLifecycleService')
      subscriptionLifecycleService.start()
    } catch (error) {
      logger.error('❌ Failed to start subscription lifecycle service:', error)
    }
    ...
    if (clearQueuesOnStartup) {
      if (typeof redis.clearAllConcurrencyQueues === 'function') {
        redis.clearAllConcurrencyQueues().catch((error) => {
          logger.error('❌ Error clearing concurrency queues on startup:', error)
        })
      } else {
        logger.info('🚦 Concurrency queue cleanup not supported for current datastore driver')
      }
    } else {
      logger.info(
    ...
    const shutdown = async (signal) => {
      ...
      if (this.server) {
        this.server.close(async () => {
          ...
          try {
            const subscriptionLifecycleService = require('./services/subscriptionLifecycleService')
            subscriptionLifecycleService.stop()
          } catch (error) {
            logger.error('❌ Error stopping subscription lifecycle service:', error)
          }
