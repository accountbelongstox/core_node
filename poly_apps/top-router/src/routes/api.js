...
async function handleMessagesRequest(req, res) {
  try {
    ...
    if (translationTarget) {
      ...
      try {
        req.body.messages = await translationService.translateMessages(req.body.messages, {
          ...
          sourceLang: translationSource || undefined,
          keyId: req.apiKey?.id
        })
      } catch (error) {
    ...
    if (isStream) {
      ...
      if (accountType === 'client') {
        logger.debug(`[DEBUG] Using WS client relay for accountId: ${accountId}`)
        response = await clientRelayService.relayNonStreamRequest(
          req,
          res,
          accountId,
          sessionHash,
