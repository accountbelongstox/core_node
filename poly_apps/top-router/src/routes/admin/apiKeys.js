  ...
  try {
    ...
    if (validSortBy === 'cost') {
      ...
      if (effectiveCostTimeRange === 'custom') {
        ...
        logger.debug(`📊 Cost sort with custom range: ${costStartDate} to ${costEndDate}`)
        ...
        logger.debug(`📊 Cost sort using precomputed index: ${effectiveCostTimeRange}`)
...
router.get('/supported-clients', authenticateAdmin, async (req, res) => {
  try {
    ...
    logger.debug(`📱 Returning ${clients.length} supported clients`)
...
router.get('/api-keys/tags', authenticateAdmin, async (req, res) => {
  try {
    ...
    logger.debug(`📋 Retrieved ${tags.length} unique tags from API keys`)
...
router.put('/api-keys/batch', authenticateAdmin, async (req, res) => {
  try {
    ...
    logger.debug(`🔍 Debug: keyIds received: ${JSON.stringify(keyIds)}`)
    ...
    if (ownerId !== undefined) {
      ...
      if (ownerId === 'admin') {
        ...
        try {
          ...
          logger.debug(`🔄 Admin reassigning API key ${keyId} to user ${user.username}`)
        } catch (error) {
...
router.delete('/api-keys/batch', authenticateAdmin, async (req, res) => {
  try {
    ...
    // 调试信息
    logger.debug(`🐛 Batch delete request body: ${JSON.stringify(req.body)}`)
    logger.debug(`🐛 keyIds type: ${typeof keyIds}, value: ${JSON.stringify(keyIds)}`)
