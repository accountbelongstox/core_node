...
const resolveWsRuntimeFlags = () => {
  const mode = String(config.websocket?.mode || 'client').toLowerCase()
  const clientEnabled = Boolean(config.websocketClient?.enabled)
  const serverEnabled = Boolean(config.websocketServer?.enabled)
  const wsClientOnly = mode === 'client' && clientEnabled
  return { mode, clientEnabled, serverEnabled, wsClientOnly }
}

// 🌐 运行时模式与本机 WS/VPN 状态
router.get('/runtime-info', authenticateAdmin, (_req, res) => {
  const ws = resolveWsRuntimeFlags()
  const wsClient = websocket.getClient ? websocket.getClient() : null
  const wsClientStatus = wsClient && typeof wsClient.getStatus === 'function' ? wsClient.getStatus() : null
  const vpnClientRuntime = vpn.getClientRuntime ? vpn.getClientRuntime() : null
  const vpnClientStatus =
    vpnClientRuntime && typeof vpnClientRuntime.getStatus === 'function'
      ? vpnClientRuntime.getStatus()
      : null

  res.json({
    success: true,
    data: {
      uiMode: ws.wsClientOnly ? 'client' : 'full',
      ws,
      wsClientStatus,
      vpnClientStatus
    }
...
async function getOemSettings() {
  ...
  const oemSettings = client ? await client.get('oem:settings') : null
...
async function scanKeys(client, pattern, count = 1000) {
  if (!client) {
    return []
  }
