...
router.post('/exchange-code', authenticateAdmin, async (req, res) => {
  try {
    ...
    logger.debug('Exchanging OpenAI authorization code:', {
      sessionId,
...
router.get('/', authenticateAdmin, async (req, res) => {
  try {
    ...
    logger.debug(`获取 OpenAI 账户列表: ${accountsWithStats.length} 个账户`)
...
router.post('/', authenticateAdmin, async (req, res) => {
  try {
    ...
    if (needsImmediateRefresh && requireRefreshSuccess) {
      ...
      try {
        logger.debug(`🔄 测试刷新 OpenAI 账户以获取完整 token 信息`)
    ...
    if (needsImmediateRefresh && !requireRefreshSuccess) {
      try {
        logger.debug(`🔄 尝试刷新 OpenAI 账户 ${createdAccount.id}`)
...
router.put('/:id', authenticateAdmin, async (req, res) => {
  try {
    ...
    if (mappedUpdates.openaiOauth?.refreshToken && needsImmediateRefresh && requireRefreshSuccess) {
      ...
      try {
        logger.debug(`🔄 验证更新的 OpenAI token (账户: ${id})`)
    ...
    if (needsImmediateRefresh && !requireRefreshSuccess) {
      try {
        logger.debug(`🔄 尝试刷新 OpenAI 账户 ${id}`)
        await openaiAccountService.refreshAccountToken(id)
        logger.debug(`✅ 刷新成功`)
      } catch (refreshError) {
