  ...
  try {
    ...
    if (granularity === 'hour') {
      ...
      if (startDate && endDate) {
        ...
        // 调试日志
        logger.debug('📊 Usage trend hour granularity - received times:')
        logger.debug(`  startDate (raw): ${startDate}`)
        logger.debug(`  endDate (raw): ${endDate}`)
        logger.debug(`  startTime (parsed): ${startTime.toISOString()}`)
        logger.debug(`  endTime (parsed): ${endTime.toISOString()}`)
        logger.debug(
...
router.get('/api-keys/:keyId/model-stats', authenticateAdmin, async (req, res) => {
  try {
    ...
    if (period === 'custom' && startDate && endDate) {
      ...
      logger.debug(`📊 Preset period pattern: ${pattern}`)
    }
    ...
    for (const pattern of searchPatterns) {
      const keys = await client.keys(pattern)
      logger.debug(`📊 Pattern ${pattern} found ${keys.length} keys`)
    ...
    for (const [model, stats] of modelStatsMap) {
      logger.debug(`📊 Model ${model} aggregated data:`, stats)
      ...
      try {
        ...
        if (targetApiKey && targetApiKey.usage) {
          ...
          if (usageData && usageData.allTokens > 0) {
            ...
            logger.debug('📊 Generated display data from API key usage stats')
          } else {
            logger.debug(`📊 No usage data found for period ${period} in API key data`)
          }
        } else {
          logger.debug(`📊 API key ${keyId} not found or has no usage data`)
        }
      } catch (error) {
    ...
    logger.debug(`📊 Returning ${modelStats.length} model stats for API key ${keyId}:`, modelStats)
...
router.get('/api-keys-usage-trend', authenticateAdmin, async (req, res) => {
  try {
    ...
    logger.debug(`📊 Getting API keys usage trend, granularity: ${granularity}, days: ${days}`)
...
router.get('/usage-costs', authenticateAdmin, async (req, res) => {
  try {
    ...
    logger.debug(`💰 Calculating usage costs for period: ${period}`)
    ...
    if (period === 'today') {
      ...
      // 计算7天统计的费用
      logger.debug(`💰 Processing ${modelUsageMap.size} unique models for 7days cost calculation`)
      ...
      logger.debug(`💰 Total period calculation: found ${allModelKeys.length} monthly model keys`)
      ...
      if (allModelKeys.length > 0) {
        ...
        // 使用模型级别的数据计算费用
        logger.debug(`💰 Processing ${modelUsageMap.size} unique models for total cost calculation`)
