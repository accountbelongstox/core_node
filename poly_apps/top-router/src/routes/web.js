...
// 🌐 运行时模式（前端根据此信息决定是否隐藏营销/用户页面）
router.get('/runtime-info', (_req, res) => {
  const ws = resolveWsRuntimeFlags()
  const vpnMode = String(config.vpn?.mode || 'client').toLowerCase()
  const vpnEnabled = Boolean(config.vpn?.enabled)
  ...
  res.json({
    ...
    data: {
      ...
      ws,
      vpn: { mode: vpnMode, enabled: vpnEnabled }
    }
  })
})
    ...
    if (!adminData || Object.keys(adminData).length === 0) {
      ...
      if (fs.existsSync(initFilePath)) {
        try {
          ...
          // 重新存储到Redis，不设置过期时间
          await redis.setSession('admin_credentials', adminData, null)
...
router.post('/auth/change-password', async (req, res) => {
  try {
    ...
    try {
      ...
      await redis.setSession('admin_credentials', updatedAdminData, null)
    } catch (fileError) {
