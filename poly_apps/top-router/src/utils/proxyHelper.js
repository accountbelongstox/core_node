...
class ProxyHelper {
  ...
  static createProxyAgent(proxyConfig, options = {}) {
    ...
    try {
      ...
      if (ProxyHelper._agentCache.has(cacheKey)) {
        const cachedAgent = ProxyHelper._agentCache.get(cacheKey)
        ProxyHelper._attachAgentErrorHandler(cachedAgent, cacheKey)
        return cachedAgent
      }
      ...
      if (agent) {
        ProxyHelper._attachAgentErrorHandler(agent, cacheKey)
  ...
  static _attachAgentErrorHandler(agent, cacheKey) {
    if (!agent || agent.__proxyErrorHandlerAttached) {
      return
    }

    agent.__proxyErrorHandlerAttached = true
    agent.on('error', (error) => {
      const errorCode = error?.code || error?.cause?.code
      logger.error('❌ Proxy agent error:', {
      ...
      if (cacheKey) {
        ProxyHelper._agentCache.delete(cacheKey)
      }
    })
  }
