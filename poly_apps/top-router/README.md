...
- **Node.js** 18或更高版本
- **数据存储**：默认 Redis 6+；可选 SQLite（设置 `DATASTORE_PROVIDER=sqlite`，首次运行前执行 `npm run setup:sqlite` 初始化数据库）
- **操作系统**: 建议Linux
...
- 开启：`TRANSLATION_ENABLED=true`，并配置 `TRANSLATION_PROVIDER` / `TRANSLATION_FALLBACK_PROVIDER`
- 触发：请求头 `x-translation-target` 或查询参数 `translation_target`（可选 `x-translation-source` / `translation_source`）
- 适用范围：Claude / OpenAI / Gemini 路由；未开启开关时不生效
