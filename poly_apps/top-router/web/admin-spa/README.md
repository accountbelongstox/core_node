...
**重要提示：**
- 开发服务器启动后，会自动在浏览器中打开
- 默认访问路径：http://localhost:3001/admin/
- 不要访问 http://localhost:3001/ （会显示404）
- 首次访问会自动跳转到登录页面

如需本地也使用 `/admin-next/` 路径，可在 `.env.development.local` 中设置：
...
A: 这是正常的。默认开发路径是 `/admin/`，必须访问完整路径：http://localhost:3001/admin/
...
- **生产环境**：部署在 `/admin-next/`（或通过 `VITE_APP_BASE_URL` 指定），无需额外配置
