...
const resolveLoginUrl = () => {
  ...
  const isUserContext = currentPath.includes('/app') || currentPath.includes('/auth/user-login')
  return getLoginUrl(hasUserToken || isUserContext ? 'user' : 'admin')
}
...
class ApiClient {
  ...
  getAuthToken() {
    ...
    return userToken || authToken || null
  }
  ...
  async handleResponse(response) {
    ...
    if (response.status === 401) {
      ...
      const currentPath = window.location.pathname + window.location.hash
      const isLoginPage =
        currentPath.includes('/auth/admin-login') ||
        currentPath.includes('/auth/user-login') ||
