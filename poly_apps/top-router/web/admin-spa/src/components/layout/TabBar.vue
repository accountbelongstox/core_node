...
const tabs = computed(() => {
  const baseTabs = [
    ...
    { key: 'subscriptions', name: '订阅总览', shortName: '订阅', icon: 'fas fa-receipt' },
    { key: 'observability', name: '事件 & 审计', shortName: '日志', icon: 'fas fa-clipboard-list' },
    { key: 'vpn', name: 'VPN 隧道', shortName: 'VPN', icon: 'fas fa-shield-alt' },
    { key: 'clients', name: 'Client 管理', shortName: 'Client', icon: 'fas fa-network-wired' },
  ...
  if (runtimeStore.isWsClientOnly) {
    return baseTabs.filter((tab) => tab.key !== 'userManagement' && tab.key !== 'subscriptions')
  }
