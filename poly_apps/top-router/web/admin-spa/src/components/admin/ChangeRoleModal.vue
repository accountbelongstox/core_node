    ...
    <div class="relative top-20 mx-auto w-96 rounded-md border bg-white p-5 shadow-lg">
      <div class="mt-3">
        <div class="mb-4 flex items-center justify-between">
          <h3 class="text-lg font-medium text-gray-900">更改用户角色</h3>
              ...
              <div class="space-y-2">
                <label class="flex items-center">
                  ...
                  <div class="ml-3">
                    ...
                    <div class="text-xs text-gray-500">拥有基本权限的普通用户</div>
                  </div>
                </label>
                <label class="flex items-center">
                  <input
                ...
                <div class="ml-3">
                  ...
                  <div class="mt-2 text-sm text-yellow-700">
                    <p v-if="selectedRole === 'admin'">
                      授予管理员权限将赋予该用户完整的系统访问权限，包括管理其他用户及其 API Key
                      的能力。
                    </p>
                    <p v-else>
                      移除管理员权限后，该用户将只能管理自己的 API Key 和查看自己的使用统计。
                    </p>
                  </div>
                </div>
              </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.success) {
      const roleText = selectedRole.value === 'admin' ? '管理员' : '用户'
      showToast(`用户角色已更新为${roleText}`, 'success')
      emit('updated')
    } else {
