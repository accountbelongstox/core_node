    ...
    >
      <div class="mt-3">
        <div class="mb-4 flex items-center justify-between">
          <h3 class="text-lg font-medium text-gray-900">创建新的 API Key</h3>
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          <div>
            <label class="block text-sm font-medium text-gray-700" for="name"> 名称 * </label>
            <input
              id="name"
              v-model="form.name"
        ...
        <div v-if="newApiKey" class="mt-6 rounded-md border border-green-200 bg-green-50 p-4">
          <div class="flex items-start">
            ...
            <div class="ml-3 flex-1">
              <h4 class="text-sm font-medium text-green-800">API Key 创建成功！</h4>
              <div class="mt-3">
                <p class="mb-2 text-sm text-green-700">
                  <strong>重要提示：</strong>请立即复制您的 API Key。您将无法再次查看！
                </p>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      error.value = result.message || '创建 API Key 失败'
    }
  } catch (err) {
    console.error('Create API key error:', err)
    error.value = err.response?.data?.message || err.message || '创建 API Key 失败'
  } finally {
    loading.value = false
  }
}
