<template>
  <div class="space-y-6">
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          <p class="text-sm text-yellow-700 dark:text-yellow-600">
            您已达到 API 密钥的最大数量（{{ maxApiKeys }}）。请删除现有密钥以创建新密钥。
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="loading" class="py-12 text-center">
      ...
      <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">正在加载 API 密钥...</p>
    </div>
          ...
          <div class="flex items-center justify-between">
              ...
              <div class="ml-4">
                <div class="flex items-center">
                  ...
                  <span
                    ...
                    class="ml-2 inline-flex items-center rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900/30 dark:text-red-300"
                  >
                    已删除
                  </span>
                </div>
            ...
            <div class="flex items-center space-x-2">
              ...
              <div class="text-right text-xs text-gray-500 dark:text-gray-400">
                <div>{{ formatNumber(apiKey.usage?.requests || 0) }} 请求</div>
              ...
              <div class="flex items-center space-x-1">
                <button
                  class="inline-flex items-center rounded border border-transparent p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                  title="查看 API 密钥"
    ...
    <div v-else class="py-12 text-center">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">没有 API 密钥</h3>
...
const handleDeleteConfirm = async () => {
  try {
    ...
    if (result.success) {
      showToast('API 密钥已成功删除', 'success')
      await loadApiKeys()
    }
  } catch (error) {
    console.error('Failed to delete API key:', error)
    showToast('删除 API 密钥失败', 'error')
  } finally {
