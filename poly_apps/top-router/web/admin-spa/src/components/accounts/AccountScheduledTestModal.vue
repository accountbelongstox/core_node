...
const buildAdminApiUrl = inject('buildAdminApiUrl', (path) => createApiUrl(path))
...
async function loadConfig() {
  ...
  try {
    ...
    let endpoint = ''
    if (platform === 'claude') {
      endpoint = buildAdminApiUrl(`/admin/claude-accounts/${props.account.id}/test-config`)
    } else {
      // 其他平台暂不支持
      loading.value = false
      return
    ...
    // 获取测试历史
    const historyEndpoint = buildAdminApiUrl(
      `/admin/claude-accounts/${props.account.id}/test-history`
    )
