          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                placeholder="例如：https://api.example.com 或 https://api.example.com/v1/messages"
                required
                type="text"
              />
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                支持填写基础域名或完整 /v1/messages，系统会自动规范化。
              </p>
...
const submit = async () => {
  ...
  try {
    if (isEdit.value) {
      ...
      const res = await adminApi.put(`/admin/ccr-accounts/${props.account.id}`, updates)
      if (res.success) {
      ...
      const res = await adminApi.post('/admin/ccr-accounts', payload)
      if (res.success) {
        // 不在这里显示 toast，由父组件统一处理
        emit('success')
      } else {
