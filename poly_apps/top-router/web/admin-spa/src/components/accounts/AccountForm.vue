...
const adminApi = inject('adminApi', apiClient)
...
const loadAccountUsage = async () => {
  ...
  try {
    const response = await adminApi.get(`/admin/claude-console-accounts/${props.account.id}/usage`)
    if (response) {
      // 更新表单中的使用量数据
        ...
        loadGroups().then(async () => {
          ...
          if (
            ...
            const checkPromises = groups.value.map(async (group) => {
              try {
                const response = await adminApi.get(`/admin/account-groups/${group.id}/members`)
...
const fetchUnifiedUserAgent = async () => {
  try {
    const response = await adminApi.get('/admin/claude-code-version')
...
const clearUnifiedCache = async () => {
  ...
  try {
    const response = await adminApi.post('/admin/claude-code-version/clear')
    if (response.success) {
      unifiedUserAgent.value = ''
