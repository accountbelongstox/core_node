  ...
  <div class="accounts-container">
    <div class="card p-4 sm:p-6">
      ...
      <div v-else class="table-container hidden md:block">
        ...
        >
          <template v-if="shouldShowCheckboxes" #header-selection>
          ...
          <template v-if="shouldShowCheckboxes" #cell-selection="{ row }">
            <div class="flex items-center">
              <input
          ...
          <template #header-name="{ column }">
            <button
              :class="sortableHeaderClass"
              type="button"
              @click="sortAccounts(column.sortKey)"
            >
              <span>{{ column.label }}</span>
              <i
                v-if="accountsSortBy === column.sortKey"
                :class="[
                  'fas',
          ...
          <template #header-platform="{ column }">
            <button
              :class="sortableHeaderClass"
              type="button"
              @click="sortAccounts(column.sortKey)"
          ...
          <template #header-expiresAt="{ column }">
            <button
              :class="sortableHeaderClass"
              type="button"
              @click="sortAccounts(column.sortKey)"
          ...
          <template #header-windows="{ column }">
              ...
              <el-tooltip placement="top">
                <template #content>
                  ...
                  >
                    ...
                    <div class="space-y-2">
                      <div class="text-sm font-semibold text-white dark:text-gray-900">OpenAI</div>
                      <div class="text-gray-200 dark:text-gray-600">
                      ...
                      <div class="space-y-1 text-gray-200 dark:text-gray-600">
                        ...
                        <div class="flex items-start gap-2">
                          <i class="fas fa-info-circle mt-[2px] text-[10px] text-indigo-500"></i>
                          <span class="font-medium text-white dark:text-gray-900"
                    ...
                    <div class="space-y-2">
                      ...
                      <div class="space-y-1 text-gray-200 dark:text-gray-600">
                        ...
                        <div class="flex items-start gap-2">
                          <i class="fas fa-calendar-alt mt-[2px] text-[10px] text-emerald-500"></i>
                          <span class="font-medium text-white dark:text-gray-900"
          ...
          <template #cell-name="{ row }">
            <div class="flex items-center">
              ...
              <div class="min-w-0">
                ...
                <div
                  v-if="row.groupInfos && row.groupInfos.length > 0"
                  class="my-2 flex flex-wrap items-center gap-2"
                >
                  <span
                    v-for="group in row.groupInfos"
                    :key="group.id"
                ...
                <div class="truncate text-xs text-gray-500 dark:text-gray-400" :title="row.id">
                  {{ row.id }}
                </div>
              </div>
            </div>
          ...
          <template #cell-platform="{ row }">
            <AccountPlatformBadge :account="row" />
          </template>
            ...
            <div class="flex flex-col gap-1">
              <span v-if="row.expiresAt">
                <span
                  ...
                  @click.stop="startEditAccountExpiry(row)"
                >
          ...
          <template #cell-status="{ row }">
            <AccountStatusIndicators :account="row" />
          </template>
          ...
          <template #cell-priority="{ row }">
            <div
              v-if="
                row.platform === 'claude' ||
                row.platform === 'claude-console' ||
                row.platform === 'bedrock' ||
                row.platform === 'gemini' ||
                row.platform === 'gemini-api' ||
                row.platform === 'openai' ||
                row.platform === 'openai-responses' ||
                row.platform === 'azure_openai' ||
                row.platform === 'ccr' ||
                row.platform === 'droid'
              "
            ...
            >
              <div class="h-2 w-16 rounded-full bg-gray-200">
                <div
                  ...
                  :style="{ width: 101 - (row.priority || 50) + '%' }"
                />
              </div>
          ...
          <template #cell-proxy="{ row }">
            <div
              v-if="formatProxyDisplay(row.proxy)"
              class="break-all rounded bg-blue-50 px-2 py-1 font-mono text-xs"
              :title="formatProxyDisplay(row.proxy)"
            >
              {{ formatProxyDisplay(row.proxy) }}
            </div>
            <div v-else class="text-gray-400">无代理</div>
          </template>

          <template #cell-usage="{ row }">
            <div v-if="row.usage && row.usage.daily" class="space-y-1">
              <div class="flex items-center gap-2">
                <div class="h-2 w-2 rounded-full bg-blue-500" />
                <span class="text-sm font-medium text-gray-900 dark:text-gray-100"
                  >{{ row.usage.daily.requests || 0 }} 次</span
                >
              </div>
              <div class="flex items-center gap-2">
                <div class="h-2 w-2 rounded-full bg-purple-500" />
                <span class="text-xs text-gray-600 dark:text-gray-300"
                  >{{ formatNumber(row.usage.daily.allTokens || 0) }}M</span
                >
              </div>
              <div class="flex items-center gap-2">
                <div class="h-2 w-2 rounded-full bg-green-500" />
                <span class="text-xs text-gray-600 dark:text-gray-300"
                  >${{ calculateDailyCost(row) }}</span
                >
              </div>
              <div
                v-if="row.usage.averages && row.usage.averages.rpm > 0"
                class="text-xs text-gray-500 dark:text-gray-400"
              >
                平均 {{ row.usage.averages.rpm.toFixed(2) }} RPM
              </div>
            </div>
          ...
          <template #cell-windows="{ row }">
            <div v-if="row.platform === 'claude'" class="space-y-2">
              <div v-if="isClaudeOAuth(row) && row.claudeUsage" class="space-y-2">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <div class="flex-1">
                      <div class="flex items-center gap-2">
                        <div class="h-2 flex-1 rounded-full bg-gray-200 dark:bg-gray-600">
                          <div
                            :class="[
                              ...
                              getClaudeUsageBarClass(row.claudeUsage.fiveHour)
                            ]"
                            :style="{
                              width: getClaudeUsageWidth(row.claudeUsage.fiveHour)
                            }"
                          />
                        </div>
                        <span
                        ...
                        >
                          {{ formatClaudeUsagePercent(row.claudeUsage.fiveHour) }}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                    重置剩余 {{ formatClaudeRemaining(row.claudeUsage.fiveHour) }}
                  </div>
                </div>
                <div class="rounded-lg bg-gray-50 p-2 dark:bg-gray-700/70">
                  <div class="flex items-center gap-2">
                      ...
                      <div class="flex items-center gap-2">
                        <div class="h-2 flex-1 rounded-full bg-gray-200 dark:bg-gray-600">
                          <div
                            :class="[
                              'h-2 rounded-full transition-all duration-300',
                              getClaudeUsageBarClass(row.claudeUsage.sevenDay)
                            ]"
                            :style="{
                              width: getClaudeUsageWidth(row.claudeUsage.sevenDay)
                            }"
                          />
                        </div>
                        <span
                          class="w-12 text-right text-xs font-semibold text-gray-800 dark:text-gray-100"
                        >
                          {{ formatClaudeUsagePercent(row.claudeUsage.sevenDay) }}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                    重置剩余 {{ formatClaudeRemaining(row.claudeUsage.sevenDay) }}
                  </div>
                </div>
                <div class="rounded-lg bg-gray-50 p-2 dark:bg-gray-700/70">
                  <div class="flex items-center gap-2">
                      ...
                      <div class="flex items-center gap-2">
                        <div class="h-2 flex-1 rounded-full bg-gray-200 dark:bg-gray-600">
                          <div
                            :class="[
                              'h-2 rounded-full transition-all duration-300',
                              getClaudeUsageBarClass(row.claudeUsage.sevenDayOpus)
                            ]"
                            :style="{
                              width: getClaudeUsageWidth(row.claudeUsage.sevenDayOpus)
                            }"
                          />
                        </div>
                        <span
                          class="w-12 text-right text-xs font-semibold text-gray-800 dark:text-gray-100"
                        >
                          {{ formatClaudeUsagePercent(row.claudeUsage.sevenDayOpus) }}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                    重置剩余 {{ formatClaudeRemaining(row.claudeUsage.sevenDayOpus) }}
                  </div>
                </div>
              </div>
              <div
                v-else-if="
                  !isClaudeOAuth(row) && row.sessionWindow && row.sessionWindow.hasActiveWindow
                "
                class="space-y-2"
              >
                <div
                  v-if="row.usage && row.usage.sessionWindow"
                  class="flex items-center gap-3 text-xs"
                >
                  <div class="flex items-center gap-1">
                    <div class="h-1.5 w-1.5 rounded-full bg-purple-500" />
                    <span class="font-medium text-gray-900 dark:text-gray-100">
                      {{ formatNumber(row.usage.sessionWindow.totalTokens) }}M
                    </span>
                  </div>
                  <div class="flex items-center gap-1">
                    <div class="h-1.5 w-1.5 rounded-full bg-green-500" />
                    <span class="font-medium text-gray-900 dark:text-gray-100">
                      ${{ formatCost(row.usage.sessionWindow.totalCost) }}
                    </span>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <div class="h-2 w-24 rounded-full bg-gray-200 dark:bg-gray-700">
                    <div
                      :class="[
                        'h-2 rounded-full transition-all duration-300',
                        getSessionProgressBarClass(row.sessionWindow.sessionWindowStatus, row)
                      ]"
                      :style="{
                        width: row.sessionWindow?.progressPercent
                          ? Math.min(100, row.sessionWindow.progressPercent) + '%'
                          : '100%'
                      }"
                    />
                  </div>
                  <span class="text-xs font-medium text-gray-700 dark:text-gray-200">
                    {{
                      row.sessionWindow?.progressPercent
                        ? row.sessionWindow.progressPercent + '%'
                        : '0%'
                    }}
                  </span>
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400">
                  {{
                    formatSessionWindow(row.sessionWindow?.startTime, row.sessionWindow?.endTime)
                  }}
                </div>
              </div>
              <div v-else class="text-xs text-gray-400">暂无统计</div>
            </div>
            <div v-else-if="row.platform === 'claude-console'" class="space-y-3">
              <div>
                <template v-if="Number(row.dailyQuota) > 0">
                  <div class="flex items-center justify-between text-xs">
                    <span class="text-gray-600 dark:text-gray-300">额度进度</span>
                    <span class="font-medium text-gray-700 dark:text-gray-200">
                      {{ getQuotaUsagePercent(row).toFixed(1) }}%
                    </span>
                  </div>
                  <div class="flex items-center gap-2">
                    <div class="h-2 w-24 rounded-full bg-gray-200 dark:bg-gray-700">
                      <div
                        :class="[
                          'h-2 rounded-full transition-all duration-300',
                          getQuotaBarClass(getQuotaUsagePercent(row))
                        ]"
                        :style="{ width: Math.min(100, getQuotaUsagePercent(row)) + '%' }"
                      />
                    </div>
                    <span class="min-w-[32px] text-xs font-medium text-gray-700 dark:text-gray-200">
                      ${{ formatCost(row.usage?.daily?.cost || 0) }} / ${{
                        Number(row.dailyQuota).toFixed(2)
                      }}
                    </span>
                  </div>
                  <div class="text-xs text-gray-600 dark:text-gray-400">
                    剩余 ${{ formatRemainingQuota(row) }}
                    <span class="ml-2 text-gray-400">重置 {{ row.quotaResetTime || '00:00' }}</span>
                  </div>
                </template>
                <template v-else>
              ...
              <div class="space-y-1">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span
                    v-if="Number(row.maxConcurrentTasks || 0) > 0"
                    class="font-medium text-gray-700 dark:text-gray-200"
                  >
                    {{ getConsoleConcurrencyPercent(row).toFixed(0) }}%
                  </span>
                </div>
                <div v-if="Number(row.maxConcurrentTasks || 0) > 0" class="flex items-center gap-2">
                  <div class="h-2 w-24 rounded-full bg-gray-200 dark:bg-gray-700">
                    <div
                      :class="[
                        'h-2 rounded-full transition-all duration-300',
                        getConcurrencyBarClass(getConsoleConcurrencyPercent(row))
                      ]"
                      :style="{ width: Math.min(100, getConsoleConcurrencyPercent(row)) + '%' }"
                    />
                  </div>
                  <span
                    :class="['min-w-[48px] text-xs font-medium', getConcurrencyLabelClass(row)]"
                  >
                    {{ Number(row.activeTaskCount || 0) }} /
                    {{ Number(row.maxConcurrentTasks || 0) }}
                  </span>
                </div>
                <div
                  v-else
            ...
            <div v-else-if="row.platform === 'openai'" class="space-y-2">
              <div v-if="row.codexUsage" class="space-y-2">
                <div class="rounded-lg bg-gray-50 p-2 dark:bg-gray-700/70">
                  <div class="flex items-center gap-2">
                    <span
                      class="inline-flex min-w-[32px] justify-center rounded-full bg-indigo-100 px-2 py-0.5 text-[11px] font-medium text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-300"
                      ...
                      <div class="flex items-center gap-2">
                        <div class="h-2 flex-1 rounded-full bg-gray-200 dark:bg-gray-600">
                          <div
                            :class="[
                              'h-2 rounded-full transition-all duration-300',
                              getCodexUsageBarClass(row.codexUsage.primary)
                            ]"
                            :style="{
                              width: getCodexUsageWidth(row.codexUsage.primary)
                            }"
                          />
                        </div>
                        <span
                          class="w-12 text-right text-xs font-semibold text-gray-800 dark:text-gray-100"
                        >
                          {{ formatCodexUsagePercent(row.codexUsage.primary) }}
                        </span>
                      </div>
                    </div>
                  </div>
                      ...
                      <div class="flex items-center gap-2">
                        <div class="h-2 flex-1 rounded-full bg-gray-200 dark:bg-gray-600">
                          <div
                            :class="[
                              'h-2 rounded-full transition-all duration-300',
                              getCodexUsageBarClass(row.codexUsage.secondary)
                            ]"
                            :style="{
                              width: getCodexUsageWidth(row.codexUsage.secondary)
                            }"
                          />
                        </div>
                        <span
                          class="w-12 text-right text-xs font-semibold text-gray-800 dark:text-gray-100"
                        >
                          {{ formatCodexUsagePercent(row.codexUsage.secondary) }}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <button
                v-if="
                  ...
                  (row.status === 'unauthorized' ||
                    row.status !== 'active' ||
                    row.rateLimitStatus?.isRateLimited ||
                    row.rateLimitStatus === 'limited' ||
                    !row.isActive)
                "
                :class="[
                ...
                :disabled="row.isResetting"
                :title="row.isResetting ? '重置中...' : '重置所有异常状态'"
                @click="resetAccountStatus(row)"
              >
                <i :class="['fas fa-redo', row.isResetting ? 'animate-spin' : '']" />
                <span class="ml-1">重置状态</span>
              </button>
              <button
                :class="[
                  'rounded px-2.5 py-1 text-xs font-medium transition-colors',
                  row.isTogglingSchedulable
                    ? 'cursor-not-allowed bg-gray-100 text-gray-400'
                    : row.schedulable
                      ? 'bg-green-100 text-green-700 hover:bg-green-200'
                      : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                ]"
                :disabled="row.isTogglingSchedulable"
                :title="row.schedulable ? '点击禁用调度' : '点击启用调度'"
                @click="toggleSchedulable(row)"
              >
                <i :class="['fas', row.schedulable ? 'fa-toggle-on' : 'fa-toggle-off']" />
                <span class="ml-1">{{ row.schedulable ? '调度' : '停用' }}</span>
              </button>
              <button
                v-if="canViewUsage(row)"
                class="rounded bg-indigo-100 px-2.5 py-1 text-xs font-medium text-indigo-700 transition-colors hover:bg-indigo-200"
                :title="'查看使用详情'"
                @click="openAccountUsageModal(row)"
              >
...
import AccountPlatformBadge from '@/components/accounts/AccountPlatformBadge.vue'
import AccountStatusIndicators from '@/components/accounts/AccountStatusIndicators.vue'
import ConfirmModal from '@/components/common/ConfirmModal.vue'
import DataTable from '@/components/common/DataTable.vue'
import CustomDropdown from '@/components/common/CustomDropdown.vue'

// 嵌入模式 prop（用于在 ClientAccountsModal 中复用）
...
const clientAccountsLoaded = ref(false)

const syncClientStoreContext = (embedded, clientId) => {
  const targetClientId = embedded ? clientId : ''
  if (accountsStore.fromClient !== embedded || accountsStore.clientId !== targetClientId) {
    clientAccountsLoaded.value = false
  }
  accountsStore.fromClient = embedded
  accountsStore.clientId = targetClientId
}

watch(
  () => [props.embedded, props.clientId],
  ([embedded, clientId]) => {
    if (embedded && clientId) {
      syncClientStoreContext(true, clientId)
      return
    }
    syncClientStoreContext(false, '')
  },
  { immediate: true }
)
...
const bindingCountsLoaded = ref(false)
...
const platformOptions = ref([
  ...
  { value: 'gemini-api', label: 'Gemini API', icon: 'fas fa-key' },
...
const accountTableColumns = computed(() => {
  const columns = [
    {
      key: 'name',
      label: '名称',
      sortKey: 'name',
      thClass: 'w-[22%] min-w-[180px]',
      tdClass: 'w-[22%] min-w-[180px]'
    },
    {
      key: 'platform',
      label: '平台/类型',
      sortKey: 'platform',
      thClass: 'w-[15%] min-w-[120px]',
      tdClass: 'w-[15%] min-w-[120px]'
    },
    {
      key: 'expiresAt',
      label: '到期时间',
      sortKey: 'expiresAt',
      thClass: 'w-[12%] min-w-[110px]',
      tdClass: 'w-[12%] min-w-[110px]'
    },
    {
      key: 'status',
      label: '状态',
      sortKey: 'status',
      thClass: 'w-[12%] min-w-[100px]',
      tdClass: 'w-[12%] min-w-[100px]'
    },
    {
      key: 'priority',
      label: '优先级',
      sortKey: 'priority',
      thClass: 'w-[8%] min-w-[80px]',
      tdClass: 'w-[8%] min-w-[80px]'
    },
    {
      key: 'proxy',
      label: '代理',
      thClass: 'w-[10%] min-w-[100px]',
      tdClass: 'w-[10%] min-w-[100px]'
    },
    {
      key: 'usage',
      label: '今日使用',
      thClass: 'w-[10%] min-w-[90px]',
      tdClass: 'w-[10%] min-w-[90px]'
    },
    {
      key: 'windows',
      label: '会话窗口',
    ...
    },
    {
      key: 'actions',
      label: '操作',
      thClass: 'w-[15%] min-w-[180px]',
      tdClass: 'w-[15%] min-w-[180px]'
    }
  ]

  if (shouldShowCheckboxes.value) {
    columns.unshift({
      key: 'selection',
      label: '',
      thClass: 'w-[50px]',
...
const sortableHeaderClass =
  'flex w-full cursor-pointer select-none items-center justify-start gap-1 text-left text-xs font-bold uppercase tracking-wider text-gray-700 hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-100'
...
const getBindingCountFromMap = (field, bindingValue) => {
  if (!field || !bindingValue) {
    return 0
  }
  const fieldMap = bindingCounts.value[field]
  if (!fieldMap || typeof fieldMap !== 'object') {
    return 0
  }
  return fieldMap[bindingValue] || 0
}

const normalizeClientAccountPlatform = (value) => {
  ...
  if (normalized === 'claude-official' || normalized === 'claude-oauth') {
    return 'claude'
  }
  if (normalized === 'claude-console') {
...
const buildClientAccountRecord = (account) => {
  const platform = normalizeClientAccountPlatform(account?.platform || account?.type)
  const isActive =
  ...
  return {
    ...
    platform,
    isActive,
    schedulable: account?.schedulable !== undefined ? account.schedulable : true,
    boundApiKeysCount: account?.boundApiKeysCount ?? 0
  }
}

const buildClientAccountsFromStore = () => {
  const allAccounts = []

  const appendAccounts = (items, platform) => {
    if (!Array.isArray(items)) {
      return
    }
    items.forEach((account) => {
      allAccounts.push(buildClientAccountRecord({ ...account, platform }))
    })
  }

  appendAccounts(accountsStore.claudeAccounts, 'claude')
  appendAccounts(accountsStore.claudeConsoleAccounts, 'claude-console')
  appendAccounts(accountsStore.bedrockAccounts, 'bedrock')
  appendAccounts(accountsStore.geminiAccounts, 'gemini')
  appendAccounts(accountsStore.geminiApiAccounts, 'gemini-api')
  appendAccounts(accountsStore.openaiAccounts, 'openai')
  appendAccounts(accountsStore.azureOpenaiAccounts, 'azure_openai')
  appendAccounts(accountsStore.openaiResponsesAccounts, 'openai-responses')
  appendAccounts(accountsStore.ccrAccounts, 'ccr')
  appendAccounts(accountsStore.droidAccounts, 'droid')
...
const loadAccounts = async (forceReload = false) => {
  ...
  try {
    if (props.embedded && props.clientId) {
      if (forceReload || !clientAccountsLoaded.value) {
        await accountsStore.fetchAllAccounts()
        clientAccountsLoaded.value = true
      }

      let filteredAccounts = buildClientAccountsFromStore()
    ...
    if (platformFilter.value === 'all') {
      ...
      switch (platformFilter.value) {
        case 'claude':
          requests.push(
            apiClient.get('/admin/claude-accounts', { params }),
            emptyResponse(), // claude-console 占位
            emptyResponse(), // bedrock 占位
            emptyResponse(), // gemini 占位
            emptyResponse(), // gemini-api 占位
            emptyResponse(), // openai 占位
            emptyResponse(), // azure-openai 占位
            emptyResponse(), // openai-responses 占位
            emptyResponse(), // ccr 占位
            emptyResponse() // droid 占位
          )
          break
        case 'claude-console':
          requests.push(
            emptyResponse(), // claude 占位
            apiClient.get('/admin/claude-console-accounts', { params }),
            emptyResponse(), // bedrock 占位
            emptyResponse(), // gemini 占位
            emptyResponse(), // gemini-api 占位
    ...
    if (claudeData.success) {
      const claudeAccounts = (claudeData.data || []).map((acc) => {
        const boundApiKeysCount = getBindingCountFromMap('claudeAccountId', acc.id)
    ...
    if (claudeConsoleData.success) {
      const claudeConsoleAccounts = (claudeConsoleData.data || []).map((acc) => {
        const boundApiKeysCount = getBindingCountFromMap('claudeConsoleAccountId', acc.id)
    ...
    if (geminiData.success) {
      const geminiAccounts = (geminiData.data || []).map((acc) => {
        const boundApiKeysCount = getBindingCountFromMap('geminiAccountId', acc.id)
    ...
    if (geminiApiData.success) {
      const geminiApiAccounts = (geminiApiData.data || []).map((acc) => {
        const bindingKey = acc.id ? `api:${acc.id}` : acc.id
        const boundApiKeysCount = getBindingCountFromMap('geminiAccountId', bindingKey)
    ...
    if (openaiData.success) {
      const openaiAccounts = (openaiData.data || []).map((acc) => {
        const boundApiKeysCount = getBindingCountFromMap('openaiAccountId', acc.id)
    ...
    if (azureOpenaiData && azureOpenaiData.success) {
      const azureOpenaiAccounts = (azureOpenaiData.data || []).map((acc) => {
        const boundApiKeysCount = getBindingCountFromMap('azureOpenaiAccountId', acc.id)
    ...
    if (openaiResponsesData && openaiResponsesData.success) {
      const openaiResponsesAccounts = (openaiResponsesData.data || []).map((acc) => {
        const boundApiKeysCount = getBindingCountFromMap('openaiAccountId', `responses:${acc.id}`)
    ...
    if (droidData && droidData.success) {
      const droidAccounts = (droidData.data || []).map((acc) => {
        return {
          ...
          platform: 'droid',
          boundApiKeysCount:
            getBindingCountFromMap('droidAccountId', acc.id) ?? acc.boundApiKeysCount ?? 0
        }
      })
    ...
    if (filteredAccounts.some((acc) => acc.platform === 'claude')) {
      loadClaudeUsage().catch((err) => {
        console.warn('Claude usage loading failed:', err)
      })
    }
  } catch (error) {
...
const loadClaudeUsage = async () => {
  try {
    ...
    console.warn('Failed to load Claude usage data:', error)
  }
}
...
const loadBindingCounts = async (forceReload = false) => {
  ...
  try {
    ...
    if (response.success) {
      const summary = response.data?.countsByField
      if (summary && typeof summary === 'object') {
        bindingCounts.value = summary
      } else if (Array.isArray(response.data?.bindings)) {
        const fallback = {}
        response.data.bindings.forEach((binding) => {
          const field =
            binding.bindingField ||
            (binding.platform === 'claude'
              ? 'claudeAccountId'
              : binding.platform === 'claude-console'
                ? 'claudeConsoleAccountId'
                : binding.platform === 'openai'
                  ? 'openaiAccountId'
                  : binding.platform === 'openai-responses'
                    ? 'openaiAccountId'
                    : binding.platform === 'gemini'
                      ? 'geminiAccountId'
                      : binding.platform === 'gemini-api'
                        ? 'geminiAccountId'
                        : binding.platform === 'droid'
                          ? 'droidAccountId'
                          : binding.platform === 'azure-openai'
          ...
          fallback[field][binding.bindingValue] = binding.count || 0
        })
        bindingCounts.value = fallback
      } else {
        bindingCounts.value = {}
      }
...
const loadApiKeys = async (forceReload = false) => {
  ...
  try {
    const response = await apiClient.get('/admin/api-keys', {
      params: { pageSize: 500, page: 1, includeDeleted: true }
    })
    if (response.success) {
...
const getBoundApiKeysCountForAccount = (account) => {
  ...
  switch (platform) {
    case 'claude':
      return getBindingCountFromMap('claudeAccountId', account.id)
    case 'claude-console':
      return getBindingCountFromMap('claudeConsoleAccountId', account.id)
    case 'gemini':
      return getBindingCountFromMap('geminiAccountId', account.id)
    case 'gemini-api':
      return getBindingCountFromMap('geminiAccountId', `api:${account.id}`)
    case 'openai':
      return getBindingCountFromMap('openaiAccountId', account.id)
    case 'openai-responses':
      return getBindingCountFromMap('openaiAccountId', `responses:${account.id}`)
    case 'azure_openai':
      return getBindingCountFromMap('azureOpenaiAccountId', account.id)
    case 'droid':
      return getBindingCountFromMap('droidAccountId', account.id)
    default:
      return 0
  }
}
...
const deleteAccount = async (account) => {
  const boundKeysCount = getBoundApiKeysCountForAccount(account)
...
const batchDeleteAccounts = async () => {
  ...
  const boundInfo = targets
    .map((account) => ({ account, boundCount: getBoundApiKeysCountForAccount(account) }))
    .filter((item) => item.boundCount > 0)

  if (boundInfo.length > 0) {
    confirmMessage += '\n\n⚠️ 以下账户存在绑定的 API Key，将自动解绑：'
    boundInfo.forEach(({ account, boundCount }) => {
      const displayName = account.name || account.email || account.accountName || account.id
      confirmMessage += `\n- ${displayName}: ${boundCount} 个`
    })
  ...
  try {
    ...
    switch (account.platform) {
      ...
      case 'openai':
        ...
        break
      case 'gemini-api':
        endpoint = `/admin/gemini-api-accounts/${accountId}`
        break
      case 'droid':
