...
const buildAdminPath = (path = '') => {
  const raw = typeof path === 'string' ? path : ''
  if (!raw) {
    return adminBase.value
  }
  if (raw.startsWith('/admin/')) {
    return `${adminBase.value}${raw.slice('/admin'.length)}`
  }
  if (raw.startsWith('/admin')) {
    return `${adminBase.value}${raw.slice('/admin'.length)}`
  }
  if (raw.startsWith('/')) {
    return `${adminBase.value}${raw}`
  }
  return `${adminBase.value}/${raw}`
}

const adminApi = {
  get: (path, options) => apiClient.get(buildAdminPath(path), options),
  post: (path, data, options) => apiClient.post(buildAdminPath(path), data, options),
  put: (path, data, options) => apiClient.put(buildAdminPath(path), data, options),
  patch: (path, data, options) => apiClient.patch(buildAdminPath(path), data, options),
  delete: (path, options) => apiClient.delete(buildAdminPath(path), options)
}

const buildAdminApiUrl = (path) => createApiUrl(buildAdminPath(path))

provide('adminApi', adminApi)
provide('buildAdminApiUrl', buildAdminApiUrl)
...
const platformRequestHandlers = {
  claude: (params) => adminApi.get('/admin/claude-accounts', { params }),
  'claude-console': (params) => adminApi.get('/admin/claude-console-accounts', { params }),
  bedrock: (params) => adminApi.get('/admin/bedrock-accounts', { params }),
  gemini: (params) => adminApi.get('/admin/gemini-accounts', { params }),
  openai: (params) => adminApi.get('/admin/openai-accounts', { params }),
  azure_openai: (params) => adminApi.get('/admin/azure-openai-accounts', { params }),
  'openai-responses': (params) => adminApi.get('/admin/openai-responses-accounts', { params }),
  ccr: (params) => adminApi.get('/admin/ccr-accounts', { params }),
  droid: (params) => adminApi.get('/admin/droid-accounts', { params }),
  'gemini-api': (params) => adminApi.get('/admin/gemini-api-accounts', { params })
}
...
const handleBalanceScriptSaved = async () => {
  ...
  try {
    const res = await adminApi.get(`/admin/accounts/${account.id}/balance`, {
    ...
    console.warn('Failed to reload balance after saving script:', error)
  }
}
  ...
  try {
    const results = await Promise.all(
      eligibleTargets.map(async (account) => {
        try {
          const response = await adminApi.post(`/admin/accounts/${account.id}/balance/refresh`, {
            platform: account.platform
          })
...
const loadBalanceCacheForAccounts = async () => {
  ...
  const responses = await Promise.all(
    platforms.map(async (platform) => {
      try {
        const res = await adminApi.get(`/admin/accounts/balance/platform/${platform}`, {
          params: { queryApi: false }
        })
        return { platform, success: !!res?.success, data: res?.data || [] }
      } catch (error) {
        console.warn(`Failed to load balance cache for ${platform}:`, error)
...
const loadAccounts = async (forceReload = false) => {
  ...
  try {
    ...
    const platformResults = await Promise.all(
      platformsToFetch.map(async (platform) => {
        ...
        try {
          ...
          console.warn(`Failed to load ${platform} accounts:`, error)
          return { platform, success: false, data: [] }
        }
      })
    )
    ...
    if (openaiResponsesRaw.length > 0) {
      ...
      try {
        const configsRes = await adminApi.get(
        ...
        console.warn('Failed to load auto-recovery configs:', error)
      }
    ...
    loadBalanceCacheForAccounts().catch((err) => {
      console.warn('Balance cache loading failed:', err)
    })
  } catch (error) {
...
const loadClaudeUsage = async () => {
  try {
    const response = await adminApi.get('/admin/claude-accounts/usage')
...
const loadBindingCounts = async (forceReload = false) => {
  ...
  try {
    const response = await adminApi.get('/admin/accounts/binding-counts')
    if (response.success) {
...
const loadApiKeys = async (forceReload = false) => {
  ...
  try {
    const response = await adminApi.get('/admin/api-keys')
    if (response.success) {
...
const loadAccountGroups = async (forceReload = false) => {
  ...
  try {
    const response = await adminApi.get('/admin/account-groups')
    if (response.success) {
...
const performAccountDeletion = async (account) => {
  ...
  try {
    const data = await adminApi.delete(endpoint)
    if (data.success) {
...
const toggleSchedulable = async (account) => {
  ...
  try {
    ...
    const data = await adminApi.put(endpoint)
