...
# 获取数据存储类型（优先使用环境变量，否则从配置读取）
resolve_datastore_provider() {
    if [ ! -z "$DATASTORE_PROVIDER" ]; then
        echo "$DATASTORE_PROVIDER"
        return
    fi
    if [ ! -z "$DATASTORE_DRIVER" ]; then
        echo "$DATASTORE_DRIVER"
        return
    fi
    if command -v node > /dev/null 2>&1; then
        node -e "try{const cfg=require('./config/config');console.log((cfg.datastore&& (cfg.datastore.provider||cfg.datastore.driver))||'redis')}catch(e){console.log('redis')}" 2>/dev/null
        return
    fi
    echo "redis"
}

DATASTORE_PROVIDER=$(resolve_datastore_provider | tr '[:upper:]' '[:lower:]')
USE_REDIS=false
if [[ "$DATASTORE_PROVIDER" == *"redis"* ]]; then
    USE_REDIS=true
fi
...
if [ "$USE_REDIS" = true ]; then
    if command -v redis-cli > /dev/null 2>&1; then
        if $REDIS_CMD ping > /dev/null 2>&1; then
            ...
            echo "  ⚠️  Redis连接失败 ($REDIS_HOST:$REDIS_PORT) - 将跳过 Redis 统计"
        fi
    else
        echo "  ⚠️  redis-cli 未安装，跳过 Redis 统计"
    fi
else
    echo "  ⚠️  当前数据存储为 ${DATASTORE_PROVIDER:-unknown}，跳过 Redis 统计"
fi
...
if [ "$REDIS_AVAILABLE" = true ]; then
    ...
    echo "  ⚠️  Redis 未启用，跳过资源统计"
fi

# 详细模式下的使用统计
if [ "$DETAIL_MODE" = true ] && [ "$REDIS_AVAILABLE" = true ]; then
    echo ""
    echo "📈 使用统计："
...
echo "🔌 WebSocket 状态："
if command -v node > /dev/null 2>&1; then
    node scripts/status.js --ws
else
    echo "  ⚠️  node 未安装，无法获取 WS 状态"
fi
