...
- **Data Store**: Redis 6+ by default; optional SQLite (set `DATASTORE_PROVIDER=sqlite` and run `npm run setup:sqlite` before first run)
...
### Optional: Translation (Claude/OpenAI/Gemini)

- Enable: `TRANSLATION_ENABLED=true`, set `TRANSLATION_PROVIDER` / `TRANSLATION_FALLBACK_PROVIDER`
- Trigger: `x-translation-target` header or `translation_target` query param (optional `x-translation-source` / `translation_source`)
- Scope: Claude / OpenAI / Gemini routes only; no effect when disabled

Example:
```bash
curl -H "x-translation-target=en" \
