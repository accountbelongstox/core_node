...
export const Card = ({ children, className = '', onClick }: any) => (
  <div 
    onClick={onClick} 
    className={`holo-card rounded-[2rem] p-6 ${onClick ? 'cursor-pointer holo-card-hover' : ''} ${className}`}
  >
    {children}
  </div>
);
...
export const Button = ({ children, variant = 'primary', className = '', onClick }: any) => {
  const base = "w-full py-4 rounded-2xl font-bold tracking-wide transition-all duration-300 active:scale-[0.98] flex items-center justify-center gap-2";
  
  const styles: any = {
    primary: "bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-xl shadow-blue-500/30 border border-white/20 hover:shadow-blue-500/50",
    secondary: "glass-btn text-slate-700 dark:text-white hover:bg-white/40",
    danger: "bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-xl shadow-red-500/20 border border-white/20",
    ghost: "bg-transparent text-slate-500 hover:bg-white/10 hover:text-slate-700 dark:hover:text-white"
  };
