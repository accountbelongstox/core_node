@echo off
REM admin-vue-tailwind Install Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for dependency management
REM This bat file serves as the Windows entry point for installing admin-vue-tailwind dependencies
REM IMPORTANT: All Poly applications MUST use BAT triggers to call PS1 scripts

echo [INFO] Installing admin-vue-tailwind application dependencies...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] admin-vue-tailwind installation failed with exit code: %ERRORLEVEL%
    exit /b %ERRORLEVEL%
)

echo [SUCCESS] admin-vue-tailwind installation completed
exit /b 0
