@echo off
REM admin-vue-tailwind Start Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for poly-vue application startup
REM This bat file serves as the Windows entry point for starting admin-vue-tailwind application
...
echo [INFO] Starting admin-vue-tailwind application...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] admin-vue-tailwind startup failed with exit code: %ERRORLEVEL%
    exit /b %ERRORLEVEL%
)

echo [SUCCESS] admin-vue-tailwind startup completed
exit /b 0
