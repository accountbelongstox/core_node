...
namespace App\Apps\DictV1\DictV1Models;

use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Support\Facades\DB;
use App\Apps\DictV1\DictV1DBTablesBrige\DictV1TableMaps;

class DictV1DictionaryModel extends Model
{
    use HasFactory;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->table = DictV1TableMaps::getTableName('DICT_V1_DICTIONARIES');
    }
    ...
    public static function countByTranslation(){
        $isTranslationField = DictV1TableMaps::getFieldName('DICT_V1_DICTIONARIES', 'isTranslation');
        return self::where($isTranslationField, true)->count();
    }

    public static function countHasTranslation(){
        $translationField = DictV1TableMaps::getFieldName('DICT_V1_DICTIONARIES', 'translation');
        return self::where($translationField, '!=', null)->count();
    }

    public function incrementQueryCount()
    {
        $queryCountField = DictV1TableMaps::getFieldName('DICT_V1_DICTIONARIES', 'queryCount');
        $lastQueryTimeField = DictV1TableMaps::getFieldName('DICT_V1_DICTIONARIES', 'lastQueryTime');
    ...
    {
        $contentField = DictV1TableMaps::getFieldName('DICT_V1_DICTIONARIES', 'content');
        return self::where($contentField, $content)->first();
    }

    public static function findByMd5($md5)
    {
        $md5Field = DictV1TableMaps::getFieldName('DICT_V1_DICTIONARIES', 'md5');
