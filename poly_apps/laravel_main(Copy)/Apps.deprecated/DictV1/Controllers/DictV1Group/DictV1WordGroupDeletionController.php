...
class DictV1WordGroupDeletionController 
{
    public function isGroupNameExist($gname,$gid=null)
    {
        $uid = Auth::id();
        return $gid ? 
            DictV1WordGroupModel::where('gid', $gid)->where('uid', $uid)->first() :
            DictV1WordGroupModel::where('gname', $gname)->where('uid', $uid)->first();
    }
