...
namespace App\Apps\DictV1\Controllers\DictV1WordQurey;
 ...
 */
class DictV1UntranslatedWordsController extends BaseController
{
    ...
    public function getUntranslatedWords(Request $request)
    {
        ...
        try {
            ...
            $query = DictV1DictionaryModel::query();
    ...
    {
        try {
            $totalWords = DictV1DictionaryModel::count();
            
            $missingTranslation = DictV1DictionaryModel::where(function ($q) {
                $q->whereNull('translation')
                  ->orWhere('translation', '')
                  ->orWhere('translation', '{}')
                  ->orWhere('translation', '[]')
            ...
            $missingPhonetic = DictV1DictionaryModel::where(function ($q) {
                $q->where(function ($subQuery) {
                    $subQuery->whereNull('usPhonetic')
                             ->orWhere('usPhonetic', '');
                })->where(function ($subQuery) {
            ...
            $missingAudio = DictV1DictionaryModel::where(function ($q) {
                $q->whereNull('voice_files')
                  ->orWhere('voice_files', '')
                  ->orWhere('voice_files', '{}')
                  ->orWhere('voice_files', '[]');
            ...
            $missingImages = DictV1DictionaryModel::where(function ($q) {
                $q->whereNull('image_files')
                  ->orWhere('image_files', '')
                  ->orWhere('image_files', '{}')
                  ->orWhere('image_files', '[]');
            ...
            $completeWords = DictV1DictionaryModel::where('isTranslation', true)
    ...
    public function getWordsByPriority(Request $request)
    {
        ...
        try {
            $words = DictV1DictionaryModel::select([
                'id',
                'content',
                'queryCount',
                'lastQueryTime',
