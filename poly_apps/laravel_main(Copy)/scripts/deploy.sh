...
initialize_system_directories() {
    ...
    for dir in "${system_dirs[@]}"; do
        if [ ! -d "$dir" ]; then
            ...
            echo -e "${GREEN}[DEPLOY] OK Created: $dir${NC}"
        fi
    done
...
check_laravel_available() {
    ...
    if [ -z "$laravel_dir" ] || [ ! -d "$laravel_dir" ]; then
        echo -e "${RED}[DEPLOY] Laravel directory not found${NC}"
        return 1
    fi

    if [ ! -f "$laravel_dir/artisan" ]; then
        echo -e "${RED}[DEPLOY] Laravel artisan not found${NC}"
        return 1
    fi
    ...
    if [ ! -d "$laravel_dir/vendor" ]; then
        ...
        if [ $? -ne 0 ]; then
            echo -e "${RED}[DEPLOY] Failed to install composer dependencies${NC}"
            return 1
        fi
    fi
    ...
    if ! $USE_SUDO php artisan --version >/dev/null 2>&1; then
        echo -e "${RED}[DEPLOY] artisan command failed${NC}"
        return 1
    fi
...
initialize_laravel_directories() {
    ...
    local laravel_dir=$(get_laravel_dir)
    if [ -z "$laravel_dir" ]; then
        echo -e "${RED}[DEPLOY] Failed to get Laravel directory${NC}"
        return 1
    fi
...
check_initialization() {
    echo -e "${BLUE}[INIT] Checking Laravel initialization${NC}"

    local laravel_dir=$(get_laravel_dir)
    if [ -z "$laravel_dir" ] || [ ! -d "$laravel_dir" ]; then
        echo -e "${RED}[INIT] Laravel directory not found${NC}"
        return 1
    fi

    cd "$laravel_dir" || return 1

    echo -e "${BLUE}[INIT] Ensuring required directories exist...${NC}"
    ...
    for dir in "${required_dirs[@]}"; do
        if [ ! -d "$dir" ]; then
            $USE_SUDO mkdir -p "$dir" 2>/dev/null || true
            echo -e "${GREEN}[INIT] OK Created: $dir${NC}"
        fi
    done

    echo -e "${BLUE}[INIT] Setting proper permissions...${NC}"

    # Set permissions on storage and bootstrap/cache
    if [ "$IS_WSL" = true ]; then
        $USE_SUDO chmod -R 777 storage bootstrap/cache 2>/dev/null || true
    else
        $USE_SUDO chown -R $USER:www-data storage bootstrap/cache 2>/dev/null || true
        $USE_SUDO chmod -R 775 storage bootstrap/cache 2>/dev/null || true
    fi

    # Ensure .env file exists
    if [ ! -f ".env" ] && [ -f ".env.example" ]; then
        cp ".env.example" ".env"
        echo -e "${GREEN}[INIT] OK Created .env from .env.example${NC}"

        # Generate APP_KEY if not set
        if ! grep -q "^APP_KEY=base64:" ".env" 2>/dev/null; then
            $USE_SUDO php artisan key:generate --force >/dev/null 2>&1
            echo -e "${GREEN}[INIT] OK Generated APP_KEY${NC}"
        fi
    fi

    echo -e "${GREEN}[INIT] OK Initialization check complete${NC}"
    return 0
}
