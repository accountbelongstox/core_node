...
import { gdir, env, encyclopedia as globalEncyclopedia } from '../../../globalvars.js';
...
let debug_receive_event = false;
let debug_receive_execute_event = false;
...
class Httptool extends Base {
    ...
    async compareFileSizesSync(remoteUrl, localPath) {
    ...
    async eggSocket(method = null, params = null, callback = null, exclude = {}) {
        try {
            if (!eggSocket) {
                eggSocket = require('@egg/ee-core/socket');
                if (!eggSocketServer) {
    ...
    sendToWebSocket(event_name, data, rawData, toAll = true) {
        ...
        let receive_on = event_name;
        if (event_name && (event_name.includes(`:`) || event_name.includes(`.`))) {
            let receive_parse = event_name.split(/[\:\.]+/);
            main_class = receive_parse[0];
            receive_on = receive_parse[1];
        }
        let send_id = `send_to_view_` + this.create_id();
        data.main_class = main_class;
        data.receive_on = receive_on;
