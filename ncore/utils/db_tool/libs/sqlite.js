...
const { filterTables, getDbFileFromConfig } = require('../tools/sqlite_tool.js');
const { buildConditions } = require('../tools/sqlite_where_builder.js');
const { SQLITE_TYPES, inferType } = require('../types/sqlite_types.js');

class SQLite {
  constructor(configOrDbFile, initConfigFile = '', debug = false) {
    this.dbFile = typeof configOrDbFile === 'string' ? configOrDbFile : getDbFileFromConfig(configOrDbFile);
  ...
  async updateOne(tabname, data, conditions) {
    ...
    const [whereClause, values] = buildConditions(conditions);
  ...
  async updateMany(tabname, data, conditions) {
    ...
    const [whereClause, condValues] = buildConditions(conditions);
    const sql = `UPDATE ${tabname} SET ${setClause} WHERE ${whereClause}`;
  ...
  async getTableMaps() {
    ...
    const filteredTables = filterTables(tableNames, this.includeFilter, this.excludeFilter);
    const tableMaps = {};
  ...
  async createTable(tablename, fields) {
    ...
    const columns = Object.entries(fields).map(([name, info]) => {
      let columnDef = `${name} ${inferType(info.type)}`;
