...
class DomainContext {
  ...
  resolveHref(baseUrl, href) {
    if (!href || href.startsWith('javascript:') || href.startsWith('mailto:') || href.startsWith('tel:') || href.startsWith('data:') || href.startsWith('blob:')) {
      return null;
    }
  ...
  canonicalize(candidateUrl) {
    try {
      ...
      const params = Array.from(urlObject.searchParams.entries()).sort((a, b) => a[0].localeCompare(b[0]));
      params.forEach(([key, value]) => sortedParams.append(key, value));
      urlObject.search = sortedParams.toString();
  ...
  isInternalLink(candidateUrl) {
    ...
    const sameDomain = this.normalizeDomain(urlObject.hostname) === this.domainKey;
    if (!sameDomain) {
      return false;
    }
    return this.isWithinScope(urlObject);
  }

  isWithinScope(candidateUrl) {
    if (!candidateUrl) {
      return false;
    }
    let urlObject;
    ...
    let candidatePath = urlObject.pathname || '/';
    candidatePath = candidatePath.replace(/\/+/g, '/');
    if (!candidatePath.startsWith(this.scopePath)) {
      return false;
    }
    if (candidatePath === this.scopePath) {
      return true;
    }
    if (candidatePath.startsWith(this.scopePath + '/')) {
      return true;
