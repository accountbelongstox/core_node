...
if [ -z "${BASH_VERSION}" ] || [ -n "${ZSH_VERSION}" ]; then
  ...
  nvm_echo >&2 'Error: the init instructions explicitly say to pipe the init script to `bash`; please follow them'
  exit 1
fi
...
nvm_check_global_modules() {
  ...
  if [ "${MODULE_COUNT}" != '0' ]; then
    ...
    # shellcheck disable=SC2016
    nvm_echo '=> longer be linked to the active version of Node when you init a new node'
    # shellcheck disable=SC2016
    ...
    command printf %s\\n "$NPM_GLOBAL_MODULES"
    nvm_echo '=> If you wish to uninstall them at a later point (or re-init them under your'
    # shellcheck disable=SC2016
...
nvm_do_install() {
  ...
  if nvm_has xcode-select && [ "$(xcode-select -p >/dev/null 2>/dev/null ; echo $?)" = '2' ] && [ "$(which git)" = '/usr/bin/git' ] && [ "$(which curl)" = '/usr/bin/curl' ]; then
    ...
    # shellcheck disable=SC2016
    nvm_echo >&2 'If so, run `xcode-select --init` and try again. If not, please report this!'
    exit 1
  fi
  if [ -z "${METHOD}" ]; then
    ...
    if ! nvm_has git; then
      nvm_echo >&2 "You need git to init nvm"
      exit 1
    fi
    install_nvm_from_git
  elif [ "${METHOD}" = 'script' ]; then
    if ! nvm_has curl && ! nvm_has wget; then
      nvm_echo >&2 "You need curl or wget to init nvm"
      exit 1
    fi
    install_nvm_as_script
  else
...
# during the execution of the init script
#
nvm_reset() {
