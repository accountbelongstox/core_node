...
async function addWatchPath(dirPath) {
    ...
    if (isPathWatched(normalizedPath)) {
        log.warn(`[chokidar] Path already being watched: ${dirPath}`);
        return;
    }
...
async function initialize(waitForFirstScan = false) {
    if (isInitialized) {
        log.warn('[chokidar] File watcher already initialized');
        return;
    }
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = totalWaitTime; i > 0; i--) {
                log.info(`[chokidar] Waiting for other programs to add paths... ${i} seconds remaining`);
                await new Promise(r => setTimeout(r, 1000));
            }

            const pathsArray = Array.from(watchedPaths);
            scanStartTime = Date.now();
            addCount = 0;
            lastAddTime = scanStartTime;
            isFirstScanComplete = false;

            watcher = chokidar.watch(pathsArray, {
                persistent: true,
                ignoreInitial: false,
                depth: 0,
                awaitWriteFinish: {
            ...
            _setupWatchers();
            isInitialized = true;
            log.warn(`[chokidar] File watcher initialized with paths: ${pathsArray.join(', ')}`);
            ...
            const checkScanComplete = setInterval(() => {
                ...
                if (now - lastAddTime > 500) {
                    ...
                    const totalTime = (now - scanStartTime) / 1000;
                    log.info(`[chokidar] Initial scan completed: processed ${addCount} files in ${totalTime.toFixed(2)} seconds`);
                    initializePromise = null;
                    resolve();
                }
            }, 100);
        } catch (error) {
            log.error('[chokidar] Failed to initialize file watcher:', error);
            initializePromise = null;
            reject(error);
        }
    });
...
function addFileToWatchMap(filePath) {
    ...
    if (!parentPath) {
        log.warn(`[chokidar] File ${filePath} does not belong to any watched directory`);
        return false;
    }
    ...
    if (wasAdded) {
        ...
        if (!isFirstScanComplete) {
            ...
            if (addCount % 1000 === 0) {
                ...
                log.info(`[chokidar] Scanning: processed ${addCount} files in ${elapsed.toFixed(2)} seconds`);
            }
        }
    }
...
function _setupWatchers() {
    watcher
        .on('add', path => {
            const result = addFileToWatchMap(path);
            if (isFirstScanComplete) {
                log.info(`[chokidar] New file ${result ? 'added' : 'ignored'}: ${path}`);
            }
        })
        .on('change', path => {
            log.info(`[chokidar] Detected file change event: ${path}`);
            const result = addFileToWatchMap(path);
            if (isFirstScanComplete) {
                log.info(`[chokidar] File ${result ? 'updated' : 'unchanged'}: ${path}`);
            }
        })
        .on('unlink', path => {
            log.info(`[chokidar] Detected file remove event: ${path}`);
            _removeFromIndex(path);
            if (isFirstScanComplete) {
                log.info(`[chokidar] File removed: ${path}`);
            }
        })
        .on('error', error => log.error(`[chokidar] Watcher error: ${error}`))
        .on('ready', () => {
            if (!isFirstScanComplete) {
                isFirstScanComplete = true;
            ...
            log.info('[chokidar] Ready for file changes');
        });
}
