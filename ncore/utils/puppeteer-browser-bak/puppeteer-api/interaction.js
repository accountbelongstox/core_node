...
class PuppeteerInteraction {
     ...
     * @param {string|null} selector - CSS selector for element content, null for full page
     * @param {number} instanceId - Instance ID (default: 0)
     * @returns {string} Page or element content
     */
    async getContent(selector = null, instanceId = this.defaultInstanceId) {
        try {
            const instance = this.getInstance(instanceId);
            if (!instance) {
                throw new Error(`Puppeteer instance ${instanceId} not found`);
            ...
            let content;
            if (selector) {
                content = await page.$eval(selector, element => element.outerHTML);
                logger.info(`Element content retrieved: ${selector}`);
            } else {
                content = await page.content();
                logger.info('Full page content retrieved');
            }
    ...
    async getContentAndWait(selector, options = {}, instanceId = this.defaultInstanceId) {
        try {
            ...
            const content = await this.getContent(null, instanceId);
