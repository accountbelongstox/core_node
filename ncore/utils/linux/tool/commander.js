const { execSync, spawn } = require('child_process');
    ...
    function runExe(command, info = true) {
    ...
    function execPowerShell(command, info = false, cwd = null, no_std = false, env = null) {
        if (!powershellPath) {
    ...
    function pipeExecCmd(command, cwd = null, env = null) {
        return execCmd(command, true, cwd, true, env);
    }

    function execCmd(command, info = false, cwd = null, no_std = false, env = null) {
        if (env) {
    ...
    function spawnAsync(command, info = true, cwd = null, logname = null, callback, timeout = 5000, progressCallback = null) {
        let cmd = '';
        let args = [];
