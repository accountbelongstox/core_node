...
function createDefaultSESSIONSECRET() {
...
const defaultConfig = {
    ...
    SESSION_SECRET: process.env.SESSION_SECRET || createDefaultSESSIONSECRET(),
...
class ConfigManager {
    ...
    async #initConfig() {
        try {
            const { env } = await require('#@global_vars');
            if (env) {
            ...
            errorByDebug(`Failed to initialize configuration: ${error}`);
            throw error;
        }
    }
