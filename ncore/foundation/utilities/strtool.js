...
function cleanString(str, options = {}) {
    ...
    result = result.replace(illegalChars, replacement);
    return result;
}
function forceUtf8(input) {
    try {
        let str;
        ...
        str = heDecode(str);
        if (!isValidUtf8(str)) {
            str = iconv.decode(Buffer.from(str, 'binary'), 'utf8');
        }
        return str;
    }
    catch (error) {
        log.error('Error forceUtf8:', error);
        return input;
    }
}
...
function extractWords(doc, validateWord) {
    const splitsSymbol = /[^a-zA-Z0-9]+/;
    const docWords = doc.split(splitsSymbol);
    const uniqueWords = new Set();
    for (const word of docWords) {
        if (word.length > 0 && (!validateWord || validateWord(word))) {
