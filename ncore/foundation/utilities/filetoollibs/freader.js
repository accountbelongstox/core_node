...
function getFileSizeKB(filePath) {
    try {
        ...
        return (stats.size / 1024).toFixed(2); // Convert bytes to KB
    } catch (error) {
        logger.error('Error reading file size:', error);
        return 0;
    }
}
/**
 ...
 */
function readWithEncoding(filePath, encoding = null, info = false, isLargeFile, callback) {
    ...
    for (const enc of encodingsToTry) {
        try {
            if (isLargeFile) {
                let done = false;
                const readStream = fs.createReadStream(filePath, { encoding: enc });
                readStream.on('data', async (chunk) => {
                    if (callback) {
                        await callback({ content: chunk, done });
                    } else {
                        logger.error(`ReadLargeFile callback is null`);
                    }
                });
                readStream.on('end', async () => {
                    done = true;
                    if (info) logger.info(`ReadLargeFile done`);
                    if (callback) {
                ...
                if (info) {
                    logger.info(`Successfully read ${filePath} with ${enc} encoding`);
                }
                return { content: content.toString(), encoding: enc };
            }
        } catch (error) {
            if (info) {
                logger.warn(`Failed to read with ${enc} encoding:`, error.message);
            }
        }
    }
...
function readLargeText(filePath, callback, encoding = 'utf-8', info = false, largeSize = 500 * 1024) {
    filePath = getAbsolutePath(filePath);
    const isLargeFile = true;
    return readWithEncoding(filePath, encoding, info, isLargeFile, callback);
}
...
function readJson(filePath, encoding = 'utf-8', info = false) {
    ...
    const content = readText(filePath, encoding, info);
    if (info) {
        logger.info(`readJson: ${filePath}`);
    }
    if (content) {
        try {
            return JSON.parse(content);
        } catch (error) {
            logger.error(`Failed to parse JSON from ${filePath}:`, error.message);
        }
    }
    return {};
}
...
function getAbsolutePath(inputPath) {
    ...
    try {
        ...
        logger.error('Error getting absolute path:', error);
    }
    return absolutePath;
}
