...
function hasOnlyOneSubdirectory(dir) {
    try {
        ...
        logger.error(`hasOnlyOneSubdirectory-Error: ${error.message}`);
        return false;
    }
}
...
function scanDirectory(dirPath, options = {}, currentDepth = 0) {
    const {
        onlyDirs = false,
        onlyFiles = false,
        depth = 1,
    ...
    const normalizedExtensions = (() => {
        ...
        return extArray.map(ext => {
            ...
            return caseSensitive ? cleanExt : cleanExt.toLowerCase();
        });
    })();
    ...
    try {
        ...
        for (const item of items) {
            ...
            try {
                ...
                if (normalizedExtensions.length > 0) {
                    ...
                    const compareExt = caseSensitive ? itemExt : itemExt.toLowerCase();
                    if (!normalizedExtensions.includes(compareExt)) {
                        continue;
                    }
                }
                results.push(fullPath);
        ...
        logger.error(`Error scanning directory ${absolutePath}: ${error.message}`);
        return [];
    }
}
    ...
    try {
        ...
        for (const item of items) {
            ...
            try {
                ...
                logger.warn(`Failed to remove ${itemPath}: ${error.message}`);
        ...
        try {
            ...
            logger.warn(`Failed to recreate directory ${dirPath}: ${error.message}`);
        }
