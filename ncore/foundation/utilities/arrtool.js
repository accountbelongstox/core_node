...
const logger = {
    ...
    warn: message => isDebug && console.warn(message),
    success: message => isDebug && console.success(message),
    debug: message => isDebug && console.debug(message),
    log: message => isDebug && console.log(message),
}
...
function splitArrayTo2D(arr, rowLength) {
    if (!Array.isArray(arr) || typeof rowLength !== 'number' || rowLength <= 0) {
...
function generateRandomRectangle(x, y, w, h, safe = true) {
    if (safe) {
        ...
        logger.log(scaledDownX, scaledDownY, scaledDownW, scaledDownH);
    }
...
function smartDelayForEach(array, delay_time = 10, per, delay = 0) {
    if (!per) per = array.length > 20000 ? 10000 : 1000;
    return {
        forEach: function (callback) {
            return new Promise((resolve) => {
                let index = 0;
                const iterate = async () => {
                    while (index < array.length) {
                        ...
                        index++;
                        if (index % per === 0) {
                            logger.debug(`Delaying for ${delay_time}ms`);
...
function randomList(arr, length) {
