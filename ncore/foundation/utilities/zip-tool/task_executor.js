...
async function processTask(task, taskId) {
    ...
    try {
        ...
        debugLog(
            `[processTask:step8] Cleanup start - ${taskPath} (${formatSize(itemSize)}) - Duration: ${datetool.formatDurationToStr(taskDuration)} - Starting cleanup process`
        );
        ...
        if (state.getQueueLength() > 0) {
            debugLog(
                `[processTask:step13] Queue check - ${taskPath} (${formatSize(itemSize)}) - Duration: ${datetool.formatDurationToStr(taskDuration)} - Processing remaining queue items: ${state.getQueueLength()} remaining`
            );
            processQueue();
        } else {
            debugLog(
                `[processTask:step13] Queue complete - ${taskPath} (${formatSize(itemSize)}) - Duration: ${datetool.formatDurationToStr(taskDuration)} - No more items in queue`
            );
        }
    }
}
...
async function processQueue() {
    ...
    try {
        ...
        if (state.getQueueLength() === 0 && state.getActiveProcesses() === 0) {
            ...
            debugLog(
                `[processQueue:step6] Queue completion - Duration: ${datetool.formatDurationToStr(queueDuration)} - No remaining tasks and no active processes`
            );

            // Step 7: State cleanup
            state.setProcessing(false);
            debugLog(
                `[processQueue:step7] State cleanup - Duration: ${datetool.formatDurationToStr(queueDuration)} - Processing state set to false`
            );

            // Step 8: Final status
            debugLog(
                `[processQueue:step8] Final status - Duration: ${datetool.formatDurationToStr(queueDuration)} - All tasks completed successfully`
            );
        } else {
            // Step 6: Queue status
            debugLog(
                `[processQueue:step6] Queue status - Duration: ${datetool.formatDurationToStr(queueDuration)} - Remaining tasks: ${state.getQueueLength()}, Active processes: ${state.getActiveProcesses()}`
            );
        }
    }
}
