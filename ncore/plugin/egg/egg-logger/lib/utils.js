...
module.exports = {
  ...
  consoleFormatter(meta) {
    ...
    msg = msg.replace(duartionRegexp, `${colors.green}$1${colors.reset}`);
    msg = msg.replace(categoryRegexp, `${colors.blue}$1${colors.reset}`);
    msg = msg.replace(httpMethodRegexp, `${colors.cyan}$1 ${colors.reset}`);
    return msg;
  },
