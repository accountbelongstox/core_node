...
const { appdir } = require('#@ncore/globalvars');
const { fpath } = require('#@ncore/utils_linux');

function checkConfig(prop) {
  const filepath = fpath.ensure_absolute(prop,appdir);
...
function loadConfig(prop) {
  ...
  const configFile = fpath.ensure_absolute(prop,appdir);
...
function loadEncryptConfig() {
  ...
  const filepath = fpath.ensure_absolute(configFile,appdir);
  if (!fs.existsSync(filepath)) {
