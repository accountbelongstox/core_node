 ...
 */
 ...
 module.exports = {
   ...
   */
  run(options = {}) {
    console.log('[ee-init] [move] Start moving resources');
    ...
    for (let i = 0; i < flags.length; i++) {
      ...
      if (!cfg) {
        console.log(chalk.blue('[ee-init] [move] ') + chalk.red(`Error: ${f} config does not exist` ));
        return;
      }

      console.log(chalk.blue('[ee-init] [move] ') + chalk.green(`Move flag: ${f}`));
      console.log(chalk.blue('[ee-init] [move] ') + chalk.green('config:'), cfg);
      ...
      if (fs.statSync(distResource).isDirectory() && !fs.existsSync(targetResource)) {
        ...
        console.log('[ee-init] [move] Clear history resources:', targetResource);
      }

      fsPro.copySync(distResource, targetResource);

      // [todo] go project, special treatment of package.json, reserved only necessary
      console.log(`[ee-init] [move] Copy ${distResource} to ${targetResource}`);
    }

    console.log('[ee-init] [move] End');
  },
