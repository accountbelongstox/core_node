...
class IconGen {
  ...
  _init() {
    ...
    try {
      ...
      console.error("[ee-init] [icon-gen] args: ", args);
      console.error("[ee-init] [icon-gen] ERROR: ", e);
      throw new Error("参数错误!!");
    }
    this.params = params;

    // ---> 组装参数
    console.log("[ee-init] [icon-gen] icon 当前路径: ", process.cwd());
   ...
   */
  generateIcons() {
    console.log("[ee-init] [icon-gen] iconGen 开始处理生成logo图片");
    if (!fs.existsSync(this.input)) {
      console.error("[ee-init] [icon-gen] input: ", this.input);
    ...
    icongen(this.input, this.output, this.iconOptions)
      .then((results) => {
        console.log("[ee-init] [icon-gen] iconGen 已生成下方图片资源");
        console.log(results);
        this._renameForEE(results);
      })
      .catch((err) => {
        console.error(err);
        throw new Error("[ee-init] [icon-gen] iconGen 生成失败!");
      });
  }
  ...
  _renameForEE(filesPath) {
    console.log("[ee-init] [icon-gen] iconGen 开始重新命名logo图片资源");
    try {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if ([".png"].includes(extname)) {
          ...
          // 处理 trays 图标 --> 复制到 public/images 目录下
          if ("16" === basename) {
      ...
      console.log("[ee-init] [icon-gen] iconGen 资源处理完成!");
    } catch (e) {
      console.error("[ee-init] [icon-gen] ERROR: ", e);
