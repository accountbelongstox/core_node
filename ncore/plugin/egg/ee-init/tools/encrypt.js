...
class Encrypt {
  constructor(options = {}) {
    ...
    const configFile = options.config || path.join(appdir,'config/bin.js');
    ...
    this.specificFiles = [ path.join(appdir,'preload/bridge.js') ];

    // 旧属性，将废弃
    this.dirs = [];
    // const directory = this.config.directory || ['electron'];
    const directory = this.config.directory || [appdir];
    ...
    console.log(chalk.blue('[ee-init] [encrypt] ') + 'cleanFiles:' + this.cleanFiles);
  }
  ...
  backup() {
    ...
    console.log(chalk.blue('[ee-init] [encrypt] ') + 'backup start');
    if (this.patterns) {
      ...
      for (let i = 0; i < this.dirs.length; i++) {
        ...
        if (!fs.existsSync(codeDirPath)) {
          console.log('[ee-init] [encrypt] ERROR: backup %s is not exist', codeDirPath);
          return
        }

        // copy
        let targetDir = path.join(this.encryptCodeDir, this.dirs[i]);
        console.log('[ee-init] [encrypt] backup target Dir:', targetDir);
    ...
    console.log(chalk.blue('[ee-init] [encrypt] ') + 'backup end');
    return true;
  }
  ...
  cleanCode() {
    this.cleanFiles.forEach((file) => {
      ...
      console.log(chalk.blue('[ee-init] [encrypt] ') + 'clean up tmp files:' + chalk.magenta(`${tmpFile}`));
    })
  }
   ...
   */
  encrypt() {
    console.log(chalk.blue('[ee-init] [encrypt] ') + 'start ciphering');
    if (this.patterns) {
      ...
      // 旧逻辑，将废弃
      console.log('[ee-init] [encrypt] !!!!!! please use the new encryption method !!!!!!');
      for (let i = 0; i < this.dirs.length; i++) {
    ...
    console.log(chalk.blue('[ee-init] [encrypt] ') + 'end ciphering');
  };
  ...
  generate(curPath, type) {
    ...
    let tips = chalk.blue('[ee-init] [encrypt] ') + 'file: ' + chalk.green(`${curPath}`) + ' ' + chalk.cyan(`(${encryptType})`);
    console.log(tips);
