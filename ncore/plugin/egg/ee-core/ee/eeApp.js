...
import Log from '../log';
import CoreElectron from '../electron';
import CoreElectronApp from '../electron/app';
import CoreElectronWindow from '../electron/window';
...
class EeApp extends BaseApp {
  ...
  async selectAppType() {
    ...
    if (Ps.isDev()) {
      let modeInfo;
      let load = 'url';
      const configFile = path.join(appdir, 'config/bin.js');
      let electronCfg = {};
  ...
  loadLocalWeb(mode, staticDir) {
    ...
    const isHttps = mainServer.protocol === 'https://';
    if (isHttps) {
   ...
   */
  async _loderPreload() {
    const filepath = this.loader.resolveModule(path.join(this.config.baseDir, 'preload', 'index'));
    if (!filepath) return;
