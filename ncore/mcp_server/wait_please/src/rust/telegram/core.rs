...
/// Generate unified feedback message
pub fn build_feedback_message(
    ...
    if is_continue {
        // Continue operation feedback message
        let continue_prompt = if let Ok(config) = crate::config::load_standalone_config() {
            config.reply_config.continue_prompt
        } else {
            "Please continue with best practices".to_string()
        };

        format!("Send successful!\n\nSelected options:\n• Continue: {}", continue_prompt)
    } else {
        // Send operation feedback message
        let mut feedback_message = "Send successful!\n\nSelected options:\n".to_string();

        if selected_options.is_empty() {
            feedback_message.push_str("• None");
        } else {
