...
pub async fn send_mcp_response(
    ...
    if is_mcp_mode {
        // MCP mode: output directly to stdout (MCP protocol requirement)
        println!("{}", response_str);
        std::io::Write::flush(&mut std::io::stdout())
            .map_err(|e| format!("Failed to flush stdout: {}", e))?;
    } else {
        // Send response through channel (if available)
        let sender = {
            let mut channel = state
                .response_channel
                .lock()
                .map_err(|e| format!("Failed to get response channel: {}", e))?;
            channel.take()
        };
