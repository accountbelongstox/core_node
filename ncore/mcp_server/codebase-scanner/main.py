...
def main():
    ...
    try:
        print("[MAIN] Starting FastMCP Codebase Scanner...")

        # Initialize scanner with project root and global access enabled
        project_root = os.environ.get("PROJECT_ROOT", "D:/programing/core_node")
        ...
        print(f"[SUCCESS] Scanner initialized")
        print(f"[CONFIG] Project root: {scanner.project_root}")
        print(f"[CONFIG] Global access: {'ENABLED' if scanner.enable_global_access else 'DISABLED'}")
        ...
        def generate_directory_tree(target_path: str = "", max_depth: int = 5, output_format: str = "both") -> str:
            ...
            """
            try:
                print(f"[TOOL] generate_directory_tree called: {target_path or 'project_root'}, depth={max_depth}, format={output_format}")
                ...
                print(f"[SUCCESS] generate_directory_tree completed")
                return json.dumps(result, indent=2, ensure_ascii=True)

            except Exception as e:
                error_result = {"error": f"Failed to generate tree: {str(e)}"}
                print(f"[ERROR] generate_directory_tree failed: {e}")
        ...
        def find_file_by_name(filename: str, search_path: str = "", exact_match: bool = False, max_results: int = 100) -> str:
            ...
            """
            try:
                print(f"[TOOL] find_file_by_name called: {filename}, path={search_path or 'project_root'}")

                path = search_path if search_path else None
                result = scanner.find_files_by_name(filename, path, exact_match, max_results)

                print(f"[SUCCESS] find_file_by_name completed - {result.get('count', 0)} files found")
                return json.dumps(result, indent=2, ensure_ascii=True)

            except Exception as e:
                error_result = {"error": f"Failed to find files: {str(e)}"}
                print(f"[ERROR] find_file_by_name failed: {e}")
                return json.dumps(error_result, indent=2)
        ...
        def search_content_in_files(search_text: str, search_path: str = "", file_pattern: str = "",
            ...
            """
            try:
                print(f"[TOOL] search_content_in_files called: '{search_text}', path={search_path or 'project_root'}")
                ...
                print(f"[SUCCESS] search_content_in_files completed - {result.get('count', 0)} files found")
                return json.dumps(result, indent=2, ensure_ascii=True)

            except Exception as e:
                error_result = {"error": f"Failed to search content: {str(e)}"}
                print(f"[ERROR] search_content_in_files failed: {e}")
                return json.dumps(error_result, indent=2)
        ...
        def get_codebase_stats(target_path: str = "") -> str:
            ...
            """
            try:
                print(f"[TOOL] get_codebase_stats called: {target_path or 'project_root'}")

                path = target_path if target_path else None
                result = scanner.get_stats(path)

                print(f"[SUCCESS] get_codebase_stats completed")
                return json.dumps(result, indent=2, ensure_ascii=True)

            except Exception as e:
                error_result = {"error": f"Failed to get stats: {str(e)}"}
                print(f"[ERROR] get_codebase_stats failed: {e}")
                return json.dumps(error_result, indent=2)
        ...
        def scan_flutter_apps() -> str:
            ...
            """
            try:
                print("[TOOL] scan_flutter_apps called")
                ...
                print(f"[SUCCESS] scan_flutter_apps completed - {len(apps)} apps found")
                return json.dumps(result, indent=2, ensure_ascii=True)

            except Exception as e:
                error_result = {"error": f"Failed to scan Flutter apps: {str(e)}"}
                print(f"[ERROR] scan_flutter_apps failed: {e}")
                return json.dumps(error_result, indent=2)
        ...
        def print_flutter_app_structure(app_name: str, mode: str = "code_assets", max_depth: int = 10) -> str:
            ...
            """
            try:
                print(f"[TOOL] print_flutter_app_structure called: {app_name}, mode={mode}")
                ...
                print(f"[SUCCESS] print_flutter_app_structure completed")
                return json.dumps(result, indent=2, ensure_ascii=True)

            except Exception as e:
                error_result = {"error": f"Failed to print Flutter app structure: {str(e)}"}
                print(f"[ERROR] print_flutter_app_structure failed: {e}")
                return json.dumps(error_result, indent=2)

        @mcp.tool()
        def health_check() -> str:
            """Check server health and return configuration

            Returns:
                Server health information and configuration
            """
            try:
...
def init():
    ...
    print("[INIT] Initializing Codebase Scanner MCP Server...")
