  ...
  class BingWordParse {
    ...
    constructor(options = {}) {
      ...
      if (typeof pinyinPro == "undefined") {
        logger.warn("pinyinPro is undefined");
      }
    ...
    get_array_plusvalue(array, index) {
    ...
    createHtml(data, pre_render = false, options = {}) {
      let { brief_mode = false, project_mode = true } = options;
      let wordbox_html = "";
      let index = 0;
      ...
      for (const [word, wordItem] of entries) {
        ...
        let id = wordItem.id;
        let advanced_translate = this.get_word_propertytounicode(
          translation,
          "advanced_translate",
        );
        let advanced_translate_type = this.get_word_propertytounicode(
          translation,
        ...
        let plural_form = this.get_word_propertytounicode(
          translation,
          "plural_form",
        );
        let image_files = this.toJSON(wordItem.image_files);
        let synonyms = this.get_word_propertytounicode(translation, "synonyms");
        let synonyms_type = this.get_word_propertytounicode(
          translation,
          "synonyms_type",
        );
        let word_translation = this.get_word_propertytounicode(
          translation,
          "word_translation",
          "translate_text",
          [],
        );
        let translation_html = this.create_translation_html(
          word,
          word_translation,
          project_mode,
        );
        let phonetic_symbol_html = this.create_phonetic_symbol_html(
          wordItem,
          voice_files,
          index,
          phonetic_symbol,
        ...
        let haveread_html = this.create_haveread_html(
          word,
          id,
          word_translation,
          last_readtime,
        ...
        if (!brief_mode && !project_mode) {
          advanced_translate_html = this.create_advanced_html(
            word,
            word_id,
            advanced_translate_type,
            advanced_translate,
          );
          synonyms_html = this.create_synonyms_html(
            word,
            word_id,
            synonyms_type,
            synonyms,
    ...
    create_synonyms_html(
      ...
      advanced_translate_type.forEach((item, index) => {
        ...
        let advanced_translate_html = this.analyze_advanced_translate(
          advanced_translate_item,
          "",
          "code",
          word,
    ...
    create_phonetic_symbol_html(wordItem, voice_files, index, phonetic_symbol) {
      ...
      for (let [key, voice_static_val] of Object.entries(voice_files)) {
        ...
        const static_url = this.joinUrl(
          this.BASE_STAIC_URL,
          `${this.STATIC_PATH}/${voice_static_val}`,
        );
        html += `
    ...
    get_word_propertytounicode(
      word_json,
      key,
      defult_value = [],
      default_valuenew,
    ) {
      let value = this.get_word_property(
        word_json,
        key,
        defult_value,
        default_valuenew,
    ...
    createTextCSS() {
      return `
          ...
          height: 480px; /* or your preferred fixed height */
