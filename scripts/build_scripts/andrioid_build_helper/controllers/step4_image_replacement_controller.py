...
class Step4ImageReplacementController:
    ...
    def initialize(self, temp_build_root: Path, app_name: str, auto_mode: bool = False) -> bool:
        """
            ...
            for density_dir, target_size in targets.items():
                ...
                if smart_resize_enabled:
                    ...
                    if resize_result.get('success', False):
                        ...
                        print(f"[{self.step_name}] [IC_LAUNCHER] [OK] Smart Resized: {target_file}")
                    else:
                        results['skipped_count'] += 1
                        print(f"[{self.step_name}] [IC_LAUNCHER] [FAIL] Smart Resize Failed: {resize_result.get('error', 'Unknown error')}")
                else:
                    ...
                    try:
                        ...
                        print(f"[{self.step_name}] [IC_LAUNCHER] [OK] Direct Copy: {target_file}")
                    except Exception as copy_error:
                        results['skipped_count'] += 1
                        print(f"[{self.step_name}] [IC_LAUNCHER] [FAIL] Copy Failed: {copy_error}")
            ...
            for target in matching_targets:
                ...
                if smart_resize_enabled and recommended_size[0] > 0 and recommended_size[1] > 0:
                    ...
                    if resize_result.get('success', False):
                        ...
                        print(f"[{self.step_name}] [{step4_image_type}] [OK] Smart Resized: {target_path.name}")
                    else:
                        results['skipped_count'] += 1
                        print(f"[{self.step_name}] [{step4_image_type}] [FAIL] Smart Resize Failed: {resize_result.get('error', 'Unknown error')}")
                else:
                    # Direct copy without resizing
                    try:
                        ...
                        print(f"[{self.step_name}] [{step4_image_type}] [OK] Direct Copy: {target_path.name}")
                    except Exception as copy_error:
                        results['skipped_count'] += 1
                        print(f"[{self.step_name}] [{step4_image_type}] [FAIL] Copy Failed: {copy_error}")
    ...
    def _show_smart_resize_menu(self) -> None:
        ...
        try:
            ...
            def format_menu_item(item: Dict, index: int) -> str:
                ...
                return f"[OK] {item['image_type'].upper()}: {item['final_filename']} {compression_display} {resize_display} {target_info}"
