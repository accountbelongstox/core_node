        ...
        for i, image_type in enumerate(image_group):
            if i == 0:
                ...
                if found_image:
                    ...
                    PrintHelper.success(f"  [OK] First image {image_type} found, constraining to: {constraint_dir}")
            else:
                ...
                if found_image:
                    results[image_type] = found_image
                    PrintHelper.success(f"  [OK] {image_type} found in constraint directory")
                    ...
                    PrintHelper.warning(f"  [MISS] {image_type} not found and no fallback available")
    ...
    def _print_search_results_summary(self, search_results: Dict):
        ...
        if icon_results:
            ...
            for image_type, result in icon_results.items():
                ...
                PrintHelper.success(f"  [OK] {image_type.upper()}: {result['filename']} [{result['source_type']}] (pattern: {matched_pattern}, priority: {format_priority})")
            if constraint_dir:
