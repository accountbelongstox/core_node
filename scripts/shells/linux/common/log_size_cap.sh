#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# log_size_cap.sh - Bound the size of append-only files under the core_node data root.
#
# The core_node data root is the shared Linux runtime base (pycore
# core_node_dirs.get_core_node_data_dir(); shell CORE_NODE_DATA_DIR).
...
#
# Usage:
#   bash log_size_cap.sh                       # cap *.log under $CORE_NODE_DATA_DIR (10MB each)
#   bash log_size_cap.sh /www/core_node 10485760
...
# Install a systemd timer that caps core_node data-root logs every LSC_TIMER_INTERVAL.
lsc_install_timer() {
...
[Unit]
Description=Cap core_node data-root log file sizes
[Service]
Type=oneshot
...
[Unit]
Description=Periodic core_node data-root log size cap
[Timer]
OnBootSec=5min
