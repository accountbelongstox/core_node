...
.DESCRIPTION
    Auto-discovered & run by prepare.ps1 (it runs every install_*.ps1 next to it,
    passing -Python <interpreter>). Installs the libraries pycore uses to extract
...
foreach ($pkg in $Packages) {
    ...
    $ok = Invoke-Pip -Py $Python -PipArgs @($pkg.Pip)
