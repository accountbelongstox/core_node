...
# passing --python <interpreter>). Installs the system tools the launcher needs:
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --python) PYTHON="${2:-}"; shift; [ $# -gt 0 ] && shift ;;   # accepted for prepare.sh compat; unused (guard $2 under set -u)
        --force)  FORCE=1;     shift   ;;
