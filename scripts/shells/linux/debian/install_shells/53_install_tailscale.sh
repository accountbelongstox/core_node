...
SCRIPT_TEMP_DIR=$(create_script_temp_dir "54_install_tailscale")
...
main() {
    ...
    # CONSOLIDATED: 65_install_tailscale.sh is the canonical, gated Tailscale
    ...
    if [ -f "$SCRIPT_CURRENT_DIR/65_install_tailscale.sh" ]; then
        log_message "Superseded by 65_install_tailscale.sh; skipping to avoid a double install."
        exit 0
    fi
