...
# 31_ensure_php85_intelligent.sh) and every PHP extension is provisioned via
...
print_step_from_common_functions "47_ensure_php_pgsql: ensuring pdo_pgsql / pgsql for PHP"
...
if [ -z "$PHP_BIN" ]; then
    print_error_from_common_functions "php not found. Run 31_ensure_php85_intelligent.sh first."
    exit 1
fi
