...
cleanup_old_php_versions() {
    ...
    if command -v php >/dev/null 2>&1; then
        ...
        if [[ "$current_version" == "8.5"* ]]; then
            echo -e "${GREEN}$SCRIPT_INDEX Verification: 'php' command points to PHP 8.5 âœ?{NC}"
        else
...
verify_php_symbolic_link_fix() {
    ...
    if command -v php >/dev/null 2>&1; then
        ...
        if [[ "$php_version" == "8.5"* ]]; then
            ...
            echo -e "${RED}$SCRIPT_INDEX ï¿?PHP command version incorrect: $php_version (expected: 8.5.x)${NC}"
            verification_passed=false
        fi
    else
...
verify_php_symbolic_link_fix() {
    ...
    if command -v php >/dev/null 2>&1; then
        ...
        if [[ "$php_version" == "8.5"* ]]; then
            ...
            echo -e "${RED}$SCRIPT_INDEX ï¿?PHP command version incorrect: $php_version (expected: 8.5)${NC}"
            success=false
        fi
    else
        echo -e "${RED}$SCRIPT_INDEX ï¿?PHP command not available${NC}"
...
display_final_status() {
    ...
    if systemctl is-active --quiet php8.5-fpm 2>/dev/null; then
        ...
        echo -e "${GREEN}$SCRIPT_INDEX php8.5-fpm service: [NOT RUNNING] âœ?{NC}"
    fi
...
main() {
    ...
    # STEP 1: ALWAYS clean up old PHP versions (ç²¾ç»†åŒ–ä¿®å¤ç¬¬1é¡?
    echo -e "${BLUE}$SCRIPT_INDEX [STEP 1/5] Cleaning up old PHP versions...${NC}"
    cleanup_old_php_versions || {
        echo -e "${YELLOW}$SCRIPT_INDEX Old PHP cleanup completed with warnings${NC}"
    }

    # STEP 2: ALWAYS try to fix missing extensions if PHP 8.5 exists (ç²¾ç»†åŒ–ä¿®å¤ç¬¬2é¡?
    if command -v php8.5 >/dev/null 2>&1; then
    ...
    # STEP 3: ALWAYS fix symbolic link (ç²¾ç»†åŒ–ä¿®å¤ç¬¬3é¡?
    ...
    # STEP 4: ALWAYS verify symbolic link fix (ç²¾ç»†åŒ–ä¿®å¤ç¬¬4é¡?
    ...
    # STEP 5: If not fully installed, run full installation (ç²¾ç»†åŒ–ä¿®å¤ç¬¬5é¡?
