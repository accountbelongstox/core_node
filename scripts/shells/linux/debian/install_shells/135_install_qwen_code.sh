...
# 15_install_node_24.sh), so it lands in the shared NODE_BIN_DIR and is available to
# ALL users. 15_install_node_24.sh chmod-777s the node dir so non-root users can run
