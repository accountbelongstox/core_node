...
#!/bin/bash
# Cursor launcher (generated by 122_install_cursor.sh -- do not edit).
...
# IME bridge (Electron/Chromium; idempotent with 131_install_chinese_wubi.sh)
...
# 131_install_chinese_wubi.sh calls after a Wubi framework switch so the Cursor
...
install_cursor() {
    ...
    # IME bridge first (mirrors 123_install_vscode.sh), then ASK before reinstalling.
    ...
    print_info_from_common_functions "  - Wubi/CJK IME: run 131_install_chinese_wubi.sh first, or re-run 122 to refresh IME config"
...
main() {
    ...
    # reinstalling; used by 131_install_chinese_wubi.sh). Default (no arg) = idempotent
