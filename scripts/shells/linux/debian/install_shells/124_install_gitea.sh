...
#
# Prerequisites:
#   - Git must be installed (run 22_install_git_ssh.sh first)
#   - SQLite3 (automatically installed)
#   - wget/curl (automatically installed)
#
# Usage:
#   ./125_install_gitea.sh   # Normal installation (no arguments)
#
...
check_git_installation() {
    ...
    if command -v git >/dev/null 2>&1; then
        ...
        print_info_from_common_functions "Please run script 22_install_git_ssh.sh first, or install Git manually"
...
install_gitea() {
    ...
    if ! check_git_installation; then
        ...
        print_info_from_common_functions "Please install Git first by running: ./22_install_git_ssh.sh"
        return 1
    fi
