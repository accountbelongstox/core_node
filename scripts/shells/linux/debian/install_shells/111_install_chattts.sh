...
TARGET_DIR="${CHATTTS_DIR:-$CORE_NODE_CACHE_DIR/pycore/chattts}"
...
[[ "${CHATTTS_SKIP:-0}" == "1" ]] && { echo "[install_chattts] [i] CHATTTS_SKIP=1 -> skipping."; complete_prereq_step "$PYTHON" "[install_chattts] " ChatTTS; }
if server_up; then
...
if py_has_module "$PYTHON" ChatTTS && [[ -f "$DEPS_SENTINEL" && -f "$API_DST" && "$FORCE" -eq 0 && "$DO_FULL" -eq 0 ]]; then
