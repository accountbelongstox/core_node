...
# prerequisite installer 19_install_default_pipx.sh ($COMPILE_DIR/pipx_venv/bin/pipx);
...
PIPX_ENSURE_SCRIPT="$SCRIPT_CURRENT_DIR/19_install_default_pipx.sh"
# Absolute paths owned by the prerequisite installer (19_install_default_pipx.sh).
...
# (delegates to 19_install_default_pipx.sh when the binary is absent, then trusts the
# resolved path).
ensure_pipx() {
    if [ -x "$PIPX_BIN" ]; then
...
# STEP 1: pipx itself (absolute path owned by 19_install_default_pipx.sh)
