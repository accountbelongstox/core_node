...
#
# Usage:
#   ./63_install_tailscale.sh   # Normal installation (no arguments)
#
...
# Install / upgrade the Tailscale package using the official installer.
# The official script adds the correct apt repository for Debian/Ubuntu and
# installs both the `tailscale` CLI and the `tailscaled` daemon.
install_tailscale_package() {
    ...
    if curl -fsSL "$TAILSCALE_INSTALL_URL" | $USE_SUDO sh; then
        print_success_from_common_functions "Tailscale package installed"
        return 0
    fi

    print_error_from_common_functions "Tailscale installer failed (URL: $TAILSCALE_INSTALL_URL)"
    return 1
}
