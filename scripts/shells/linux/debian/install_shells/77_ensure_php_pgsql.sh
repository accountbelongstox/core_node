...
# 96_configure_php85.sh) and every PHP extension is provisioned via
...
if [ -z "$PHP_BIN" ]; then
    print_error_from_common_functions "php not found. Run 96_configure_php85.sh first."
    exit 1
fi
...
#     96_configure_php85.sh (Ubuntu -> ondrej PPA, Debian -> Sury), but
