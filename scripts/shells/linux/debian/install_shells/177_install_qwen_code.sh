...
# 17_install_node_toolchain_24.sh), so it lands in the shared NODE_BIN_DIR and is available to
# ALL users. 17_install_node_toolchain_24.sh chmod-777s the node dir so non-root users can run
...
if [ -z "$NPM_BIN_RESOLVED" ]; then
    echo "[$SCRIPT_INDEX] [ERROR] npm not found. Run 17_install_node_toolchain_24.sh first."
    return
fi
