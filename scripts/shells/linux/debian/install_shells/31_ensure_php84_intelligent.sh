...
if [ "$INSTALL_PHP" != "true" ]; then
    echo -e "${YELLOW}$SCRIPT_INDEX PHP installation is disabled (INSTALL_PHP: $INSTALL_PHP). Cleaning up repositories...${NC}"
    
    # Remove PHP repository files
    ...
    echo -e "${GREEN}$SCRIPT_INDEX PHP repositories cleaned up${NC}"
    exit 0
fi
...
cleanup_old_php_versions() {
    ...
    if command -v php >/dev/null 2>&1; then
        ...
        if [[ "$current_version" == "8.4"* ]]; then
            echo -e "${GREEN}$SCRIPT_INDEX Verification: 'php' command points to PHP 8.4 �?{NC}"
        else
...
# Prevent Apache2 conflicts during PHP installation - now using common function
prevent_apache2_conflicts() {
    prevent_apache2_conflicts_from_common_functions "$SCRIPT_INDEX"
}
...
install_php_core() {
    ...
    # Step 1: Install PHP 8.4 core package
    echo -e "${YELLOW}$SCRIPT_INDEX Step 1: Installing php8.4...${NC}"
    if ! dpkg -l | grep -q "^ii.*php8.4[[:space:]]"; then
        if $USE_SUDO apt install php8.4 -y 2>/dev/null || $USE_SUDO apt install php8.4 -y --allow-unauthenticated; then
    ...
    # Prevent Apache2 installation by holding apache2 packages
    ...
    $USE_SUDO apt-mark unhold apache2 apache2-bin apache2-data apache2-utils libapache2-mod-php* 2>/dev/null || true
...
install_php_fpm() {
    ...
    if dpkg -l | grep -q "^ii.*apache2"; then
        ...
        $USE_SUDO systemctl stop apache2 2>/dev/null || true
        $USE_SUDO systemctl disable apache2 2>/dev/null || true
        $USE_SUDO apt remove --purge apache2* libapache2-mod-php* -y 2>/dev/null || true
        $USE_SUDO apt autoremove -y 2>/dev/null || true
    fi
...
execute_installation() {
    ...
    # Step 0: Prevent Apache2 conflicts
    prevent_apache2_conflicts || {
        echo -e "${YELLOW}$SCRIPT_INDEX Apache2 cleanup completed with warnings${NC}"
    }
