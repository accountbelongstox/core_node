...
#!/bin/bash
# Cursor launcher (generated by 121_install_cursor.sh -- do not edit).
...
install_cursor() {
    ...
    # Idempotent: if Cursor is already installed at the current upstream version, do
    # nothing; otherwise (older/unknown version) clean up and reinstall automatically.
    if is_cursor_installed; then
        ...
        # Up to date, OR the latest version cannot be confirmed (offline / API change):
        # KEEP the working install -- never wipe it on an UNKNOWN remote (that would
        # delete a good install and then fail to re-download while offline). Only update
        # when the remote version is KNOWN and differs from what is installed.
        if [[ -z "$remote_version" ]] || [[ "$installed_version" == "$remote_version" ]]; then
            if [[ -z "$remote_version" ]]; then
                ...
                print_success_from_common_functions "Cursor $installed_version is already up to date - nothing to do (idempotent)."
            fi
            # (Re)assert the launcher + desktop entry so they stay correct/present.
            create_desktop_entry || true
            return 0
        fi

        print_step_from_common_functions "Updating Cursor (${installed_version:-unknown} -> ${remote_version}); removing old install..."
        cleanup_cursor
...
main() {
    # Non-interactive & idempotent. Cleanup happens ONLY when explicitly requested via
    # an argument (so the unattended install workflow never blocks on a prompt); the
    # default action is an idempotent install/update.
    case "${1:-}" in
