...
check_pnpm_availability() {
    if command -v pnpm >/dev/null 2>&1; then
        ...
        log_success "pnpm is available (version: $pnpm_version)"
    else
        PNPM_AVAILABLE=false
...
install_pnpm() {
    if [[ "$PNPM_AVAILABLE" == true ]]; then
        log_info "pnpm is already available"
        return 0
    fi
...
main() {
    ...
    if [[ "$PNPM_AVAILABLE" == false ]]; then
        if ! install_pnpm; then
            log_error "Failed to install pnpm, will proceed with NPM if available"
        fi
    fi
    ...
    log_success "Node modules and pnpm setup completed successfully!"
}
