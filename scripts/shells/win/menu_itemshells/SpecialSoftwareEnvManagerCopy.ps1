...
function Save-SecretToManager {
    ...
    try {
        ...
        $secretFile = Join-Path $dirs.RAW_DIR $KeyName
        
        Set-Content -Path $secretFile -Value $Value -Encoding UTF8 -Force | Out-Null
        
        Write-ColorMessage -Message "Saved secret to SecretManager: $KeyName" -Type "Success"
        return $true
    } catch {
        Write-ColorMessage -Message "Failed to save secret to SecretManager: $KeyName - $($_.Exception.Message)" -Type "Warning"
        return $false
    }
}
    ...
    for ($i = 0; $i -lt $script:CurrentConfig.Variables.Count; $i++) {
        ...
        if ($skipNext) {
            ...
            if ($autoValue) {
                ...
                # Save to SecretManager with key pattern: {VariableName}_{FileName} (without .bat extension)
                if (-not [string]::IsNullOrWhiteSpace($script:CurrentFileName)) {
                    $fileNameWithoutExt = [System.IO.Path]::GetFileNameWithoutExtension($script:CurrentFileName)
                    $secretKeyName = "$($var.Name)_$fileNameWithoutExt".ToUpper()
                    Save-SecretToManager -KeyName $secretKeyName -Value $autoValue | Out-Null
                }
                continue
            }
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($userInput)) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($script:CurrentFileName)) {
                $fileNameWithoutExt = [System.IO.Path]::GetFileNameWithoutExtension($script:CurrentFileName)
                $secretKeyName = "$($var.Name)_$fileNameWithoutExt".ToUpper()
                Save-SecretToManager -KeyName $secretKeyName -Value $userInput | Out-Null
            }
        } else {
            ...
            if ($hasCurrentValue) {
                ...
                switch ($choice) {
                    ...
                    default {
                        ...
                        # Save to SecretManager with key pattern: {VariableName}_DEFAULT
                        if (-not [string]::IsNullOrWhiteSpace($currentValues[$var.Name])) {
                            $secretKeyName = "$($var.Name)_DEFAULT".ToUpper()
                            Save-SecretToManager -KeyName $secretKeyName -Value $currentValues[$var.Name] | Out-Null
                        }
                    }
                }
            } else {
                # Check for default value if no current value and no user input
                    ...
                    if ($defaultValue) {
                        ...
                        # Save to SecretManager with key pattern: {VariableName}_DEFAULT
                        $secretKeyName = "$($var.Name)_DEFAULT".ToUpper()
                        Save-SecretToManager -KeyName $secretKeyName -Value $defaultValue | Out-Null
                    } else {
            ...
            if ($newValues.ContainsKey($var.Name)) {
                if ($newValues[$var.Name] -eq "__DELETE__") {
                    ...
                    if ($success) {
                        ...
                        # Ensure value is saved to SecretManager (in case it wasn't saved during input)
                        if ($newValues[$var.Name] -ne "" -and $newValues[$var.Name] -ne "__DELETE__") {
                            $secretKeyName = "$($var.Name)_DEFAULT".ToUpper()
                            Save-SecretToManager -KeyName $secretKeyName -Value $newValues[$var.Name] | Out-Null
                        }
                    }
                }
            }
