...
<#
.SYNOPSIS
    AI Management actions (Claude Code Agent Teams) - sourceable, no menu loop.
.DESCRIPTION
    Holds the reusable action functions previously inlined in AIManagementMenu.ps1
    so BOTH the standalone AI menu and the unified AI & MCP menu can dot-source
    them without duplicating the wizard. Dot-source this file to use.
#>

#region Variable Declarations
$script:AI_ACTIONS_DIR = $PSScriptRoot
$script:CLAUDE_ENV_PS1 = Join-Path $script:AI_ACTIONS_DIR "ClaudeAssistantEnv.ps1"
...
if (-not (Test-Path -LiteralPath $script:CLAUDE_ENV_PS1)) {
    Write-Host "[X] ClaudeAssistantEnv.ps1 not found: $script:CLAUDE_ENV_PS1" -ForegroundColor $script:COLOR_ERROR
} else {
