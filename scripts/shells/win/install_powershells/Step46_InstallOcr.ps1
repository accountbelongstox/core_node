...
function Test-PyModule {
    ...
    # swallow it inside Python and signal via a stdout marker (not exit code).
...
$easyocrSkip = Join-Path $Global:CORE_NODE_CACHE_DIR 'ocr\.easyocr_skip'
...
if ((Test-PyModule -Py $Python -Module 'winrt.windows.media.ocr') -and -not $Force) {
    ...
    $ok = Invoke-Pip -PipArgs $WinrtOcrPackages
    if (-not $ok) {
...
# ai-vision remain). Always exit 0 so PreparePycorePrerequisites.ps1 continues to launch.
exit 0
