...
.PARAMETER Python
    Accepted for PreparePycorePrerequisites.ps1 compatibility; unused (this is a Node prerequisite).
...
try {
    ...
    if (Test-Path -LiteralPath $nodeModules) {
        ...
        Write-Host '[!] pnpm install did not produce node_modules; UI may not start.' -ForegroundColor DarkYellow
    }
}
finally {
    Pop-Location
