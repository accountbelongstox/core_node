...
create_venv() {
    ...
    if [ -n "$PYTHON_ENV_DIR" ]; then
        ...
        sudo python3 -m venv "$PYTHON_ENV_DIR"
        
        if [ $? -eq 0 ]; then
            echo "Virtual environment created successfully"
            # Set proper permissions
            sudo chmod -R 755 "$PYTHON_ENV_DIR"
...
install_python_essentials() {
    ...
    # Install Python and essential packages without creating symbolic links
...
main() {
    ...
    if check_python_installed; then
        if pip3 -V >/dev/null 2>&1; then
            echo "Python environment is already properly set up"
        else
            is_new_install=true
            install_python_essentials
        fi
