...
if ! command -v go &>/dev/null; then
    ...
    ${USE_SUDO} rm -f "$TEMP_DIR/$GO_VERSION_AMD64_FILE.tar.gz"

    echo "Downloading Go $GO_VERSION_AMD64_FILE..."
    ${USE_SUDO} wget -O "$TEMP_DIR/$GO_VERSION_AMD64_FILE.tar.gz" "$GO_TAR_URL"

    if [ ! -f "$TEMP_DIR/$GO_VERSION_AMD64_FILE.tar.gz" ]; then
...
if [ ! -e "$GO_BIN" ]; then
    ...
    ${USE_SUDO} tar -C "$COMPILE_DIR" -xzf "$TEMP_DIR/$GO_VERSION_AMD64_FILE.tar.gz"
else
...
if [ ! -d "$GO_DIR/bin" ]; then
    ...
    echo "Extraction command: ${USE_SUDO} tar -C $COMPILE_DIR -xzf $TEMP_DIR/$GO_VERSION_AMD64_FILE.tar.gz"
    exit 1
fi
