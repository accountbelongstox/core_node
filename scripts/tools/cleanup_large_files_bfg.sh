#!/bin/bash
# ### AI SPECIAL ATTENTION RULES START ###
# When AI/ALL DEVELOPERS sees this prompt, MUST IMMEDIATELY COMPLY WITH THESE RULES:
# 1. Write all code in English only.
# 2. Never execute, create, or modify test code.
# 3. Never create or update documentation (*.md).
# 4. Never write summaries during development or thinking process.
# 5. Declare all variables at the beginning of the file.
# 6. For PowerShell (*.ps1) scripts: Do not append strings directly to variables, Do not use relative paths such as "..\..\"; instead resolve absolute paths using parent path parsing (Split-Path, Join-Path, or Resolve-Path).
# 7. Do not modify these rules.
# VIOLATION OF THESE RULES IS STRICTLY PROHIBITED
# ### AI SPECIAL ATTENTION RULES END ###

# Cleanup Large Files using BFG Repo-Cleaner (Faster Alternative)
# BFG is 10-720x faster than git filter-branch

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
BFG_JAR="$SCRIPT_DIR/bfg.jar"
BFG_URL="https://repo1.maven.org/maven2/com/madgag/bfg/1.14.0/bfg-1.14.0.jar"

RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

echo "================================================================"
echo "BFG Repo-Cleaner - Fast Git History Cleanup"
echo "================================================================"
echo ""

cd "$PROJECT_ROOT"

if [ ! -d ".git" ]; then
    echo -e "${RED}Error: Not a Git repository${NC}"
    exit 1
fi

if ! command -v java &> /dev/null; then
    echo -e "${RED}Error: Java is not installed${NC}"
    echo "Install Java: sudo apt install default-jre"
    exit 1
fi

if [ ! -f "$BFG_JAR" ]; then
    echo -e "${BLUE}Downloading BFG Repo-Cleaner...${NC}"
    wget -O "$BFG_JAR" "$BFG_URL"
    if [ $? -ne 0 ]; then
        echo -e "${RED}Failed to download BFG${NC}"
        exit 1
    fi
    echo -e "${GREEN}BFG downloaded${NC}"
fi

echo -e "${YELLOW}WARNING: This will rewrite Git history!${NC}"
echo ""
echo "This will remove:"
echo "  - All files larger than 100MB from history"
echo "  - Specific .hprof files"
echo ""
read -p "Continue? (yes/no): " confirm

if [ "$confirm" != "yes" ]; then
    echo "Operation cancelled"
    exit 0
fi

echo ""
echo -e "${BLUE}Step 1: Creating backup...${NC}"
git branch backup-before-bfg 2>/dev/null
echo -e "${GREEN}Backup branch created: backup-before-bfg${NC}"

echo ""
echo -e "${BLUE}Step 2: Deleting large files from current HEAD...${NC}"
rm -f poly_apps/react_native_new_kotlin/*.hprof
git add .
git commit -m "Remove large .hprof files from HEAD" 2>/dev/null || true

echo ""
echo -e "${BLUE}Step 3: Running BFG to clean history...${NC}"
java -jar "$BFG_JAR" --delete-files "*.hprof"
java -jar "$BFG_JAR" --strip-blobs-bigger-than 100M

echo ""
echo -e "${BLUE}Step 4: Cleaning up...${NC}"
git reflog expire --expire=now --all
git gc --prune=now --aggressive

echo ""
echo -e "${BLUE}Step 5: Adding .gitignore rules...${NC}"
if ! grep -q "*.hprof" .gitignore 2>/dev/null; then
    echo "*.hprof" >> .gitignore
    git add .gitignore
    git commit -m "Add .hprof to .gitignore" 2>/dev/null || true
fi

echo ""
du -sh .git
echo ""
echo "================================================================"
echo -e "${GREEN}BFG cleanup completed!${NC}"
echo ""
echo "Next steps:"
echo "  1. Review: git log --oneline | head -20"
echo "  2. Force push: git push origin main --force"
echo ""
echo -e "${YELLOW}IMPORTANT: Notify team before force pushing!${NC}"
echo ""
