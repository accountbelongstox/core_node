...
<#
.SYNOPSIS
    Windsurf MCP Servers Configuration (PowerShell version)
.DESCRIPTION
    Configures MCP servers for Windsurf (Codeium) by updating mcp_config.json.
    Windsurf uses "serverUrl" for remote servers. Uses _json_sync_helper.py
...
$script:WINDSURF_CONFIG_PATH = $null
...
function Find-WindsurfSettingsPath {
    $homeDir = [System.Environment]::GetFolderPath("UserProfile")
    $possiblePaths = @(
        (Join-Path $homeDir ".codeium\windsurf\mcp_config.json"),
        (Join-Path $homeDir ".codeium\windsurf-next\mcp_config.json")
    )
...
#region Main Logic
Write-Host "================================================================================"
Write-Host "[WINDSURF] Configuring MCP servers via mcp_config.json"
Write-Host "================================================================================"
Write-Host ""
Write-Host "[INFO] Using Python: $script:PYTHON_CMD"

$script:WINDSURF_CONFIG_PATH = Find-WindsurfSettingsPath
Write-Host "[INFO] Windsurf MCP file: $script:WINDSURF_CONFIG_PATH"
Write-Host ""

$configs = Get-AllMCPConfigs -Target "windsurf"
...
$tempFile = Join-Path $env:TEMP "mcp_windsurf_entries.json"
$entries | ConvertTo-Json -Depth 10 | Out-File -FilePath $tempFile -Encoding utf8

& $script:PYTHON_CMD -u $script:JSON_HELPER_PY $script:WINDSURF_CONFIG_PATH $tempFile "windsurf"

Remove-Item -LiteralPath $tempFile -ErrorAction SilentlyContinue

Write-Host ""
Write-Host "[INFO] Windsurf: open Cascade > MCP servers panel and click Refresh to load new servers"
Write-Host "================================================================================"
Write-Host "[SUMMARY] Windsurf MCP Configuration Complete"
Write-Host "================================================================================"
#endregion
