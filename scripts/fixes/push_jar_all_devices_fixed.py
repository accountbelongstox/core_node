...
def push_to_device(serial):
    ...
    # Step 1: Push jar file
    print(f"[{serial}] STEP 1: Pushing jar (using double-slash path)...")
    ...
    print(f"\n[{serial}] STEP 2: Verifying file exists...")
    ...
    print(f"\n[{serial}] STEP 3: Verifying MD5 hash...")
    md5_result = subprocess.run(
        [ADB, "-s", serial, "shell", "md5sum /data/local/tmp/scrcpy-server.jar"],
        capture_output=True,
        text=True,
        timeout=5
    )
    ...
    if md5_result.returncode != 0:
        print(f"[{serial}] ❌ MD5 CHECK FAILED:")
        print(md5_result.stderr)
        return False

    device_hash = md5_result.stdout.split()[0]
    print(f"[{serial}] ✅ MD5 hash: {device_hash}")
...
def main():
    ...
    if success_count == len(devices):
        ...
        print(f"\n⚠️  {fail_count} device(s) failed. Check errors above.")
