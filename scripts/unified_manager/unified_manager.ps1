...
function Show-Menu {
    ...
    Write-Host "Up/Down: Navigate | Left/Right: Toggle script | Enter: Launch | Space: Select | E: Execute | S: Save | K: Create KMP APP | Q: Quit" -ForegroundColor White
    Write-Host ""
}
...
function Remove-GitDirectory {
    ...
    if (Test-Path $gitPath) {
        Write-Host "Removing .git directory: $gitPath" -ForegroundColor Yellow
        Write-Host "Command: Remove-Item -Path `"$gitPath`" -Recurse -Force" -ForegroundColor Gray
        Remove-Item -Path $gitPath -Recurse -Force -ErrorAction SilentlyContinue
        if (Test-Path $gitPath) {
            Write-Host "Warning: Failed to remove .git directory completely" -ForegroundColor Red
            return $false
        } else {
...
function Create-NewKmpApp {
    ...
    try {
        ...
        if (Test-Path $newAppPath) {
            ...
            Write-Host "You can now navigate to the app in the menu and launch it." -ForegroundColor Cyan

            # Rescan apps to include the new one
            Write-Host ""
            Write-Host "Rescanning applications..." -ForegroundColor Cyan
            Scan-Apps
        } else {
