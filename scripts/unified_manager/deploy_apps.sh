...
# Unified Manager - Deploy Applications as System Services
# Interactive menu for deploying applications with systemd service integration
...
build_menu_items() {
    ...
    done > /tmp/.core_node/unified_manager/deploy_menu_apps.tmp
    ...
    MENU_ITEMS+=("Exit")
    MENU_ACTIONS+=("EXIT")
    MENU_DESCRIPTIONS+=("Exit the deployment menu")

    rm -f /tmp/.core_node/unified_manager/deploy_menu_apps.tmp
}
...
display_menu() {
    clear
    echo "==============================================="
    echo "        NCore Application Deployment"
    echo "==============================================="
    echo ""
...
deploy_application() {
    ...
    if [ -f "$deploy_script" ]; then
        ...
        write_warning "Deploy script not found, using start script: $start_script"
    else
        write_error "Neither deploy.sh nor start.sh found for $app_name"
        
        # Create embedded script for ncore apps
        if [ "$app_type" = "ncore-app" ]; then
            write_info "Creating embedded deploy script for ncore app"
    ...
    if [ -z "$script_to_deploy" ] || [ ! -f "$script_to_deploy" ]; then
        write_error "No valid script found for deployment"
        read -p "Press Enter to continue..."
        return 1
    fi
    
    # Deploy as service
    write_info "Deploying as system service..."
    if [ -f "$SERVICE_MANAGER" ]; then
        bash "$SERVICE_MANAGER" create "$script_to_deploy" "$app_name" "$description"
        if [ $? -eq 0 ]; then
            write_success "Application $app_name deployed successfully as ncore-$app_name service"
        else
            write_error "Failed to deploy $app_name as service"
        fi
    else
...
create_embedded_ncore_script() {
    ...
    local embedded_script="$script_dir/deploy.sh"
...
# Auto-generated deploy script for NCore app: $app_name
...
echo "[INFO] Deploying NCore application: $app_name"
...
# Deploy $app_name in production mode
...
show_app_management() {
    ...
    while true; do
        ...
        case "$choice" in
            1) deploy_application_service "$app_name" ;;
...
# Function to deploy application service (renamed from deploy_application)
deploy_application_service() {
    ...
    # Determine script to deploy
    local script_to_deploy=""

    if [ -n "$deploy_cmd" ] && [ "$deploy_cmd" != "null" ]; then
        # Use deploy command from registry
        script_to_deploy=$(echo "$deploy_cmd" | sed 's/^bash //')
        write_info "Using deploy script: $script_to_deploy"
    else
        # Try to find deploy script
        local deploy_script="$app_path/scripts/deploy.sh"
        if [ -f "$deploy_script" ]; then
            script_to_deploy="$deploy_script"
            write_info "Using deploy script: $script_to_deploy"
        else
            write_info "No deploy script found, creating embedded script"
            if [ "$app_type" = "ncore-app" ]; then
                write_info "Creating embedded deploy script for ncore app"
                script_to_deploy=$(create_embedded_ncore_script "$app_name" "$app_path")
            else
    ...
    if [ -f "$SERVICE_MANAGER" ]; then
        ...
        echo "==============================================="
        echo ""
        echo "Service deployed successfully. Starting live log view..."
