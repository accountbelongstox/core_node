...
if ($piPath -and
    ...
    if ($volcProfileType) {
        ...
        if ($mode -eq 'volc-coding' -and -not $volcApiKey) {
            $legacySecretPath = Join-Path $coreNodePath ".secret_keys\.secret_ignore\ARK_API_KEY_2"
            $volcApiKey = & $nodeExePath $harnessSettingsScriptPath secret-file $legacySecretPath
            if ($volcApiKey) {
                Write-Host "[INFO] Loaded Volcengine API Key from ARK_API_KEY_2" -ForegroundColor Green
            }
            else {
                Write-Host "[WARN] Volcengine API Key not found in ARK_API_KEY_2. A valid API key is required." -ForegroundColor Yellow
            }
            
            $legacyBaseUrlPath = Join-Path $coreNodePath ".secret_keys\.secret_ignore\ARKCLI_API_2"
