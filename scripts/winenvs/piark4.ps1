    ...
    if ($volcProfileType) {
        ...
        if ($mode -eq 'volc-coding' -and -not $volcApiKey) {
            $legacySecretPathCandidates = @(
                (Join-Path $coreNodePath ".secret_keys\.secret_ignore\ARKCLI_API_KEY_4"),
                (Join-Path $coreNodePath ".secret_keys\.secret_ignore\ARK_API_KEY_4")
            )
            ...
            if (-not $volcApiKey) {
                Write-Host "[WARN] Volcengine API Key not found in ARKCLI_API_KEY_4 (or ARK_API_KEY_4). A valid API key is required." -ForegroundColor Yellow
            }
