...
# Isolated user profile: D:\programing\Users\ark1
...
# Extra CLI args are forwarded to claude (e.g. ark1.ps1 -p "hi").
# =============================================================================
...
$ultraSettingsFile = Join-Path $env:TEMP "ark1_ultracode_settings.json"
...
Write-Host "Ark CLI (Volcano Ark / GLM) #1 - v4 [arkcli -> Claude]" -ForegroundColor Yellow
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
# Isolated Claude + arkcli user profile (map path: D:\programing\Users\ark1).
...
$arkUserDir = Join-Path $arkUserBase "ark1"
...
$arkcliProfile = Read-SecretFile (Join-Path $secretDir "ARKCLI_PROFILE_1")
$arkcliModel = Read-SecretFile (Join-Path $secretDir "ARKCLI_MODEL_1")
$arkcliMcpProfile = Read-SecretFile (Join-Path $secretDir "ARKCLI_MCP_PROFILE_1")
$arkcliOvResource = Read-SecretFile (Join-Path $secretDir "ARKCLI_OV_RESOURCE_1")
$arkcliApi = Read-SecretFile (Join-Path $secretDir "ARKCLI_API_1")
$arkcliApiKey = Read-SecretFile (Join-Path $secretDir "ARKCLI_API_KEY_1")
...
Write-Host "============================================================" -ForegroundColor Cyan
if ($usePlainClaude) {
    Write-Host "Press Enter to start Ark CLI (Volcano Ark / GLM) #1 [plain Claude / API KEY]..." -ForegroundColor Yellow
} else {
    Write-Host "Press Enter to start Ark CLI (Volcano Ark / GLM) #1 [arkcli -> Claude]..." -ForegroundColor Yellow
}
if (-not $enableTeam) {
    Write-Host "Tip: re-run with -team to enable Agent Teams (e.g. ark1.ps1 -team)" -ForegroundColor DarkGray
}
Write-Host "============================================================" -ForegroundColor Cyan
