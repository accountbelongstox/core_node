...
.NOTES
    - Tool Name: OpenAI
    - Command Prefix: openai
    - PowerShell Command: openai
    - File Number: 5
    - File Name: openai5.ps1
...
Write-Host "Running: openai5.ps1" -ForegroundColor Yellow
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
$env:OPENAI_API_BASE = Get-SecretKey -KeyName "OPENAI_API_BASE_5"
if ($env:OPENAI_API_BASE) {
    Write-Host "[SUCCESS] Loaded OPENAI_API_BASE = $($env:OPENAI_API_BASE)" -ForegroundColor Green
} else {
    Write-Host "[WARNING] Failed to load OPENAI_API_BASE" -ForegroundColor Yellow
}

$env:OPENAI_API_KEY = Get-SecretKey -KeyName "OPENAI_API_KEY_5"
...
if ($env:OPENAI_API_BASE) {
    $envVarsParts += "`$env:OPENAI_API_BASE='$($env:OPENAI_API_BASE)'"
}
...
if ($envVarsCommand) {
    $fullCommandDisplay = "$envVarsCommand; openai"
} else {
    $fullCommandDisplay = "openai"
}
#endregion
...
Write-Host "OpenAI - Pre-Launch Tasks" -ForegroundColor Yellow
Write-Host ""

# Execute pre-launch script if it exists
$preLaunchScript = Join-Path $aiToolsDirPath "openai_pre_launch.ps1"
...
Write-Host "  [1] Upgrade OpenAI to latest version (runs in separate window)" -ForegroundColor White
Write-Host "  [2] Sync MCP server configurations (runs now)" -ForegroundColor White
Write-Host ""

$upgradeChoice = Read-Host "Do you want to upgrade OpenAI? (y/N)"
if ($upgradeChoice -eq "y" -or $upgradeChoice -eq "Y") {
    $upgradeScript = Join-Path $aiToolsDirPath "openai_update.bat"
    Write-Host ""
    Write-Host "[INFO] Launching OpenAI upgrade in separate window..." -ForegroundColor Yellow
...
$currentWorkingDir = Get-Location
$syncScript = Join-Path $aiToolsDirPath "openai_sync_mcp_servers.py"
Write-Host ""
Write-Host "Syncing MCP Server Configurations..." -ForegroundColor Yellow
Write-Host ""
Write-Host "[INFO] Executing: python -u `"$syncScript`" --target openai --working-dir `"$currentWorkingDir`"" -ForegroundColor Cyan
Write-Host "[INFO] Working Directory: $currentWorkingDir" -ForegroundColor Cyan
Write-Host ""

python -u "$syncScript" --target openai --working-dir "$currentWorkingDir"

Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "Press Enter to start OpenAI..." -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "Press Enter to start OpenAI..." -ForegroundColor Yellow
Write-Host "============================================================" -ForegroundColor Cyan
$null = Read-Host "Press Enter to continue"

Write-Host ""
Write-Host "Executing: openai" -ForegroundColor White
Write-Host ""
