    ...
    if ($volcProfileType) {
        ...
        if ($mode -eq 'volc-coding' -and -not $volcApiKey) {
            $legacySecretPathCandidates = @(
                (Join-Path $coreNodePath ".secret_keys\.secret_ignore\ARKCLI_API_KEY_6"),
                (Join-Path $coreNodePath ".secret_keys\.secret_ignore\ARK_API_KEY_6")
            )
            ...
            if (-not $volcApiKey) {
                Write-Host "[WARN] Volcengine API Key not found in ARKCLI_API_KEY_6 (or ARK_API_KEY_6). A valid API key is required." -ForegroundColor Yellow
            }
            
            $legacyBaseUrlPath = Join-Path $coreNodePath ".secret_keys\.secret_ignore\ARKCLI_API_6"
        ...
        if ($mode -eq 'volc-coding') {
            $legacyModelPath = Join-Path $coreNodePath ".secret_keys\.secret_ignore\ARKCLI_MODEL_6"
