...
3. Offer a pnpm upgrade prompt [y/N] (default N) when the installed
   @moonshot-ai/kimi-code lags the latest registry version.
4. Prompt for the model [Y/n] (default Y = kimi k3 256K / ``k3-256k``);
   on N offer k3-256k / k3 / kimi-for-coding (kimi2.8 preview) /
   kimi-for-coding-highspeed (kimi2.7 code highspeed).
5. Configure the Kimi For Coding provider non-interactively via the
...
6. Force ``default_permission_mode = "auto"`` in config.toml and launch
   ``kimi --auto`` (Never Ask) so no "Approve once" prompts appear.
7. On desktop systems, offer Chrome MCP (apps/mcp-chrome) installation
...
#   -> pnpm upgrade prompt [y/N] -> model prompt [Y/n]
...
# Model selection (default Y = kimi k3 256K / @@DEFAULT_MODEL@@).
Write-Host "Use default model kimi k3 256K (@@DEFAULT_MODEL@@)? [Y/n]: " -ForegroundColor Yellow -NoNewline
...
# Model selection (default Y = kimi k3 256K / @@DEFAULT_MODEL@@).
printf '\033[33mUse default model kimi k3 256K (@@DEFAULT_MODEL@@)? [Y/n]: \033[0m'
