...
flowchart TB
    subgraph BigCtl["大CTL RosbotFlowController"]
        A1_Start["[A1] 启动 ROSBOT，设总状态、更新 UI"]
        A1e_Event["[A1e] 事件驱动入口（一次执行，非 tick）"]
        A1_Start --> A1e_Event
        A1e_Event --> F_Entry
    end
    ...
    subgraph B["小流程CTL B BattlenetLoginCtl"]
        ...
        B7_WaitUI["[B7] 轮询 UI 直到出现元素（非 tick）"]
    ...
    subgraph F["小流程CTL F 预判（事件驱动，非 tick）"]
        F_Entry["[F0] 预判入口（事件驱动）"]
        F1_HasD3{"[F1] D3 是否在线？"}
        F1c_EndD3["[F1c] 结束 D3 进程"]
        F1d_Offline["[F1d] 识别到掉线"]
        F2_RosbotOnline{"[F2] ROSBOT 是否在线？"}
        F3_LogTimeout{"[F3] ROSBOT 日志超时？（轮询，非 tick）"}
    ...
    subgraph E["小流程CTL E ROSBOT 运行流程"]
