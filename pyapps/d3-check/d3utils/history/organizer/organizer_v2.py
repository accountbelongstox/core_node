...
# -*- coding: utf-8 -*-
"""
Organizer V2: Uses parser V2 + aggregator + formatter.
"""
...
class HistoryOrganizerV2(HistoryOrganizerBase):
    """Organizer V2: Multi-level indent parser with time window aggregation."""
    
    def __init__(self, history_path: str) -> None:
        super().__init__(history_path)
        self._parser = HistoryParserV2()
    ...
    def get_latest_stats_as_lines(self, min_entry_ts: Optional[float] = None) -> List[str]:
        ...
        If min_entry_ts is None, returns last block's earned as label:value lines.
        If min_entry_ts is set, aggregates blocks in time window [min_entry_ts, ...].
        """
        if not os.path.isfile(self._path):
            return []
        
        # Read full file for aggregation
        lines: List[str] = []
        try:
        ...
        if min_entry_ts is None:
            # Return last block's earned as label:value lines
            last_block = blocks[-1]
