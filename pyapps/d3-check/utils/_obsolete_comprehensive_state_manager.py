...
class ComprehensiveStateManager:
    ...
    def print_comprehensive_status(self, force_print: bool = False):
        ...
        ColorPrint.blue("=" * 80)
        ColorPrint.green("[COMPREHENSIVE_STATUS] Complete System State")
        ColorPrint.blue("-" * 40)
        
        # RoS-BoT Status
        rosbot = status["rosbot"]
        ColorPrint.blue(f"[ROSBOT] Status: {rosbot['startup_status']}")
        ColorPrint.blue(f"[ROSBOT] Current exe: {rosbot['current_other_exe']}")
        ColorPrint.blue(f"[ROSBOT] Window title: '{rosbot['current_window_title']}'")
        ColorPrint.blue(f"[ROSBOT] Runtime: {rosbot['continuous_runtime_seconds']:.1f}s")
        ColorPrint.blue(f"[ROSBOT] First cleanup: {rosbot['first_startup_cleanup']}")
        
        # Battle.net Status
        battlenet = status["battlenet"]
        ColorPrint.blue(f"[BATTLENET] Status: {battlenet['startup_status']}")
        ColorPrint.blue(f"[BATTLENET] Window title: '{battlenet['current_window_title']}'")
        
        # Diablo Status
        diablo = status["diablo"]
        ColorPrint.blue(f"[DIABLO] Status: {diablo['startup_status']}")
        ColorPrint.blue(f"[DIABLO] Window title: '{diablo['actual_window_title']}'")
        
        # System Runtime
        runtime = status["system_runtime"]
        ColorPrint.blue(f"[SYSTEM] Runtime: {runtime['total_runtime_seconds']:.1f}s")
        ColorPrint.blue(f"[SYSTEM] Detection cycles: {runtime['detection_count']}")
