...
class SmallMapDetector:
    ...
    def __init__(self):
        ...
        if not self.template_config:
            ColorPrint.red("[SmallMapDetector] Template configuration not found for d4_small_map")
            return
        ...
        ColorPrint.blue("[SmallMapDetector] Initialized")
        ColorPrint.blue(f"[SmallMapDetector] Template: {self.template_path}")
        ColorPrint.blue(f"[SmallMapDetector] Threshold: {self.threshold}")
        ColorPrint.blue(f"[SmallMapDetector] Method: {self.match_method}")
    ...
    def detect_small_map(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ColorPrint.green(f"[SmallMapDetector] Detection result: {location_type} (confidence: {match_result.get('confidence', 0):.3f})")
            
            return result
            
        except Exception as e:
            error_msg = f"Error in small map detection: {e}"
            ColorPrint.red(f"[SmallMapDetector] {error_msg}")
    ...
    def _extract_minimap_region(self, screenshot: Image.Image, start: Tuple[int, int], end: Tuple[int, int]) -> Optional[np.ndarray]:
        ...
        if minimap_region.size == 0:
            ColorPrint.yellow("[SmallMapDetector] Extracted minimap region is empty")
            return None
    ...
    def _save_debug_image(self, minimap_region: np.ndarray, match_result: Dict[str, Any], result: Dict[str, Any]):
        ...
        ColorPrint.green(f"[SmallMapDetector] Debug image saved: {debug_path}")
        ...
        ColorPrint.blue(f"[SmallMapDetector] Raw minimap region saved: {region_path}")
