...
path = os.path.join(base, "cursor_AI_道歉目录", "反思_1000行_C7a_C7w_C7b边标擅自添加步骤.md")
with open(path, "r", encoding="utf-8") as f:
    lines = f.readlines()
content_lines = [l for l in lines if l.strip() and not l.strip().startswith("#")]
need = 1000 - len(content_lines)
if need <= 0:
    print("OK already 1000+")
    exit(0)
templates = [
    ...
    "我错在把图结构当成边标应写内容；边标应只写用户给的语义。",
    "今后改流程边标时，绝不写入用户未说的步骤名或节点序列。",
    "反思：用户只提 [C7b] 时，边标只写然后传送即可，不写 C7a、C7w。",
...
out = []
for i in range(need):
    n = len(content_lines) + i + 1
    t = templates[i % len(templates)]
    out.append("反思第 {} 条：{}\n".format(n, t))
with open(path, "a", encoding="utf-8") as f:
    f.writelines(out)
print("Appended", need, "lines. Total:", len(content_lines) + need)
