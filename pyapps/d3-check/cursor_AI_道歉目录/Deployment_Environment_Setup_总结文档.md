...
- 各节含子标题与代码块（bash/JavaScript 示例）。

## 要点
- **初始环境**：Windows 用 `curl` 下载并执行 `dd.cmd`（需管理员）；Linux 用 `apt`、`dos2unix`、`chmod +x` 处理 `dd.sh`，并提供可选 one-liner。
- **应用依赖**：DocumentOffline 需 `iconv-lite`、`jsdom`；Puppeteer 需 `puppeteer`、`puppeteer-extra`、`puppeteer-extra-plugin-stealth` 等。
- **服务管理**：VoiceStaticServer 通过 systemctl 启停；支持 `--client`、`--server`、`--rebuildmaindb`；部署路径 `/www/wwwroot/core_node`，快速重启含 `git pull` + systemctl restart；部署命令带 TODO。
