...
└─────────────────────────────────────────────────────────────────────────────┘
         ...
         └─ 出现 d3_game_tool（游戏工具）→ 片段 1 成功
              ...
              │ 打开地图按 M（前步已做）；按 M 后间隔 2 秒，  │
              │ 缩小地图 point(751,413)，点击传送 click(610,126)。 │
              └─────────────────────────────────────┘
                          │
...
状态 = "game_tool"（当前界面已有 d3_game_tool）
         ...
         ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ 截图（发送前）→ 向游戏发送 M → 截图（发送后）→ 对比相似度。                      │
│ 高度相似则 M 无反应视为掉线；否则未掉线，再按一次 M 复位地图后传送。             │
└─────────────────────────────────────────────────────────────────────────────┘
         │
         ├─ 超时 → 结束 D3，走分支 B
         │
         └─ 任一次出现悬赏进度 → 等待 2 秒后执行传送（缩小地图 (751,413)，点击传送 (610,126)）
                          │
                          ▼
