...
**原理**：每设备独立socket namespace

```python
# 设备1
scid1 = 0xa1b2c3d4
socket_name1 = "scrcpy_a1b2c3d4"

# 设备2
scid2 = 0x5e6f7890
socket_name2 = "scrcpy_5e6f7890"

# 设备17
scid17 = 0x1a2b3c4d
socket_name17 = "scrcpy_1a2b3c4d"
```

**隔离保证**：
- 不同socket名称 → 不同localabstract地址
- 即使在同一设备上启动多个服务器，也不会冲突
- 但我们是多设备，每设备一个服务器更简单

### 端口分配策略

```python
def allocate_ports(device_index: int) -> dict:
    """为设备分配唯一端口"""
    base = 27183 + (device_index * 10)
    return {
        'video': base,        # 27183, 27193, 27203, ...
        'control': base + 1   # 27184, 27194, 27204, ...
    }

# 17台设备
devices = []
for i, serial in enumerate(serials):
    scid = random.randint(0, 0xFFFFFFFF)
    ports = allocate_ports(i)
