...
class UnifiedPriceManager:
    ...
    def _update_stats(self):
        ...
        with self.lock:
            ...
            cursor.execute("SELECT COUNT(*) FROM unified_prices WHERE source = 'historical'")
            self.stats['historical_records'] = cursor.fetchone()[0]

            cursor.execute("SELECT COUNT(*) FROM unified_prices WHERE source = 'realtime'")
            self.stats['realtime_records'] = cursor.fetchone()[0]

            self.stats['coins_tracked'] = self.get_coin_count()
