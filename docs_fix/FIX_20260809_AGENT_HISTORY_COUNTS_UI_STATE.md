...
- Agent History load data filters the shared AI usage store to `ai_sources.OPENROUTER_ATTEMPT_SOURCES` (`agent_history_article`, `agent_history_translate`, and since 2026-09-27 `agent_history_prompt_rewrite`); the UI reads the list from runtime `ai_dashboard.sources`. It does not create a separate request counter.
