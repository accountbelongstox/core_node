...
| --- | --- | --- | --- |
| G01 | Any user can confirm their own deposit and activate a role without an administrator | S1 | done |
...
| G03 | Refund approval and processing only require a low role level instead of administrator authorization | S1 | done |
| G04 | Refund processing credits the payer without debiting the payee (creates money); payment ends as cancelled instead of refunded; no locking | S1 | done |
| G05 | One payment can receive unlimited refund requests; payment status is not checked | S1 | done |
| G06 | Any signed-in user can read any payment by id | S1 | done |
| G07 | Wallet-to-wallet payment holds funds and then withdraws again (fails or double-deducts, frozen funds stuck) | S1 | done |
...
| G12 | A wrong administrator super code is silently ignored at registration | S3 | done |

## 2. User workspace

| ID | Function | Interface | Server | Pkg | Status |
| --- | --- | --- | --- | --- | --- |
| U01 | Project funding: client funds an accepted proposal into escrow, project moves to open | missing | missing | S1+U1 | wip (server done) |
| U02 | Escrow release to the developer on client approval of a task | n/a | missing | S1 | done |
...
| U17 | Developer payout / withdrawal request and history | missing | missing | S1+U1 | wip (server done) |
| U18 | Invoice list (payee only creates invoices) | missing | missing | S1+U1 | wip (server done) |
| U19 | User refund request list with status | missing | missing | S1+U1 | wip (server done) |
| U20 | Deposit information per role with correct policy amounts; deposit for a chosen role | wrong | wrong | S1+U1 | wip (server done) |
| U21 | Bank-transfer deposit instructions reachable | dead link | missing | S1+U1 | wip (server done) |
...
| U27 | Role request for existing accounts (become developer / client) | missing | missing | S3+U1 | wip (server done) |
...
| U30 | Idempotency key honored for deposits, payments, funding, analysis accept, refunds | missing | missing | S1+U1 | wip (server done) |
...
| ID | Function | Interface | Server | Pkg | Status |
| --- | --- | --- | --- | --- | --- |
| A01 | Refund approve / reject / process under administration | missing | misplaced | S1+U2 | wip (server done) |
| A02 | Deposit reject and deposit refund; confirming administrator recorded | missing | missing | S1+U2 | wip (server done) |
| A03 | Withdrawal requests review and processing | missing | missing | S1+U2 | wip (server done) |
| A04 | Payments, escrows, and disputed payments listing with resolve actions | missing | missing | S1+U2 | wip (server done) |
| A05 | Project intervention: pause / resume / cancel / archive with reason | missing | missing | S3+U2 | wip (server done) |
| A06 | User detail: profile, roles, KYC, deposits, wallet, projects | missing | missing | S3+U2 | wip (server done) |
| A07 | Role status transitions enforced (pending to active/rejected, active to suspended and back) with reason and confirmation | partial | weak | S3+U2 | wip (server done) |
| A08 | KYC review only for pending items, reject reason, private document viewer | partial | weak | S3+U2 | wip (server done) |
| A09 | Testimonial moderation: list, approve, hide, order | missing | missing | S3+U2 | wip (server done) |
| A10 | Reviewer applications oversight with revoke | missing | missing | S3+U2 | wip (server done) |
| A11 | Read-only platform policy (deposits, commission) | missing | missing | S3+U2 | wip (server done) |
| A12 | Audit activity log written on every administrator mutation and listed | missing | missing | S3+U2 | wip (server done) |
...
| A16 | Contact messages inbox | missing | missing | S3+U2 | wip (server done) |
...
| P03 | Public showcase: open tasks and completed projects (redacted) | missing | missing | S3+U3 | wip (server done) |
| P04 | Public statistics count only published projects and protected (escrowed) funds; active developer label | wrong | wrong | S3+U3 | wip (server done) |
| P05 | Contact form on the information page | missing | missing | S3+U3 | wip (server done) |
| P06 | Testimonial submission by clients of completed projects; localized testimonials | missing | missing | S3+U1 | wip (server done) |
| P07 | Estimate options loaded from the server; budget type used; formatted amounts; team as role counts | partial | partial | S3+U3 | wip (server done) |
