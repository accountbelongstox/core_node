...
**Pycore:** For work under `pycore`, use `development-guides/PYTHON_PYCORE.md` as the only Pycore specification; do not apply Pycore requirements from this file or other project documents.

**Laravel:** For Laravel modifications, refer only to `development-guides/LARAVEL_GUIDE.md`.

**MCP Chrome:** For mcp-chrome modifications, refer only to `development-guides/MCP_CHROME_GUIDE.md`.

**Shell:** For shell scripts, refer only to `development-guides/DD_SHELL_GUIDE_THIS_FILE_NO_AI_EDIT.md`. Never run builds, tests, services, or verification unless asked. Callers trust resolved PS1/SH references without existence/status checks. Installers repair only missing binaries, files, or pip packages and otherwise run. PowerShell does not parse versions with regex or enforce fine package versions. Hardcode compatibility only at ABI-major boundaries or delegate to pip. Do not use exit codes for return values.
