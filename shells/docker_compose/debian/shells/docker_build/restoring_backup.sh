...
restore_latest_backup() {
    echo "Restoring the latest backup from $BACKUP_DIR..."
    
    latest_backup=$(ls -1t "$BACKUP_DIR"/*.tar.gz | head -n 1)
    
    if [ -f "$latest_backup" ]; then
        echo "Found latest backup: $latest_backup"
        rm -rf "$WWW_DIR"  # Remove the current /www directory
        mkdir -p "$WWW_DIR"  # Recreate the /www directory
        tar -xzf "$latest_backup" -C "$WWW_DIR"  # Extract the backup into /www
        echo "Restoration completed."
    else
        echo "No backup found in $BACKUP_DIR to restore."
    fi
}

# Check if the /www directory exists and is not empty
if [ ! -d "$WWW_DIR" ] || is_empty_dir "$WWW_DIR"; then
    echo "/www directory is missing or empty. Attempting to restore from backup..."
    restore_latest_backup
else
