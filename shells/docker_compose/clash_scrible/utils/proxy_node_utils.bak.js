...
async function downloadYamlFiles(currentGroup) {
    const { groupScribleData, groupMd5, groupJsonFile } = getGroupScribleData(currentGroup);
    const groupYamlsDir = path.join(yamls_dir, groupMd5);

    if (!groupScribleData || Object.keys(groupScribleData).length === 0) {
        logger.logRed(`No valid group data found for group: ${currentGroup}`);
        logger.logYellow('Possible reasons:');
        logger.log('1. The group does not exist.');
        logger.log('2. The group exists but has no subscription nodes.');
        logger.log('3. There might be an issue with the group data file.');
        logger.logBlue('Suggestions:');
        logger.log('- Check if the group name is correct.');
        logger.log('- Ensure that you have added subscription nodes to this group.');
        logger.log('- Verify the integrity of the group data file.');
        return;
    }
    ...
    for (const urlEntity of Object.values(groupScribleData)) {
        ...
        if (url.startsWith('file::')) {
            logger.logYellow(`${urlEntity.name} is a file. Skipping URL download. This file will be read and parsed directly.`);
            continue;
        }
...
async function processYamlFiles( currentGroup = "default", template = "default") {
    let templatePath;
    if (template === "custom" && fs.existsSync(custom_template)) {
        templatePath = custom_template;
        logger.logGreen("Using custom template");
    ...
    let configProxies = await parseYamlForProxies(templatePath);
    const { groupScribleData, groupMd5, groupJsonFile } = getGroupScribleData(currentGroup);
    const groupYamlsDir = path.join(yamls_dir, groupMd5);
