...
select_install_type() {
    while true; do
        PS3="Select install type (1: server, 2: pve, 3: local, 4: docker-internal, 5: caddy-golang, 0: cancel): "
        options=("server" "pve" "local" "docker-internal" "caddy-golang" "cancel")
        select install_type in "${options[@]}"; do
            case "$install_type" in
                "server"|"local"|"docker-internal"|"caddy-golang")
