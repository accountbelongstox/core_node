# -*- coding: utf-8 -*-
"""
OCR model provisioning: PREWARM_SPEC + CnSTD/CnOCR model download from HF.

Single source of truth for OCR prewarm (zh / en / cht), each with latest models
per language. Drives both HF download and prewarm. Reuses the HF helpers in
_hf_helpers (native Python API, no CLI).
