...
class UserDataStoreThread(threading.Thread):
    r"""
    Own the file-backed user data document on one dedicated thread.
    ...
    def record_content_history(self, entry: Dict[str, Any], cap: int = 200) -> None:
        ...
        if not record.get("ts"):
            record["ts"] = time.time()
        key = (record.get("source_key"), record.get("type"))
        if key != (None, None):
            entries = [
                item for item in entries
                if (item.get("source_key"), item.get("type")) != key
            ]
    ...
    def save_feature_config(self, name: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            ColorPrint.red(
                f"[UserDataStore] Failed to save feature config {name}: {exc}"
            )


class UserDataStore:
    """Synchronous facade for the THREAD_BUS-owned store worker."""
    ...
    @property
    def base_dir(self) -> Path:
        """Directory that holds the store and feature config files."""
        return self._base_dir
    ...
    def get_content_history(
        ...
        return self._request('get_content_history', limit=limit) or []

    def feature_config_path(self, name: str, ext: str = "json") -> Path:
        return self._base_dir / f"{name}.{ext.lstrip('.')}"

    def load_feature_config(self, name: str) -> Dict[str, Any]:
        return self._request('load_feature_config', name=name) or {}

    def save_feature_config(self, name: str, data: Dict[str, Any]) -> None:
        self._request('save_feature_config', name=name, data=data)
    ...
    def _request(self, operation: str, **payload: Any) -> Any:
        ...
        if not response.get('success'):
            raise RuntimeError(
                response.get('error', 'User data operation failed')
            )
