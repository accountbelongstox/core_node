...
- protocol.py            - pure protocol/data layer (ProtocolVersion, MessageType,
                           DetectionResult, _process_start_time)
- server.py              - PRIMARY-side TCP server (_SingletonServerMixin:
                           bind/listen/listener/_handle_client/stop)
- detector.py            - SingletonDetector orchestrator + public hooks
...
from pycore.pyfoundations.singleton.server import _SingletonServerMixin
...
class SingletonDetector(_SingletonServerMixin):
    ...
    def __init__(
        ...
        Args:
            ...
            protocol_version: Protocol identifier used to isolate singleton groups
            on_shutdown_request: Optional callback before global shutdown begins
            takeover_timeout: Maximum seconds to wait for the previous instance
        """
        self.app_id = app_id
    ...
    def _log(self, message: str, level: str = "INFO"):
        ...
        if self.debug or level in ["ERROR", "WARNING"]:
            ...
            if level == "ERROR":
                ColorPrint.red(log_msg)
            elif level == "WARNING":
                ColorPrint.yellow(log_msg)
            else:
                ColorPrint.gray(log_msg)
    ...
    def _validate_message(self, message: Dict) -> bool:
        ...
        if message.get("protocol") != self.protocol_version:
