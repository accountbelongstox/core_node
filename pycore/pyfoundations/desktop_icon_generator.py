...
class DesktopIconGenerator:
    ...
    def _convert_png_to_ico(self, png_path, ico_path=None):
        ...
        if ico_path.exists():
            ...
            if ico_mtime >= png_mtime:
                ColorPrint.plain(f"[INFO] Using existing ICO (up to date): {ico_path}")
                return ico_path
    ...
    def create_shortcut(self, target_path, name=None, icon_path=None, working_dir=None,
        ...
        if shortcut_exists:
            ...
            if not needs_update:
                ColorPrint.plain(f"Shortcut already exists and matches: {shortcut_path}")
        ...
        ColorPrint.plain(f"{action} shortcut: {shortcut_path}")

        # Set AppUserModelID property if provided (prevents duplicate taskbar icons)
        if app_user_model_id:
            ColorPrint.plain(f"[DesktopIconGenerator] Setting AppUserModelID on shortcut...")
            if set_shortcut_app_user_model_id(shortcut_path, app_user_model_id):
                ColorPrint.plain(f"[DesktopIconGenerator] AppUserModelID set: {app_user_model_id}")
            else:
                ColorPrint.plain(f"[DesktopIconGenerator] Failed to set AppUserModelID")
    ...
    def update_shortcut(self, shortcut_path, target_path=None, name=None, icon_path=None,
        ...
        ColorPrint.plain(f"Updated shortcut: {shortcut_path}")
        return shortcut_path
    ...
    def delete_shortcut(self, name):
        ...
        if shortcut_path.exists():
            ...
            ColorPrint.plain(f"Deleted shortcut: {shortcut_path}")
            return True
        else:
            ColorPrint.plain(f"Shortcut not found: {shortcut_path}")
            return False
    ...
    def create_group_folder(self, folder_name, target_folder_path=None):
        ...
        if folder_name.endswith('.lnk'):
            ...
            ColorPrint.plain(f"Created folder shortcut: {folder_shortcut}")
            return folder_shortcut
        else:
            # Create actual folder
            folder_path = self.desktop_path / folder_name
            folder_path.mkdir(exist_ok=True)
            ColorPrint.plain(f"Created folder: {folder_path}")
            return folder_path
    ...
    def create_shortcut_in_folder(self, folder_path, target_path, name=None, icon_path=None,
        ...
        ColorPrint.plain(f"Created shortcut in folder: {shortcut_path}")
        return shortcut_path
    ...
    def batch_create_shortcuts(self, shortcuts_config):
        ...
        for config in shortcuts_config:
            ...
            if not target_path:
                ColorPrint.plain("Warning: Skipping shortcut - target_path required")
                continue
