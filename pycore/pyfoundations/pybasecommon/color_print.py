...
class ColorPrint:
    ...
    def stream(message, color="gray", log_level="DEBUG"):
        ...
        """
        c = getattr(ColorPrint, color.upper(), ColorPrint.GRAY) if isinstance(color, str) else color
        if not isinstance(c, str):
            c = ColorPrint.GRAY
        if not ColorPrint._mcp_mode:
            if ColorPrint._disable_colors:
                print(message, file=ColorPrint._output_stream)
            else:
                print(f"{c}{message}{ColorPrint.RESET}", file=ColorPrint._output_stream)
        ColorPrint._log_to_callback(message, color if isinstance(color, str) else "gray", log_level)
