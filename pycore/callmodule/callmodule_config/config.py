...
class Config:
    ...
    # pyutils TTS orchestrator (edge-tts) and reports them back (/tts/worker/report).
    # OFF by default - word audio is generated by Puter.js (browser, pycore-manager
    # Queue Center batch bar) uploading directly to /word/audio/upload; pycore
    # edge-tts no longer interacts with the word Task Queue unless manually enabled.
    # Enable with PYCORE_TTS_WORKER=1 (manual load) - paired with the UI heartbeat
    # toggle POST /api/heartbeat/enable/tts_queue_poller.
    TTS_WORKER_ENABLED_ON_START = (
        os.getenv("PYCORE_TTS_WORKER", "0") in ("1", "true", "True")
    )
    # Word-audio edge-tts lane is a FALLBACK only - Puter.js (browser, pycore-manager
    # Queue Center 1000-word batch bar) is the primary word-audio generator. Enable
    # the pycore edge-tts fallback with PYCORE_WORD_AUDIO_EDGE_FALLBACK=1 (e.g. when
    # Puter.js is unavailable / no browser is open). Default OFF = Puter primary.
    WORD_AUDIO_EDGE_FALLBACK = (
        os.getenv("PYCORE_WORD_AUDIO_EDGE_FALLBACK", "0") in ("1", "true", "True", "yes", "on")
    )
