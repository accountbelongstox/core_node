...
def register_local_ai_image_routes(server):
    server.post(name=UI_AI_IMAGE_IMAGE, handler=ai.image)
    server.post(name=UI_AI_IMAGE_IMAGE_TEST, handler=ai.image_test)

    def image_history_handler(params, request_id, context):
        return ai.image_history(int((params or {}).get("limit") or 50))

    server.post(name=UI_AI_IMAGE_IMAGE_HISTORY, handler=image_history_handler)

    def image_history_file_handler(params, request_id, context):
        return ai.image_history_file(str((params or {}).get("image_id") or ""))

    server.post(name=UI_AI_IMAGE_IMAGE_HISTORY_FILE, handler=image_history_file_handler)

    def image_history_reveal_handler(params, request_id, context):
        return ai.image_history_reveal(str((params or {}).get("image_id") or ""))

    server.post(name=UI_AI_IMAGE_IMAGE_HISTORY_REVEAL, handler=image_history_reveal_handler)

    def image_history_delete_handler(params, request_id, context):
        return ai.image_history_delete(str((params or {}).get("image_id") or ""))

    server.post(name=UI_AI_IMAGE_IMAGE_HISTORY_DELETE, handler=image_history_delete_handler)

    server.post(name=UI_AI_IMAGE_IMAGE_HISTORY_CLEAR, handler=ai.image_history_clear)
