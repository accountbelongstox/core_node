...
def register_local_sentence_audio_routes(server):
    ...
    server.post(name=UI_SENTENCE_AUDIO_STATUS, handler=get_status)
    ...
    server.post(name=UI_SENTENCE_AUDIO_CONFIG, handler=config_handler)

    server.post(name=UI_SENTENCE_AUDIO_RUN_ONCE, handler=sa.run_once)
    server.post(name=UI_SENTENCE_AUDIO_QUEUE_SNAPSHOT, handler=sa.queue_snapshot)

    def variants_index_handler(params, request_id, context):
        params = params or {}
        return sa.variants_index(str(params.get("lang") or "en"))

    server.post(name=UI_SENTENCE_AUDIO_VARIANTS_INDEX, handler=variants_index_handler)
    ...
    server.post(name=UI_SENTENCE_AUDIO_VARIANTS_STORE, handler=variants_store_handler)
