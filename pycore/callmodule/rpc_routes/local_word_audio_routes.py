...
def register_local_word_audio_routes(server):
    ...
    server.route(name=UI_WORD_AUDIO_STATUS, handler=wa.status)
    ...
    server.route(name=UI_WORD_AUDIO_TEST, handler=test_handler)
    ...
    server.route(name=UI_WORD_AUDIO_MISSING_BATCH, handler=missing_batch_handler)
    ...
    server.route(name=UI_WORD_AUDIO_WORD_AUDIO_MEDIA, handler=word_audio_media_handler)

    server.route(name=UI_WORD_AUDIO_UPLOAD_WORD_AUDIO, handler=wa.upload_word_audio)

    def fetch_youdao_handler(params, request_id, context):
        params = params or {}
        return wa.fetch_youdao(str(params.get("word") or ""), int(params.get("type") or 2))

    server.route(name=UI_WORD_AUDIO_FETCH_YOUDAO, handler=fetch_youdao_handler)
    ...
    server.route(name=UI_WORD_AUDIO_EDGE_SYNTH, handler=edge_synth_handler)

    server.route(name=UI_WORD_AUDIO_FIX_WORD_TEXT, handler=wa.fix_word_text)
    server.route(name=UI_WORD_AUDIO_BOOST_PRIORITY, handler=wa.boost_priority)
