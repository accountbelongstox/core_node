...
def register_local_image_search_routes(server):
    ...
    server.post(path=UI_IMAGE_SEARCH_STATUS, handler=image_search.status)
    ...
    server.post(path=UI_IMAGE_SEARCH_SEARCH, handler=search_handler)
    ...
    server.post(path=UI_IMAGE_SEARCH_SEARCH_AI, handler=search_ai_handler)
    ...
    def delete_history_handler(params, request_id, context):
        return image_search.delete_history(str(params.get("entry_id") or ""))

    server.post(path=UI_IMAGE_SEARCH_DELETE_HISTORY, handler=delete_history_handler)

    server.post(path=UI_IMAGE_SEARCH_CLEAR_HISTORY, handler=image_search.clear_history)

    def resource_handler(params, request_id, context):
        return image_search.resource(str(params.get("url") or ""))

    server.post(path=UI_IMAGE_SEARCH_RESOURCE, handler=resource_handler)
