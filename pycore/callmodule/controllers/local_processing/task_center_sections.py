# -*- coding: utf-8 -*-
"""Build Queue Center sections from the canonical category registry.
...
def _lifecycle(control: Dict[str, Any], error: Optional[str]) -> str:
    if error:
        return "error"
    configured = _to_bool(control.get("configured"))
    running = _to_bool(control.get("running"))
...
def _toggle(control: Dict[str, Any]) -> QueueCenterToggleEnvelope:
    ...
    return {
        ...
        "enabled": _to_bool(control.get("configured")),
...
def _worker_metrics(workers: List[Dict[str, Any]], running: bool = False) -> Dict[str, Any]:
    ...
    return {
        "online": running or any(_to_bool(worker.get("online")) for worker in workers),
...
def _build_queue_section(
    ...
    contract.update({
        ...
        "worker": _worker_metrics(matching_workers, _to_bool(control.get("running"))),
        ...
        "stale": _to_bool(overview.get("stale")),
    })
    return contract
...
def build_section_contracts(
    ...
    heartbeat_online = any(_to_bool(row.get("enabled")) for row in callbacks if isinstance(row, dict))
    heartbeat.update({
