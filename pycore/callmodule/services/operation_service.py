...
def _notify_after_commit(event: OperationEvent, scope: Optional[str] = None) -> None:
    payload = _event_payload(event, scope)
    THREAD_BUS.trigger_event("operation.changed", payload, async_mode=True)
    get_rpc_delivery_service().fanout_outbox_event(event.event_id)


def _broadcast(event: OperationEvent, scope: Optional[str] = None) -> None:
    """Backward-compatible alias after outbox is written in the same DB transaction."""
    _notify_after_commit(event, scope)
...
__all__ = ["OperationService", "RevisionConflictError"]
