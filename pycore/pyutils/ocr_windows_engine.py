...
def _ocr_result_to_raw_result(ocr_result: Any) -> List[Dict[str, Any]]:
    """Convert WinRT OcrResult (Lines -> Words) to list of {text, position} like CnOCR."""
    ...
    for line in ocr_result.lines:
        for word in line.words:
            ...
            text = getattr(word, "text", None) or getattr(word, "Text", "") or ""
            position = [
                [x, y],
                [x + w, y],
                [x + w, y + h],
            ...
            raw.append({"text": text, "position": position})
    return raw
...
class WindowsOCREngine:
    ...
    def _recognize_sync(self, img: "Image.Image") -> List[Dict[str, Any]]:
        ...
        ocr_result = asyncio.run(self._recognize_async(bitmap))
