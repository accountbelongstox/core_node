...
通用的跨进程单例检测器，使用端口范围扫描和协议验证。

特点：
- 端口范围扫描：从起始端口开始，依次尝试直到找到可用端口或正确实例
- 协议验证：确保检测到的是本程序而非其他程序
- PRIMARY/SECONDARY 模式：第一个实例为 PRIMARY，其他为 SECONDARY
- 跨进程通信：基于 JSON 消息协议
...
class SingletonDetector:
    ...
    def _log(self, message: str, level: str = "INFO"):
        ...
        if self.debug or level in ["ERROR", "WARNING"]:
            ...
            print(f"[{timestamp}] [{level}] SingletonDetector({self.app_id}): {message}")
    ...
    def _try_bind_port(self, port: int) -> bool:
        ...
        try:
            ...
            self._log(f"✓ Successfully bound to port {port} (PRIMARY instance)")
    ...
    def detect_and_bind(self) -> DetectionResult:
        ...
        for offset in range(self.port_range):
            ...
            if response:
                ...
                self._log("✓ Found existing instance (SECONDARY mode)")
            ...
            if self._try_bind_port(port):
                self._log("✓ Became PRIMARY instance")
                return DetectionResult(
                    is_primary=True,
                    port=port,
        ...
        # Exhausted all ports
        self._log("✗ Failed to find available port in range", "ERROR")
        return DetectionResult(
            is_primary=False,
            port=0,
            existing_instance=False,
