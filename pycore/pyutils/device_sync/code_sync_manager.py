...
via the RPC WebSocket (THREAD_BUS 'code_sync_update').

Control is entirely UI-driven; there is no tray menu for code sync anymore.
"""
...
except Exception:  # pragma: no cover - fallback if helper is absent
    def get_core_node_root():
...
class CodeSyncManager:
    ...
    def _broadcast(self) -> None:
        try:
            from pycore import THREAD_BUS
            THREAD_BUS.trigger_event("code_sync_update", self.mesh.snapshot())
        except Exception:
            pass
