...
class DeviceManager:
    ...
    async def get_device_info(self, serial: str, adb_path: str = "adb") -> Optional[DeviceInfo]:
        ...
        try:
            ...
            sdk_version = int(ADBManager.get_prop(serial, "ro.build.version.sdk", adb_path))
            ...
            print(f"Failed to get device info for {serial}: {e}")
            return None
    ...
    async def connect_device(
        ...
        async with self._lock:
            try:
                ...
                error_msg = f"Failed to connect device {serial}: {e}"
                print(error_msg)

                # Update state with error
                if serial in self.device_states:
                    self.device_states[serial].error = str(e)
    ...
    async def disconnect_device(self, serial: str) -> bool:
        ...
        async with self._lock:
            try:
                ...
                # Stop scrcpy-server
                if device:
                    await asyncio.to_thread(device.stop_server)
