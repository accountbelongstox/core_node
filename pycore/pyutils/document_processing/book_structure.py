# -*- coding: utf-8 -*-
"""
Build the structured representation of a book's text.
...
def build_book_structure(text: str, language: Optional[str] = None) -> Dict[str, Any]:
    ...
    for tok in segment_with_markers(text):
        ...
        if not content:
            continue
        norm = normalize_sentence_key(content)
        if not norm:
            continue
    ...
    book_cid = _content_id(normalize_sentence_key(strip_punctuation(text)))
