...
neural_tts_local_weights_ready (Linux), qwen.engine model resolution,
...
def audit_local_weights(verbose: bool = False) -> tuple:
    ...
    if not weights.is_dir():
        if verbose:
            ColorPrint.red(f"[AUDIT] Local weights dir missing: {weights}")
        return False, None, repo_id

    weight_files = sorted(
        p for p in weights.rglob("*") if p.is_file() and p.suffix.lower() in _core.WEIGHT_SUFFIXES
    )
    if not weight_files:
        if verbose:
            ColorPrint.red(f"[AUDIT] No weight files under {weights}")
        return False, None, repo_id
    ...
    for path in weight_files:
        ...
        expected = _core.catalog_bytes(repo_id or "", rel, _static_sizes(repo_id or ""))
        ok = True
        ...
        if expected > 0 and size < expected:
            ...
            ok = False
            detail = "empty file"
        elif path.suffix.lower() == ".safetensors" and not _core.safetensors_readable(path):
            ok = False
            detail = f"corrupt or incomplete ({size:,} bytes)"
        if verbose:
            status = "OK" if ok else "BAD"
            message = f"[AUDIT] {status} {rel}: {detail}"
            if ok:
