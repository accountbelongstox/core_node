...
class ModuleLoader:
    ...
    def __init__(self, pycore_root: Optional[Path] = None):
        self.pycore_root = Path(pycore_root or Path.cwd()).resolve()
    ...
    def load_module_standard(self, module_path: str):
        ...
        Args:
            module_path: Module path (e.g., 'pycore.pyutils.common.ocr.manager')
    ...
    def load_module_from_file(self, file_path: str, module_name: Optional[str] = None):
        ...
        if not Path(file_path).is_absolute():
            full_path = self.pycore_root / file_path
        else:
