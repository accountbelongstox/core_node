...
# Base name for the global indexed loader (scans GOOGLE_API_KEY_1.._5 then bare).
GOOGLE_API_KEY_BASE = "GOOGLE_API_KEY"
...
class GeminiManager:
    ...
    def __init__(self):
        ...
        # Logical key name for cache/info display. The actual value resolves via
        # the global indexed loader (GOOGLE_API_KEY_1.._5 then bare), so a key
        # stored under only _2 still loads — never hardcode a single index.
        self.default_api_key_name = GOOGLE_API_KEY_BASE
    ...
    def _get_client(self, api_key_name: Optional[str] = None) -> Optional[GeminiClient]:
        ...
        Args:
            api_key_name: Exact secret key name to use. When omitted, the value is
                resolved via the global indexed loader over GOOGLE_API_KEY_1.._5
                then bare GOOGLE_API_KEY (first non-empty wins).
        ...
        if key_name not in self.clients:
            # An explicit name is read exactly; otherwise scan the indexed variants.
            if api_key_name:
                api_key = get_secret_key(api_key_name)
            else:
                api_key = get_secret_key_indexed(GOOGLE_API_KEY_BASE)
