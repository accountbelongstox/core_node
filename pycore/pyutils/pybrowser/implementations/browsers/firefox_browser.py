#!/usr/bin/env python3
"""
Firefox Browser Implementation - Thread Mode

Firefox browser running in dedicated thread.
...
class FirefoxBrowser(ThreadedBrowser):
    ...
    Each FirefoxBrowser instance is a threading.Thread that:
    - Launches and manages its own Firefox WebDriver
    - Processes commands via internal queue
    - Provides thread-safe API for browser operations

    Usage:
        browser = FirefoxBrowser(config={'headless': False})
        browser.start()  # Launches Firefox in thread
    ...
    def __init__(self, config: Dict[str, Any] = None, thread_name: str = None):
        """
        Initialize Firefox browser thread
    ...
    def _get_driver_service(self):
        ...
        if driver_mode == 'auto':
            ...
            found_driver = find_driver('firefox')
            if found_driver:
        ...
        if driver_mode == 'system_path' or (driver_mode == 'local' and not driver_path):
            ...
            if system_driver:
                ...
                ColorPrint.red(f"{self.name}: GeckoDriver not found in system PATH")
    ...
    def _launch_browser(self):
        """
        Launch Firefox browser (runs in thread context)
        ...
        """
        ColorPrint.blue(f"{self.name}: Launching Firefox browser...")
        ...
        # Launch Firefox (errors propagate)
        ...
        ColorPrint.green(
            f"{self.name}: Firefox browser launched successfully (v{self.version})"
        )
    ...
    def set_preference(self, key: str, value: Any) -> bool:
        """
        Set Firefox preference (thread-safe)

        Note: This only works before browser launch.
        For runtime changes, restart the browser with new config.
        ...
        if not self.is_launched:
            ...
            ColorPrint.yellow(
                f"{self.name}: Cannot set preferences after browser launch. Restart browser with new config."
            )
            return False
    ...
    def __repr__(self) -> str:
        ...
        return f"<FirefoxBrowser name={self.name} status={status} browser={launched} version={version}>"
