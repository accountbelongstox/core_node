...
------------------------------------------------------------------------------
  * Singleton service registered as a PyHeartbeat callback (interval 60s).
    Pycore UI toggles it through the unified Assist user settings; Pycore owns
    the callback and the Laravel claim/report HTTP calls. Batch size uses the
    deployment default Config.TTS_WORKER_BATCH (env
...
from pycore.pyutils.tts.tts_queue_worker_threads import (
    run_tts_worker_lane,
    task_deque,
)
...
class TTSQueuePollerService:
    ...
    def __init__(self, laravel_api_url: str = ""):
        ...
        self.batch_size = max(1, min(_MAX_BATCH, TTS_WORKER_BATCH))
        ...
        ColorPrint.green(
            ...
            f"batch={self.batch_size}, enabled={assist_capability_enabled('tts')})"
        )
    ...
    def on_endpoint_changed(self, new_url: str) -> None:
        ...
        laravel_endpoint_manager.resolve() on each tick, so all we need is
    ...
    def _is_enabled(self) -> bool:
        ...
        try:
            return bool(
                shared_heartbeat_system.is_callback_enabled("tts_queue_poller")
            )
        except Exception:  # noqa: BLE001 — heartbeat not up yet
            return assist_capability_enabled("tts")
    ...
    def _report(
        ...
        try:
            if success:
                ...
                resp = laravel_client.post(REPORT_PATH, base_url=base, data=fields,
                                                 timeout=_REPORT_TIMEOUT)
        except Exception as e:
    ...
    def poll_and_process(self) -> None:
        ...
        flag, which is synchronized from the in-memory user settings map.
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "worker_id": self.worker_id,
            "enabled_on_start": assist_capability_enabled("tts"),
...
tts_queue_poller_service = TTSQueuePollerService()
