# -*- coding: utf-8 -*-
"""
Task Center service — one aggregate view over pycore's task layers.
...
from pycore.pyctl.queue_center.overview_service import (
    build_fast_lane,
    get_queue_overview,
)
...
def _remote_queue_section(
    ...
    return {
        ...
        "laravel_endpoint": laravel_endpoint_manager.peek_stored_base_url(),
        "laravel_active_endpoint": laravel_endpoint_manager.get_active_base_url(),
        "laravel_snapshot_age_s": snapshot_age_s,
        "event_connected": snapshot.get("event_connected", False),
...
def _control_state(
    ...
    word_audio_configured = master and bool(capabilities.get("tts"))
    sentence_audio_configured = master and bool(capabilities.get("sentence_audio"))
    translation_configured = master and (
        bool(capabilities.get("translation"))
        or bool(capabilities.get("ai_translate"))
    )
    return {
        "assist_translation": {
            ...
            "graceful_stop": to_bool(translation_intent.get("graceful_stop")),
        },
        "word_audio": {
            ...
            "graceful_stop": to_bool(word_audio_intent.get("graceful_stop")),
        },
        "sentence_audio": {
            ...
            "graceful_stop": to_bool(sentence_intent.get("graceful_stop")),
        },
    }
