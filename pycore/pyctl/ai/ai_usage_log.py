...
# Lives under <core_node>/.data/.ai_state; the prior <core_node>/.ai_state
...
def _migrate_old_state():
    ...
    <core_node>/.data/.ai_state once (idempotent; shared by all AI-state modules)."""
    try:
...
def _state_dir():
    """Shared ``.data/.ai_state`` dir under the core_node root (legacy fallback when
