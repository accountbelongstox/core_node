...
Focus: the Vortex Sandbox (shell label "Vortex Sandbox", pycore theme, route `/vortex`). The binding contract with pycore is `apps/vortex/api/VortexPycoreContract.ts`; its pycore side belongs to pycore.
...
- Every task subject starts with its owner role tag, e.g. `[vortex] ...`.
