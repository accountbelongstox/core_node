@echo off
REM VoiceClientAndCaddy NCore App Start Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for advanced logic
REM This bat file serves as the Windows entry point for starting VoiceClientAndCaddy application

echo [INFO] Starting VoiceClientAndCaddy NCore application...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] VoiceClientAndCaddy startup failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] VoiceClientAndCaddy startup completed
exit /b 0
