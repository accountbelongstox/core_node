@echo off
REM VoiceClientAndCaddy NCore App Deploy Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for production deployment
REM This bat file serves as the Windows entry point for deploying VoiceClientAndCaddy application

echo [INFO] Deploying VoiceClientAndCaddy NCore application...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] VoiceClientAndCaddy deployment failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] VoiceClientAndCaddy deployment completed
exit /b 0
