@echo off
REM VoiceClientAndCaddy NCore App Install Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for dependency management
REM This bat file serves as the Windows entry point for installing VoiceClientAndCaddy dependencies

echo [INFO] Installing VoiceClientAndCaddy NCore application dependencies...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] VoiceClientAndCaddy installation failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] VoiceClientAndCaddy installation completed
exit /b 0
