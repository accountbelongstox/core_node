...
Write-Host "[INFO] Deploying NCore application: ai_translator_app" -ForegroundColor Green
...
try {
    ...
    Write-Host "[INFO] Starting ai_translator_app in production mode..." -ForegroundColor Cyan
    $env:NODE_ENV = "production"
    node ./main.js app=ai_translator_app
}
catch {
    Write-Host "[ERROR] Failed to deploy ai_translator_app: $_" -ForegroundColor Red
    exit 1
}
