...
class CoreNodeInitMCPServer {
    ...
    async initialize() {
        try {
            ...
            // Lazy import to avoid starting services before singleton check
            ...
            singletonBrowser = getSingletonBrowser();
            frontendLauncher = getFrontendLauncher({
            ...
            try {
                ...
                if (serverResult.wsRpcServer) {
                    logger.info('ITTools WebSocket RPC server started on ws://localhost:19881');
                } else {
                    logger.warn('ITTools WebSocket RPC server failed to start');
                }

                if (serverResult.httpServer) {
                    logger.info('ITTools HTTP API server started on http://localhost:19880');
                    logger.info('Health check available at: http://localhost:19880/health/ping');
                }
            } catch (itToolsError) {
...
module.exports = CoreNodeInitMCPServer;
