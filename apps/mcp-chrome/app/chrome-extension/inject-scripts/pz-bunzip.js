 ...
 */
(() => {
  if (self.__pzBunzipReady) return;
  self.__pzBunzipReady = true;

  const bzip2 = {};

  bzip2.array = function array(bytes) {
    let bit = 0;
    let byte = 0;
  ...
  bzip2.header = function header(bits) {
  ...
  bzip2.decompress = function decompress(bits, size, len) {
  ...
  bzip2.simple = function simple(bits) {
  ...
  self.unpackDuoreaderPzBytes = function unpackDuoreaderPzBytes(byteList) {
    const xored = [];
    for (let i = 0; i < byteList.length; i += 1) {
      xored[i] = (byteList[i] ^ XOR_KEY) & 0xff;
    }
    const bits = bzip2.array(xored);
