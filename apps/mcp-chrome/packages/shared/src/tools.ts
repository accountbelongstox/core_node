...
export const TOOL_NAMES = {
  BROWSER: {
    ...
    WEB_SEARCH: 'chrome_web_search',
    BOOK_COVER_SEARCH: 'chrome_book_cover_search',
    BING_DICTIONARY: 'chrome_bing_dictionary',
    QWEN_TTS: 'chrome_qwen_tts',
    NOTEBOOKLM: 'chrome_notebooklm',
    NOTEBOOKLM_CREATE: 'chrome_notebooklm_create',
    GEMINI_IMAGE: 'chrome_gemini_image',
    TASK_CENTER: 'chrome_task_center',
  },
  DEEPSEEK: {
    SEND_PROMPT: 'chrome_deepseek_send_prompt',
    GET_TASK_STATUS: 'chrome_deepseek_get_task_status',
    GET_RESULT: 'chrome_deepseek_get_result',
    LIST_TASKS: 'chrome_deepseek_list_tasks',
    CANCEL_TASK: 'chrome_deepseek_cancel_task',
  },
  RECORD_REPLAY: {
...
// Tools blocked at the extension dispatch boundary on Firefox. Tools with a
// dedicated Firefox variant (dialog, console, file upload, performance) or an
// in-tool fallback (screenshot, network capture) are NOT listed here.
export const FIREFOX_UNSUPPORTED_TOOLS: ReadonlySet<string> = new Set<string>([
  TOOL_NAMES.BROWSER.JAVASCRIPT,
]);

export const FIREFOX_UNSUPPORTED_TOOL_REASONS: Record<string, string> = {
  [TOOL_NAMES.BROWSER.JAVASCRIPT]:
    'This tool requires the chrome.debugger API, which is not available to MV3 extensions on Firefox',
};
...
export const TOOL_SCHEMAS: Tool[] = [
  ...
  },
  {
    ...
    description: 'Navigate the current tab back or forward in its history',
    inputSchema: {
      type: 'object',
      properties: {
        forward: {
  ...
  },
  {
    name: TOOL_NAMES.BROWSER.SEARCH_TABS_CONTENT,
    description:
      'Semantically search the content of open browser tabs using the built-in vector engine',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Run a web search on Bing or Google and return structured results (web, images, or news)',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Search query text' },
        engine: {
          type: 'string',
          enum: ['bing', 'google'],
          description: 'Search engine to use (default: bing)',
        },
        mode: {
          type: 'string',
          enum: ['web', 'images', 'news'],
          description: 'Result mode (default: web)',
        },
        maxResults: {
          type: 'number',
        ...
        waitForVerification: {
          type: 'boolean',
          description: 'Wait through engine verification challenges (default: false)',
        },
        verificationTimeoutMs: {
          type: 'number',
          description: 'Verification wait timeout in ms (default: 120000)',
        },
        openInNewTab: {
          type: 'boolean',
          description: 'Open the search in a new tab instead of reusing one (default: false)',
        },
        tabId: { type: 'number', description: 'Optional target tab ID' },
      },
      required: ['query'],
  ...
  },
  {
    ...
    description: 'Search book cover image URLs by title and optional author',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        engine: {
          type: 'string',
          enum: ['google', 'bing'],
          description: 'Preferred image search engine (default: google)',
        },
        waitForVerification: {
          type: 'boolean',
          description: 'Wait through engine verification challenges (default: false)',
  ...
  },
  {
    ...
    description:
      'Look up a word in the Bing dictionary; returns phonetics (with audio), translations, definitions, examples, synonyms, and sample images',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        openInNewTab: {
          type: 'boolean',
          description: 'Open the dictionary page in a new tab (default: false)',
        },
      },
      required: ['word'],
    },
  ...
  {
    name: TOOL_NAMES.BROWSER.QWEN_TTS,
    description:
      'Generate speech audio with Qwen TTS (voice design, voice clone, or custom voice) and optionally download the file',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        mode: {
          type: 'string',
          enum: ['voice_design', 'voice_clone', 'custom_voice'],
          description: 'TTS mode (default: voice_design)',
        },
        voiceDescription: {
          type: 'string',
          description: 'Natural-language voice description (voice_design mode)',
        },
        language: { type: 'string', description: 'Text language hint' },
        styleInstruction: { type: 'string', description: 'Speaking style instruction' },
        speaker: { type: 'string', description: 'Speaker name (custom_voice mode)' },
        waitTimeoutMs: {
          type: 'number',
          description: 'Generation wait timeout in ms (default: 180000)',
        },
        openInNewTab: {
          type: 'boolean',
          description: 'Open the Qwen TTS page in a new tab (default: false)',
        },
        tabId: { type: 'number', description: 'Optional target tab ID' },
        download: {
          type: 'boolean',
          description: 'Download the generated audio (default: true)',
        },
  ...
  },
  {
    ...
    description:
      'Ask a question in a Google NotebookLM notebook chat and return the source-grounded answer, or extract the latest answer',
    inputSchema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'Question to ask; omit to extract the latest answer',
        },
        prompt: { type: 'string', description: 'Alias of question' },
        notebookUrl: {
          type: 'string',
          description: 'Notebook URL to open/reuse (default: reuse an open NotebookLM tab)',
        },
        timeoutMs: {
          type: 'number',
          description: 'Answer wait timeout in ms (default: 60000)',
        },
      },
      required: [],
    },
  ...
  {
    name: TOOL_NAMES.BROWSER.NOTEBOOKLM_CREATE,
    description:
      'Create a Google NotebookLM notebook from a text/topic prompt (async start/status job pair). Requires an authenticated Google session',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          ...
          enum: ['start', 'status'],
          description:
            '"start": create the notebook and add the text source; "status": poll a running job (default: start, or status when jobId is given)',
        },
        text: { type: 'string', description: 'Source text/topic (start)' },
        jobId: { type: 'string', description: 'Job ID returned by start (status)' },
        openInNewTab: {
          type: 'boolean',
          description: 'Open NotebookLM in a new tab (default: false)',
        },
        timeoutMs: { type: 'number', description: 'Operation timeout in ms' },
      },
      required: [],
    },
  },
  {
    name: TOOL_NAMES.BROWSER.GEMINI_IMAGE,
    description:
      'Generate images with Google Gemini (async start/status job pair). Requires an authenticated Google session',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          ...
          enum: ['start', 'status'],
          description:
            '"start": submit the image prompt; "status": poll a running job (default: start, or status when jobId is given)',
        },
        prompt: { type: 'string', description: 'Image generation prompt (start)' },
        jobId: { type: 'string', description: 'Job ID returned by start (status)' },
        openInNewTab: {
          type: 'boolean',
          description: 'Open Gemini in a new tab (default: false)',
        },
        timeoutMs: { type: 'number', description: 'Operation timeout in ms' },
      },
      required: [],
  ...
  },
  {
    ...
    description:
      'Inspect and control the extension task center (word pipeline capabilities and validity tests)',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
        ...
        capabilities: {
          type: 'array',
          items: { type: 'string' },
          description: 'Capabilities to activate (start)',
        },
        capability: { type: 'string', description: 'Capability key (set_capability)' },
        enabled: { type: 'boolean', description: 'Enable flag (set_capability)' },
        words: { type: 'array', items: { type: 'string' }, description: 'Words (test_validity)' },
        provider: {
          type: 'string',
          enum: ['deepseek', 'gemini', 'chatgpt'],
          description: 'AI provider (test_validity)',
        },
        targetLanguage: { type: 'string', description: 'Target language (test_validity)' },
        timeoutMs: { type: 'number', description: 'Operation timeout in ms' },
      },
      required: ['action'],
    },
  },
  {
    ...
    description: 'Start recording audio from the current browser tab',
    inputSchema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: TOOL_NAMES.BROWSER.AUDIO_STOP,
    description: 'Stop the current tab audio recording and return the recording',
    inputSchema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: TOOL_NAMES.BROWSER.AUDIO_STATUS,
    description: 'Get the current tab audio recording status',
    inputSchema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: TOOL_NAMES.BROWSER.AUDIO_DURATION,
    description: 'Get the duration of the current tab audio recording',
    inputSchema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: TOOL_NAMES.DEEPSEEK.SEND_PROMPT,
    description:
      'Send a prompt to DeepSeek Chat, optionally waiting for the answer; returns a task ID for polling',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        waitForCompletion: {
          type: 'boolean',
          description: 'Wait for the answer before returning (default: false)',
        },
        timeout: { type: 'number', description: 'Completion timeout in ms (default: 300000)' },
        timeoutMs: { type: 'number', description: 'Alias of timeout' },
        autoRetry: { type: 'boolean', description: 'Auto-retry on failure (default: true)' },
        newConversation: {
          type: 'boolean',
          description: 'Start a new conversation (default: false)',
        },
        model: { type: 'string', description: 'Model name hint' },
        deepThink: { type: 'boolean', description: 'Enable deep thinking mode' },
        search: { type: 'boolean', description: 'Enable web search (default: false)' },
      },
      required: ['prompt'],
    },
  },
  {
    ...
    description: 'Get the status of a DeepSeek task',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Get the result of a DeepSeek task, optionally waiting for completion',
    inputSchema: {
      type: 'object',
      properties: {
        taskId: { type: 'string', description: 'Task ID' },
        ...
        timeout: { type: 'number', description: 'Completion timeout in ms (default: 60000)' },
      },
      required: ['taskId'],
    },
  },
  {
    ...
    description: 'List DeepSeek tasks with optional status filter and pagination',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Cancel a running DeepSeek task',
    inputSchema: {
      type: 'object',
      properties: {
        taskId: { type: 'string', description: 'Task ID' },
  ...
  },
  {
    name: 'chrome_chatgpt',
    description:
      'Send a prompt to ChatGPT in the browser (async start/status job pair) and optionally capture the read-aloud audio',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['start', 'status'],
          description: '"start": submit the prompt; "status": poll a running job',
        },
        prompt: { type: 'string', description: 'Prompt text (start)' },
        jobId: { type: 'string', description: 'Job ID returned by start (status)' },
        withAudio: {
          type: 'boolean',
          description: 'Capture the read-aloud audio of the reply (default: false)',
        },
        tabId: { type: 'number', description: 'Optional target tab ID' },
        timeoutMs: { type: 'number', description: 'Operation timeout in ms' },
      },
  ...
  },
  {
    name: 'chrome_gemini',
    description:
      'Send a prompt to Google Gemini in the browser (async start/status job pair) and optionally capture the reply audio',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
        ...
        withAudio: {
          type: 'boolean',
          description: 'Capture the reply audio (default: false)',
        },
        tabId: { type: 'number', description: 'Optional target tab ID' },
        timeoutMs: { type: 'number', description: 'Operation timeout in ms' },
      },
