...
```
┌────────────────────┐     ┌─────────────────────────┐     ┌────────────────────┐
│ 弹窗 / MCP 工具    │ --> │ 后台工具处理器          │ --> │ 离屏录音器          │
│ - API 服务器配置   │     │ - 配置合并              │     │ - 标签 & 麦克风采集 │
│ - 会话元数据       │     │ - 离屏生命周期管理      │     │ - Web Audio 混音     │
└────────────────────┘     └────────────┬────────────┘     └─────────┬──────────┘
                                        │                            │
                                        │                            │
                                        ▼                            ▼
                             ┌───────────────────┐         ┌────────────────────┐
                             │ WebSocket 服务端  │         │ HTTP Chunk/文件 API │
                             │ - Metadata 消息   │         │ - FormData 负载     │
                             │ - 二进制音频帧    │         │ - chunkIndex/isFinal│
                             └───────────────────┘         └────────────────────┘
```

1. **配置收集**：弹窗持久化服务器列表与 JSON 元数据，MCP 工具可在运行时覆盖。
2. **后台合并**：`handleAudioStart` 规范化服务器数组，注入元数据，创建离屏文档并获取标签捕获流 ID。
3. **离屏录音**：混合标签与麦克风音频、执行静音检测，并通过 WebSocket/HTTP 推送 MediaRecorder chunk。
4. **元数据传播**：每次 HTTP 请求携带 `audio`、`chunkIndex`、`timestamp`、`isFinal` 以及所有 `sessionMetadata` 字段；WebSocket 会在二进制帧前发送 `{ type: 'metadata', data: {...} }`。
5. **后端集成**：例如 Laravel AppQyV1 可读取这些字段，执行 FFmpeg 转码并复用现有存储管理器。
