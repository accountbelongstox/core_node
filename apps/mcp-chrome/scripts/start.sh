...
mcp_ensure_node_deps() {
    ...
    if [ -z "$mcp_node_bin_dir" ]; then
        ...
        echo -e "${YELLOW}  Run prerequisite: scripts/shells/linux/debian/install_shells/15_install_node_24.sh${NC}"
        exit 1
    fi
    ...
    if ! command -v pnpm &>/dev/null; then
        ...
        echo -e "${YELLOW}  Run prerequisite: scripts/shells/linux/debian/install_shells/28_ensure_pnpm_packages.sh${NC}"
        exit 1
    fi
