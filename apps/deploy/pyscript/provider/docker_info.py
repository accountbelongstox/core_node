...
class DockerInfo(Base):
    ...
    def gen_docker_id(self):
        root_id = "root_id_"+strtool.generate_id()
        return root_id

    def get_docker_dir(self):
        docker_id = self.gen_docker_id()
        main_dir = env.get_env("MAIN_DIR")
        docker_dir = os.path.join(main_dir, f"docker/{docker_id}")
    ...
    def get_private_mirrors(self):
        REGISTRY_PORT = env.get_env("REGISTRY_PORT")
        private_mirrors = [
            f"http://{baseServer}:{REGISTRY_PORT}"
        ]
        return  private_mirrors
