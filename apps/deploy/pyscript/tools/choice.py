...
class Choice(Base):
    ...
    def set_and_collection_envs(self, configs, setting_name="", show=False):
        if not configs:
            return
        self.collection_settings(configs, setting_name)
        if setting_name != "":
        ...
        for item in configs:
            ...
            ENV.set_env(key, val)
            settings.save_to_tmp_settings(key,val,APP_NAME)
