from pycore.utils_linux import file

FLASK_NAME = "FlaskMain"
DEBUG = True
TESTING = False
SECRET_KEY = '\xba\x1c0\x85crl#\xfa\xa9[\xa3\xb4\x8c\xe2\xd4@l\xff\x92\x1f\xb6\xbc\xbb'
PORT = 3000
SOCKET_PORT = 3000
STATIC_FOLDER = file.resolve_path('static', "apps/deploy/web")
TEMPLATE_FOLDER = file.resolve_path('templates', "out/flask_router")
TEMP_FOLDER = file.resolve_path('tampermonkey', "out/flask_router")
AUTHENTICATIONS_PATHS = [""]

class FlaskCouter:
    def get_config(self):
        config_dict = {
            'FLASK_NAME': FLASK_NAME,
            'DEBUG': DEBUG,
            'TESTING': TESTING,
            'SECRET_KEY': SECRET_KEY,
            'PORT': PORT,
            'SOCKET_PORT': SOCKET_PORT,
            'STATIC_FOLDER': STATIC_FOLDER,
            'TEMPLATE_FOLDER': TEMPLATE_FOLDER,
            'TEMP_FOLDER': TEMP_FOLDER,
            'AUTHENTICATIONS_PATHS': AUTHENTICATIONS_PATHS,
        }
        return config_dict

flask_couter = FlaskCouter().get_config()
