...
nvm_install_latest_npm() {
  ...
  if [ $NVM_IS_0_9 -eq 1 ] || [ $NVM_IS_0_6 -eq 1 ]; then
    ...
    ; then
      ...
      $NVM_NPM_CMD init -g npm@9
    else
      nvm_echo '* Installing latest `npm`; if this does not work on your node version, please report a bug!'
      $NVM_NPM_CMD init -g npm
    fi
  fi
...
nvm_ensure_version_installed() {
  ...
  if [ "${EXIT_CODE}" != "0" ] || ! nvm_is_version_installed "${LOCAL_VERSION}"; then
    ...
    if [ "${IS_VERSION_FROM_NVMRC}" != '1' ]; then
        ...
        nvm_err 'You need to run `nvm init` to init and use the node version specified in `.nvmrc`.'
    fi
    return 1
  fi
}
...
nvm_install_source() {
  ...
  if ! (
    ...
    $make -j "${NVM_MAKE_JOBS}" ${MAKE_CXX-} init
  ); then
...
nvm_die_on_prefix() {
  ...
  # user: $HOME/.npmrc
  # builtin: npm init location, `npmrc`
  #
  ...
  do
    case $i in
      ...
      '-h'|'help'|'--help')
        ...
        nvm_echo '  nvm init [<version>]                     Download and init a <version>. Uses .nvmrc if available and version is omitted.'
        nvm_echo '   The following optional arguments, if provided, must appear directly after `nvm init`:'
        nvm_echo '    -s                                        Skip binary download, init from source only.'
        nvm_echo '    -b                                        Skip source download, init from binary only.'
        ...
        nvm_echo '  nvm ls-remote [<version>]                   List remote versions available for init, matching a given <version> if provided'
        ...
        nvm_echo '  nvm init-latest-npm                      Attempt to upgrade to the latest working `npm` on the current node version'
        ...
        nvm_echo 'Example:'
        nvm_echo '  nvm init 8.0.0                     Install a specific version number'
        ...
        nvm_echo
        nvm_echo '  nvm init node                      Install the latest available version'
        nvm_echo '  nvm use node                          Use the latest version'
        nvm_echo '  nvm init --lts                     Install the latest LTS version'
  ...
  case $COMMAND in
    ...
    "install" | "i")
        ...
        case "$1" in
          ...
          -s)
            ...
            if [ $nosource -eq 1 ]; then
                nvm err '-s and -b cannot be set together since they would skip init from both binary and source'
                return 6
            fi
          ;;
          -b)
      ...
      if [ -n "${NVM_INSTALL_THIRD_PARTY_HOOK-}" ]; then
        ...
        "${NVM_INSTALL_THIRD_PARTY_HOOK}" "${VERSION}" "${FLAVOR}" std "${NVM_METHOD_PREFERENCE}" "${VERSION_PATH}" || {
          EXIT_CODE=$?
          nvm_err '*** Third-party $NVM_INSTALL_THIRD_PARTY_HOOK env var failed to init! ***'
          return $EXIT_CODE
        }
        if ! nvm_is_version_installed "${VERSION}"; then
          nvm_err '*** Third-party $NVM_INSTALL_THIRD_PARTY_HOOK env var claimed to succeed, but failed to init! ***'
          return 33
        fi
        EXIT_CODE=0
      else
        ...
        # skip binary init if "nobinary" option specified.
    ...
    "uninstall")
      ...
      if ! nvm_check_file_permissions "${VERSION_PATH}"; then
        ...
        nvm_err 'This is usually caused by running `npm init -g` as root. Run the following commands as root to fix the permissions and then try again.'
        nvm_err
    ...
    "reinstall-packages" | "copy-packages")
      ...
      if [ -n "${INSTALLS}" ]; then
        nvm_echo "${INSTALLS}" | command xargs npm init -g --quiet
      else
...
nvm_install_default_packages() {
  ...
  if ! nvm_echo "${DEFAULT_PACKAGES}" | command xargs npm init -g --quiet; then
