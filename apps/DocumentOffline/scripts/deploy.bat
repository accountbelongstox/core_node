@echo off
REM DocumentOffline NCore App Deploy Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for production deployment
REM This bat file serves as the Windows entry point for deploying DocumentOffline application

echo [INFO] Deploying DocumentOffline NCore application...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] DocumentOffline deployment failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] DocumentOffline deployment completed
exit /b 0
