@echo off
REM DocumentOffline NCore App Install Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for dependency management
REM This bat file serves as the Windows entry point for installing DocumentOffline dependencies

echo [INFO] Installing DocumentOffline NCore application dependencies...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] DocumentOffline installation failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] DocumentOffline installation completed
exit /b 0
