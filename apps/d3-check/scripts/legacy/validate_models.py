...
class ModelValidator:
    ...
    def __init__(self, models_dir: Optional[Path] = None):
        ...
        Args:
            models_dir: Directory containing trained models (defaults to .cache/training_data/d4_modules/classification)
        """
        self.project_root = Path(current_dir)
        # Default to unified training output directory for classification models
        default_models_dir = self.project_root / ".cache" / "training_data" / "d4_modules" / "classification"
    ...
    def load_registry(self) -> bool:
        ...
        if not self.models_dir.exists():
            ...
            ColorPrint.yellow("\n   Please train classification models first:")
            ColorPrint.yellow("   python train_all.py --mode classification")
            return False
        ...
        for model_file in model_files:
            # For classification models, we need to infer the expected image size
            # Default to 76x76 (common for UI elements)
            img_size = {'width': 76, 'height': 76}
        ...
        for model_info in self.registry['models']:
            ColorPrint.green(f"   - {model_info['model_name']}: {model_info['img_size']['width']}x{model_info['img_size']['height']}")
