...
ncore_path = os.path.join(os.path.dirname(os.path.dirname(project_root)), "ncore")
...
class BagLayoutDetector:
    ...
    # Color similarity threshold for separator line detection (2% tolerance)
    SEPARATOR_COLOR_TOLERANCE = 0.02
    # Scan width ratio for separator detection (178/217 ≈ 0.82)
    SEPARATOR_SCAN_WIDTH_RATIO = 178.0 / 217.0
    ...
    COLOR_REFERENCES = {
        ...
        'yellow': [
            (0x0f, 0x48, 0x50),  # 50480f
            (0x15, 0x50, 0x56),  # 565015
            (0x1e, 0x64, 0x6e),  # 6e641e
            (0x18, 0x52, 0x5a),  # 5a5218
            (0x27, 0xa3, 0xaf),  # afa327
        ],
        'dark_gold_1slot': [
    ...
    # Color tolerance (5% of 255)
    COLOR_TOLERANCE = int(255 * 0.05)
    ...
    def _has_separator_line(
        ...
        Strategy:
        1. Take the middle 10% height range between the two slots
        2. For each pixel row in this range:
           - Take the center pixel of the row as reference
           - Scan the entire row (excluding 2% from each edge)
           - If all pixels are within 2% tolerance of center pixel → uniform row → separator line
        ...
        """
        # Calculate scan region (middle 10% between two slots)
        ...
        # Scan range: 10% of slot height centered on boundary
        scan_range = int(slot_height * 0.10)
        y1 = max(0, boundary_y - scan_range // 2)
        y2 = min(bag_image.shape[0], boundary_y + scan_range // 2)
        ...
        for scan_row_idx in range(gray.shape[0]):
            ...
            # Calculate scan width centered on the row
            # Example: if row width is 217, scan width is 178 (82%)
            scan_width = int(len(row_pixels) * self.SEPARATOR_SCAN_WIDTH_RATIO)
            if scan_width < 3:  # Need at least 3 pixels to scan
                scan_width = min(3, len(row_pixels))

            # Get center index
            center_idx = len(row_pixels) // 2

            # Calculate start and end indices centered on center_idx
            half_scan = scan_width // 2
            start_idx = max(0, center_idx - half_scan)
            end_idx = min(len(row_pixels), center_idx + half_scan)

            # Get pixels to check (centered scan region)
            pixels_to_check = row_pixels[start_idx:end_idx]

            if len(pixels_to_check) == 0:
                continue

            # Get center pixel as reference
            center_pixel = float(row_pixels[center_idx])

            # Check if all pixels are within tolerance of center pixel
            tolerance = 255 * self.SEPARATOR_COLOR_TOLERANCE  # 2% of 255

            all_similar = True
            for pixel_val in pixels_to_check:
                if abs(float(pixel_val) - center_pixel) > tolerance:
                    all_similar = False
                    break
            ...
            if all_similar:
                ColorPrint.gray(f"[Separator] Found uniform row at ({row},{col})->({row+1},{col}), "
                              f"center_pixel={center_pixel:.1f}, scan_width={len(pixels_to_check)}")
                return True
    ...
    def _calculate_color_percentages(self, region: np.ndarray, is_2slot: bool = False) -> Dict:
        ...
        for pixel in non_black_pixels:
            ...
            # Try to match each color category
            # Blue
            for ref_b, ref_g, ref_r in self.COLOR_REFERENCES['blue']:
                if (abs(int(b) - ref_b) <= self.COLOR_TOLERANCE and
                    abs(int(g) - ref_g) <= self.COLOR_TOLERANCE and
                    abs(int(r) - ref_r) <= self.COLOR_TOLERANCE):
                    color_counts['blue'] += 1
                    matched = True
                    break
            ...
            # Yellow
            for ref_b, ref_g, ref_r in self.COLOR_REFERENCES['yellow']:
                if (abs(int(b) - ref_b) <= self.COLOR_TOLERANCE and
                    abs(int(g) - ref_g) <= self.COLOR_TOLERANCE and
                    abs(int(r) - ref_r) <= self.COLOR_TOLERANCE):
            ...
            # Green
            for ref_b, ref_g, ref_r in self.COLOR_REFERENCES['green']:
                if (abs(int(b) - ref_b) <= self.COLOR_TOLERANCE and
                    abs(int(g) - ref_g) <= self.COLOR_TOLERANCE and
                    abs(int(r) - ref_r) <= self.COLOR_TOLERANCE):
            ...
            # For 2-slot items, check 2-slot references first
            dark_gold_refs = []
            if is_2slot:
                dark_gold_refs.extend(self.COLOR_REFERENCES['dark_gold_2slot'])
            dark_gold_refs.extend(self.COLOR_REFERENCES['dark_gold_1slot'])

            for ref_b, ref_g, ref_r in dark_gold_refs:
                if (abs(int(b) - ref_b) <= self.COLOR_TOLERANCE and
                    abs(int(g) - ref_g) <= self.COLOR_TOLERANCE and
                    abs(int(r) - ref_r) <= self.COLOR_TOLERANCE):
            ...
            # Black (dark pixels that don't match other colors)
            for ref_b, ref_g, ref_r in self.COLOR_REFERENCES['black']:
                if (abs(int(b) - ref_b) <= self.COLOR_TOLERANCE and
                    abs(int(g) - ref_g) <= self.COLOR_TOLERANCE and
                    abs(int(r) - ref_r) <= self.COLOR_TOLERANCE):
            ...
            if color_analysis:
                ...
                for (row, col), colors in color_analysis.items():
                    if 'extract_region' in colors:
                        ...
                        cv2.rectangle(extraction_bag, (x1, y1), (x2, y2), (0, 255, 0), 2)
