@echo off
REM DevOps NCore App Deploy Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for production deployment
REM This bat file serves as the Windows entry point for deploying DevOps application

echo [INFO] Deploying DevOps NCore application...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] DevOps deployment failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] DevOps deployment completed
exit /b 0
