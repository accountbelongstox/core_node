@echo off
REM DevOps NCore App Install Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for dependency management
REM This bat file serves as the Windows entry point for installing DevOps dependencies

echo [INFO] Installing DevOps NCore application dependencies...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] DevOps installation failed with exit code: %ERRORLEVEL%
    exit /b %ERRORLEVEL%
)

echo [SUCCESS] DevOps installation completed
exit /b 0
