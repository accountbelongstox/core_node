@echo off
REM WebLocalAreaNetwork NCore App Deploy Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for production deployment
REM This bat file serves as the Windows entry point for deploying WebLocalAreaNetwork application

echo [INFO] Deploying WebLocalAreaNetwork NCore application...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] WebLocalAreaNetwork deployment failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] WebLocalAreaNetwork deployment completed
exit /b 0
