@echo off
REM WebLocalAreaNetwork NCore App Install Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for dependency management
REM This bat file serves as the Windows entry point for installing WebLocalAreaNetwork dependencies

echo [INFO] Installing WebLocalAreaNetwork NCore application dependencies...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] WebLocalAreaNetwork installation failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] WebLocalAreaNetwork installation completed
exit /b 0
