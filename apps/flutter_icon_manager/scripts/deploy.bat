@echo off
REM flutter_icon_manager NCore App Deploy Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for production deployment
REM This bat file serves as the Windows entry point for deploying flutter_icon_manager application

echo [INFO] Deploying flutter_icon_manager NCore application...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] flutter_icon_manager deployment failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] flutter_icon_manager deployment completed
exit /b 0
