@echo off
REM GetDocFromUrlByPuppeteer NCore App Start Script (BAT Entry Point)
REM Complexity: Complex - Triggers PowerShell script for advanced logic
REM This bat file serves as the Windows entry point for starting GetDocFromUrlByPuppeteer application

echo [INFO] Starting GetDocFromUrlByPuppeteer NCore application...
...
REM Check exit code
if %ERRORLEVEL% neq 0 (
    echo [ERROR] GetDocFromUrlByPuppeteer startup failed with exit code: %ERRORLEVEL%
        exit /b %ERRORLEVEL%
)

echo [SUCCESS] GetDocFromUrlByPuppeteer startup completed
exit /b 0
