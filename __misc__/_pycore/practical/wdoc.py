...
class Wdoc(Base):
    ...
    def count_document_words(self, doc):
        splits_symbol = re.compile(r'[^a-zA-Z\'\-\_]+')
    ...
    def extra_words(self, doc,splits_symbol=None):
        ...
        if isinstance(doc, list):
            ...
            # splits_symbol = splits_symbol or re.compile(r'(?<=[a-z])(?=[A-Z])|[^a-zA-Z\'\-]+')
    ...
    def sentence_filter(self, simple):
        ...
        # axx axx axx 形式
        first_alphabets = list(set(re.compile(r'^[a-zA-Z]{1}|(?<=\s)[a-zA-Z]{1}').findall(simple)))
        ...
        # xxx !0 xxx形式清除
        words = re.compile(r'[^a-zA-Z0-9]+').split(simple)
    ...
    def sort_word(self, word):
        ...
        notalphabet = re.compile(r"[^a-z]", re.IGNORECASE)
